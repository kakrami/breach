...
export function createObstacleGrid({ cellSize = 8, cellHeight = 3, playerHeight }) {
  ...
  function register(obstacle) {
    ...
    const minCX = Math.floor(minX / cellSize), maxCX = Math.floor(maxX / cellSize);
    const minCY = Math.floor(obstacle.minY / cellHeight), maxCY = Math.floor(obstacle.maxY / cellHeight);
    const minCZ = Math.floor(minZ / cellSize), maxCZ = Math.floor(maxZ / cellSize);
    const entry = { obstacle, visit:0 }; entries.push(entry);
  ...
  let stamp = 0;
  function blocked(x, z, y, height = playerHeight) {
    stamp = (stamp + 1) >>> 0;
    if (!stamp) { for (const entry of entries) entry.visit = 0; stamp = 1; }
    const cx = Math.floor(x / cellSize), cz = Math.floor(z / cellSize);
    const minCY = Math.floor(y / cellHeight), maxCY = Math.floor((y + playerHeight * .92) / cellHeight);
    for (let cy = minCY; cy <= maxCY; cy += 1) {
      ...
      for (const entry of list) {
        ...
        if (!(y + height * .92 > obstacle.minY && y < obstacle.maxY - .04)) continue;
        if (obstacle.type === 'box') {
          if (x > obstacle.minX && x < obstacle.maxX && z > obstacle.minZ && z < obstacle.maxZ) return true;
        } else if (Math.hypot(x - obstacle.x, z - obstacle.z) < obstacle.r) return true;
      }
    }
    return false;
  }
