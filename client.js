...
import * as HighlandsGeometry from './world-geometry.js?v=1.41.0';
import * as DepotGeometry from './world-geometry-depot.js?v=1.41.0';
import * as YardGeometry from './world-geometry-yard.js?v=1.41.0';
import * as RigGeometry from './world-geometry-rig.js?v=1.41.0';
import * as HighlandsWorldCollision from './world-collision.js?v=1.41.0';
import * as DepotWorldCollision from './world-collision-depot.js?v=1.41.0';
import * as YardWorldCollision from './world-collision-yard.js?v=1.41.0';
import * as RigWorldCollision from './world-collision-rig.js?v=1.41.0';
import {
...
} from './game-config.js?v=1.41.0';
import { createProjectileCollisionGrid } from './collision-grid.js?v=1.41.0';
import { createAudioEngine } from './audio-engine.js?v=1.41.0';
import { normalizeMatchState as normalizeSharedMatchState } from './match-model.js?v=1.41.0';
import { MATCH_STATUS, matchAllowsLobbyEdits, matchAllowsMovement, matchAllowsCombat, matchPhaseChanged } from './gameplay-phase.js?v=1.41.0';
import { MAX_PLAYER_PHYSICS_STEP_SEC, advanceVerticalMotion, advanceKnockback, sweepHorizontalMovement, createTraversalPlan, traversalPose, tacticalThrowVelocity, LADDER_CLIMB_SPEED, ladderById, ladderClimbPoint, ladderBottomExitPoint, ladderTopExitPoint, findLadderEntry, ladderClimbStep } from './movement-model.js?v=1.41.0';
import { SHELL_PANEL, createSessionShell, detectInputPlatform } from './app-lifecycle.js?v=1.41.0';
import { GAMEPAD_BUTTON, createGamepadInput } from './gamepad-input.js?v=1.41.0';
...
const loadoutFocusSlot={lobby:'primary',match:'primary'},loadoutPreviewDesired=new Map(),loadoutPreviewContexts=new Map(),loadoutAttachmentSlot={'lobby-primary':'optic','lobby-secondary':'optic','match-primary':'optic','match-secondary':'optic'},loadoutPreviewAdsKeys=new Set();
...
function loadoutComparisonAttachments(surface,weaponSlot,weapon,attachments){const activeSlot=activeLoadoutAttachmentSlot(surface,weaponSlot,weapon),compare={...normalizeWeaponAttachments(weapon,attachments)};if(activeSlot)compare[activeSlot]='';return compare;}
...
function renderAttachmentEditor(surface,weaponSlot,draft){
  ...
  const activeSlot=activeLoadoutAttachmentSlot(surface,weaponSlot,weapon),activeItems=groups.find(([slot])=>slot===activeSlot)?.[1]||[],equipped=ATTACHMENT_SLOTS.reduce((n,slot)=>n+(current?.[slot]?1:0),0),head=document.createElement('div');head.className='gunsmith-attachment-head';const title=document.createElement('strong');title.textContent='ATTACHMENTS';const count=document.createElement('small');count.textContent=`${equipped} / ${groups.length} EQUIPPED`;head.append(title,count);
  const slotGrid=document.createElement('div');slotGrid.className='gunsmith-slot-grid';for(const [slot] of groups){const id=current?.[slot]||'',item=id?ATTACHMENTS[id]:null,b=document.createElement('button');b.type='button';b.className='gunsmith-slot-button';b.classList.toggle('active',slot===activeSlot);b.classList.toggle('equipped',!!item);const label=document.createElement('span');label.textContent=slot.toUpperCase();const value=document.createElement('strong');value.textContent=item?.short||item?.name||'NONE';b.append(label,value);b.addEventListener('click',()=>{loadoutAttachmentSlot[`${surface}-${weaponSlot}`]=slot;renderAttachmentEditor(surface,weaponSlot,loadoutDraftForSurface(surface));syncLoadoutFocusedView(surface,loadoutDraftForSurface(surface));});slotGrid.append(b);}
  ...
  const selectedId=current?.[activeSlot]||'',selectedItem=selectedId?ATTACHMENTS[selectedId]:null,detailBox=document.createElement('div');detailBox.className='gunsmith-attachment-detail';const detailTitle=document.createElement('strong');detailTitle.textContent=selectedItem?.name||`BASE ${activeSlot.toUpperCase()}`;const desc=document.createElement('span');desc.textContent=selectedItem?.description||`No ${activeSlot} attachment equipped.`;const tagWrap=document.createElement('div');tagWrap.className='gunsmith-effect-tags';for(const tag of attachmentEffectTags(selectedItem,weapon)){const chip=document.createElement('i');chip.className=`gunsmith-effect-tag ${tag.good?'good':'bad'}`;chip.textContent=tag.text;tagWrap.append(chip);}detailBox.append(detailTitle,desc,tagWrap);host.append(head,slotGrid,options,detailBox);
}
...
const ENGINE_MODULE_URL = './vendor/three.module.min.js?v=1.41.0';
...
function closeMatchLoadout(){loadoutDraft=null;loadoutBaseDraft=null;matchClassDrafts=null;matchClassBase=null;shell.closePanel(SHELL_PANEL.LOADOUT);}
...
const CONTROLLER_NAV_GROUP_SELECTOR='.settings-grid,.weapon-fields,.loadout-choice-grid,.attachment-editor,.gunsmith-slot-grid,.gunsmith-attachment-options,.loadout-class-strip,.loadout-weapon-tabs,.lobby-mode-picker,.lobby-setup-row,.lobby-team-picker,.admin-bot-controls,.pause-actions,.pause-actions-player,.pause-actions-system,.join-controls,.lobby-map-choice-grid,.lobby-cheat-tabs,.chat-key-row,.game-text-key-row';
