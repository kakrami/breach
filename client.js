import { createMoonSky } from './mod-environment.js?v=1.46.0';
window.__breachModuleBooted=true;
import * as HighlandsGeometry from './world-geometry.js?v=1.46.0';
import * as DepotGeometry from './world-geometry-depot.js?v=1.46.0';
import * as YardGeometry from './world-geometry-yard.js?v=1.46.0';
import * as RigGeometry from './world-geometry-rig.js?v=1.46.0';
import * as HighlandsWorldCollision from './world-collision.js?v=1.46.0';
import * as DepotWorldCollision from './world-collision-depot.js?v=1.46.0';
import * as YardWorldCollision from './world-collision-yard.js?v=1.46.0';
import * as RigWorldCollision from './world-collision-rig.js?v=1.46.0';
import {
...
} from './game-config.js?v=1.46.0';
import { createProjectileCollisionGrid } from './collision-grid.js?v=1.46.0';
import { createAudioEngine } from './audio-engine.js?v=1.46.0';
import { normalizeMatchState as normalizeSharedMatchState } from './match-model.js?v=1.46.0';
import { MATCH_STATUS, matchAllowsLobbyEdits, matchAllowsMovement, matchAllowsCombat, matchPhaseChanged } from './gameplay-phase.js?v=1.46.0';
import { MAX_PLAYER_PHYSICS_STEP_SEC, advanceVerticalMotion, advanceKnockback, sweepHorizontalMovement, createTraversalPlan, traversalPose, tacticalThrowVelocity, LADDER_CLIMB_SPEED, ladderById, ladderClimbPoint, ladderBottomExitPoint, ladderTopExitPoint, findLadderEntry, ladderClimbStep } from './movement-model.js?v=1.46.0';
import { SHELL_PANEL, createSessionShell, detectInputPlatform } from './app-lifecycle.js?v=1.46.0';
import { GAMEPAD_BUTTON, createGamepadInput } from './gamepad-input.js?v=1.46.0';
import { createControllerRecording } from './controller-recording.js?v=1.46.0';
import { createPointerSessions } from './pointer-sessions.js?v=1.46.0';
import { createSafeStorage } from './browser-storage.js?v=1.46.0';
import { createUiFocusScope } from './ui-focus.js?v=1.46.0';
import { createUiGestures } from './ui-gestures.js?v=1.46.0';
...
nameInput.value=preferences.getItem('breachName')||`Player${Math.floor(Math.random()*90+10)}`;
let preferredTeam=preferences.getItem('breachTeam')==='red'?'red':'blue';
const storedPreferredPrimary=preferences.getItem('breachPrimary'),storedPreferredSecondary=preferences.getItem('breachSecondary');
let storedAttachmentPreferences={};try{storedAttachmentPreferences=JSON.parse(preferences.getItem('breachAttachments')||'{}')||{};}catch{}
...
let preferredTactical=normalizeTactical(preferences.getItem('breachTactical'));
let preferredLethal=normalizeLethal(preferences.getItem('breachLethal'));
let storedLoadoutClasses=[];try{storedLoadoutClasses=JSON.parse(preferences.getItem('breachLoadoutClasses')||'[]')||[];}catch{}
let preferredActiveClassId=normalizeLoadoutClassId(preferences.getItem('breachActiveClass'));
...
let storedKillstreakSelection=[];try{storedKillstreakSelection=JSON.parse(preferences.getItem('breachKillstreaks')||'[]')||[];}catch{}
...
let masterMuted=preferences.getItem('breachMuted')==='1';
...
let assaultFireMode=preferences.getItem('breachAssaultFireMode')==='semi'?'semi':'auto';
...
function loadPlayerSettings(){
  let saved={};try{saved=JSON.parse(preferences.getItem('breachPlayerSettings')||'{}')||{}}catch{}
...
const gameplayPointers=createPointerSessions(canvas,{onCancel:state=>releaseCanvasRole(state.id,false)});
...
let controllerUiFocus=null,controllerUiEditing=null,controllerUiFocusKey='',controllerFocusScope='',controllerScoreboardHeld=false;
...
function suspendGameplayInput(){
  ...
  setAim(false);
  mouseFireDown=false;
  scoreboardOpen=false;controllerScoreboardHeld=false;
}
...
const ENGINE_MODULE_URL = './vendor/three.module.min.js?v=1.46.0';
...
async function prepareGameRuntime(){
  ...
  void unlockPromise.then(unlocked=>{if(unlocked)return warmGameplayAudio(currentWeapon);}).catch(()=>{});
  return true;
}
...
function getClientId(){
  let id = preferences.getItem('breachClient');
  if (!id) {
    ...
    preferences.setItem('breachClient', id);
  }
  return id;
}
function getClientAuth(){
  let secret=String(preferences.getItem('breachClientAuth')||'').toLowerCase();
  if(!/^[a-f0-9]{64}$/.test(secret)){
    ...
    preferences.setItem('breachClientAuth',secret);
  }
  return secret;
}
...
function applyWorldSettings(value){
  ...
  movementSettings=modMovement(worldSettings);
...
function rememberTeam(team){preferredTeam=team==='red'?'red':'blue';preferences.setItem('breachTeam',preferredTeam);document.documentElement.style.setProperty('--team',TEAM_COLORS[preferredTeam]);}
...
function rememberLoadoutClasses(classes=loadoutClasses,classId=activeClassId){preferredLoadoutClasses=normalizeLoadoutClasses(classes,selectedLoadout());preferredActiveClassId=normalizeLoadoutClassId(classId);preferences.setItem('breachLoadoutClasses',JSON.stringify(preferredLoadoutClasses));preferences.setItem('breachActiveClass',preferredActiveClassId);}
function rememberKillstreakSelection(selection=killstreakSelection){const next=normalizeKillstreakSelection(selection);preferredKillstreakSelection=[...next];preferences.setItem('breachKillstreaks',JSON.stringify(next));}
...
function setMasterMuted(next){
  ...
  masterMuted=muted;preferences.setItem('breachMuted',masterMuted?'1':'0');syncMusicUI();
...
function renderLobbyRoster(modeOverride=currentGameMode()){
  ...
  const playerRow=p=>{const owner=p.id===matchOwnerId,role=owner?'HOST':p.admin?'ADMIN':'',roleChip=role?` <span class="lobby-role-chip">${role}</span>`:'';return`<div class="lobby-player ${p.self?'self':''}"><span class="lobby-player-color ${teamBased?p.team:'ffa'}"></span><div class="lobby-player-copy"><strong>${escapeHtml(p.bot?'[BOT] '+p.name:p.name)}${p.self?' · YOU':''}${roleChip}</strong><small>${teamBased?(gameModeSpec(mode).cooperative?'SURVIVOR':String(p.team||'blue').toUpperCase())+' · ':''}${escapeHtml(WEAPON_SPECS[p.primaryWeapon]?.name||'ASSAULT RIFLE')}</small></div>${hostActions(p)}</div>`;};
...
function syncPauseContext(){
  ...
  const teamBtn=$('teamSwitchBtn');if(teamBtn)teamBtn.classList.toggle('hide',!spec.teamBased||!!spec.cooperative);const teamText=$('teamSwitchText');if(teamText)teamText.textContent=pendingTeam?`SWITCHING TO ${pendingTeam.toUpperCase()} ON RESPAWN`:'SWITCH TEAM';
}
...
function savePlayerSettings(){preferences.setItem('breachPlayerSettings',JSON.stringify(playerSettings));}
...
function bindUI(){
  ...
  document.addEventListener('keydown', e => {
    ...
    if(e.isTrusted)setActiveInputMode(INPUT_MODE.KEYBOARD_MOUSE,{quiet:true});
    ...
    if(scoreboardOpen){if(e.code==='Tab')e.preventDefault();return;}
  ...
  document.addEventListener('keyup', e => {
    ...
    if(isEditableTarget(e.target)||!owned)return;
...
async function dismissChat({restorePointer=true}={}){
  ...
  const recapture=restorePointer&&activeInputMode===INPUT_MODE.KEYBOARD_MOUSE&&shell.inMatch&&!shell.paused&&!shell.panel&&!controllerInputActive();let captured=true;
...
function isEditableTarget(target){return !!target?.isContentEditable||!!target?.closest?.('input,textarea,select');}
...
function onCanvasPointerDown(e){
  ...
  if(chatOpen){
    ...
    if(pointInRect(p.x,p.y,layout.chat)){e.preventDefault();armCanvasAction(e,layout.chat,()=>void dismissChat({restorePointer:false}));return;}
    if(chatPanel?.maxScroll>0&&pointInRect(p.x,p.y,layout.chatFeed)){e.preventDefault();chatDrag={startY:p.y,startScroll:chatScroll,rowH:chatPanel.rowH};touchRoles.set(e.pointerId,'chat-scroll');}
    return;
  }
  if(scoreboardOpen){
  ...
  if(directTouch){
    ...
    // Any canvas control that changes surfaces is armed on press and
    ...
    if(liveHud&&pointInRect(p.x,p.y,layout.chat)){armCanvasAction(e,layout.chat,openChat);return;}
    if(activeRound&&hp<=0&&layout.deathLoadout&&pointInRect(p.x,p.y,layout.deathLoadout)){armCanvasAction(e,layout.deathLoadout,openMatchLoadout);return;}
    if(pointInRect(p.x,p.y,layout.team)){armCanvasAction(e,layout.team,toggleScoreboard);return;}
...
function armCanvasAction(event,rect,action){
  const state=gameplayPointers.get(event.pointerId);if(state){state.rect=rect?{...rect}:null;state.action=action;state.context=controllerInputContext();}
}
function releaseCanvasRole(id,completed){
  const role=touchRoles.get(id);touchRoles.delete(id);
...
function onCanvasPointerEnd(e){
  ...
  if(!completed||!state?.action||!shell.canPlay||state.distance>12||state.context!==controllerInputContext())return;
...
async function createMatch(){
  ...
  preferences.setItem('breachName',myName);disableMenu(true);setStatus('Creating lobby…');
  try{
...
async function joinMatch(code){
  ...
  preferences.setItem('breachName',myName);disableMenu(true);setStatus(`Joining ${code}…`);void connectMatch(code,false,attempt);
}
...
function updateEquipmentTrajectory(){
  ...
  for(let t=0;t<=maxT&&count<TRAJECTORY_MAX_POINTS;t+=TRAJECTORY_RENDER_STEP){
    const x=startX+v.vx*t,y=startY+v.vy*t-.5*modGravity(TACTICAL_GRAVITY,worldSettings)*t*t,z=startZ+v.vz*t,i=count*3;
...
function updateThrowables(dt){
  ...
  for(const g of throwables.values()){
    ...
    if(!g.stuck){const age=Math.max(0,Math.min(.34,(srv-g.snapshotAt)/1000));px+=g.snapshotVel.x*age;pz+=g.snapshotVel.z*age;if(!g.rolling)py+=g.snapshotVel.y*age-.5*modGravity(TACTICAL_GRAVITY,worldSettings)*age*age;}
...
function controllerElementVisible(el,surface){
  ...
  if(el.closest('[hidden],.hide,[inert],[aria-hidden="true"],[aria-disabled="true"]'))return false;
...
function ensureControllerUiFocus(){
  ...
  const scope=controllerUiScopeId(surface);
...
function handleControllerUiNavigation(pressed){
  ...
  if(nav){
    if(controllerUiEditing===focus&&controllerEditableField(focus)){if(nav.dx)stepControllerField(focus,nav.dx);return true;}
...
function updateGamepadInput(dt){
  ...
  const enabled=!document.hidden&&shell.snapshot().focused;
  gamepadFrame=gamepadInput.poll({context:controllerInputContext(),enabled});
  if(!enabled){clearControllerGameplayInput();return;}
...
function initHudOverlay(){
  ...
  hudMaterial=new THREE.MeshBasicMaterial({transparent:true,depthTest:false,depthWrite:false,toneMapped:false});
  const quad=new THREE.Mesh(new THREE.PlaneGeometry(2,2),hudMaterial);quad.position.z=-1;hudScene.add(quad);
  resizeHudOverlay();
}
...
function resizeHudOverlay(){
  ...
  const width=Math.max(2,Math.round(viewW*hudScale)),height=Math.max(2,Math.round(viewH*hudScale));
  // Uploaded texture dimensions are immutable. Replace the GPU resource when
  // viewport/DPR/quality changes, then bind it to the existing overlay material.
  if(!hudTexture||hudCanvas.width!==width||hudCanvas.height!==height){
    hudTexture?.dispose();hudCanvas.width=width;hudCanvas.height=height;
...
function computeHudLayout(){
  ...
  const menuW=isTouch?44:40,menuH=isTouch?44:34,chatW=menuW;
  ...
  const bottom=safe.bottom+margin,ammoW=64,ammoH=16,baseKillW=compact?220:(isTouch?252:300),killH=compact?58:(isTouch?88:112);
  ...
  const modeX=contentLeft+modeR,modeY=leftFireY-leftFireR-modeR-8,steadyX=leftFireX+leftFireR+modeR+(compact?8:10),steadyY=leftFireY;
  const touchControls=touchGameplayControlsVisible(),ammoX=touchControls?fireX-fireR-8-ammoW:contentRight-ammoW,ammoY=viewH-bottom-ammoH;
  // Reserve full-size touch slots while drawing only a small label at the bottom.
  // On narrow screens the row stays inside the corridor between combat controls.
  const streakW=KILLSTREAK_SELECTION_COUNT*44,streakH=44;
  const streakLeft=touchControls?stickyX+equipR+8:contentLeft,streakRight=touchControls?crouchX-crouchR-8:contentRight;
  const streakX=Math.max(streakLeft,Math.min((contentLeft+contentRight-streakW)/2,streakRight-streakW)),streakY=viewH-bottom-streakH;
  const streakSlots=Array.from({length:KILLSTREAK_SELECTION_COUNT},(_,index)=>({x:streakX+index*44,y:streakY,w:44,h:streakH}));
  ...
  const chatOverlapsAmmo=chatFeedRight>ammoX&&chatFeedX<ammoX+ammoW,chatClosedBottom=mapY+mapSize,chatOpenBottom=Math.min(viewH-bottom-8,composerTop-6,chatOverlapsAmmo?ammoY-8:Infinity),chatFeedBottom=chatOpen?chatOpenBottom:chatClosedBottom,chatFeedH=Math.max(0,chatFeedBottom-chatFeedY);
  return{compact,safe,moveBoundary,
    ...
    chat:{x:chatX,y:controlsY,w:chatW,h:menuH},menu:{x:menuX,y:controlsY,w:menuW,h:menuH},streak:{x:streakX,y:streakY,w:streakW,h:streakH,slots:streakSlots},
    ammo:{x:ammoX,y:ammoY,w:ammoW,h:ammoH},joy:{x:defaultJoyX,y:defaultJoyY,r:joyR},
...
function drawKillstreakPanel(c,r){
  ...
  c.save();c.textAlign='center';c.textBaseline='middle';c.font='800 11px system-ui';c.shadowColor='rgba(0,0,0,.85)';c.shadowBlur=3;
  equipped.forEach((kind,index)=>{
    const slot=r.slots[index],ready=killstreakStatus.available.includes(kind),selected=ready&&killstreakSelected===kind;
    const label=kind==='lightning'?'LGT':kind==='asteroids'?'AST':kind==='earthquake'?'EQ':kind==='solarnuke'?'NUKE':KILLSTREAK_SPECS[kind].short;
    const x=slot.x+slot.w/2,bottom=slot.y+slot.h;
    c.fillStyle=ready?HUD_ACCENT:'#8c99a3';c.fillText(label,x,bottom-10);
    if(selected){c.fillStyle=HUD_ACCENT;c.fillRect(x-10,bottom-3,20,2);}
  });
  c.restore();
}
...
function drawMatchStatus(c,w,h){
  ...
  if(matchState.status==='ended'){
    ...
    c.fillStyle='#fff';c.font='900 14px system-ui';if(spec.cooperative){c.fillText(`WAVE ${matchState.wave} · ${matchState.blueScore} ZOMBIES ELIMINATED`,w/2,y+88);}else if(teamMode){const t=teamScores();c.fillText(`${t.blue}  BLUE   ·   RED  ${t.red}`,w/2,y+88);}else c.fillText(matchState.winnerName?`${String(matchState.winnerName).toUpperCase()} WINS`:'MATCH COMPLETE',w/2,y+88);
...
function drawTeamBar(c,r){
  ...
  if(spec.cooperative){
    ...
    c.textAlign='right';c.fillStyle='#ff9ca4';c.fillText(`${matchState.waveRemaining} LEFT`,r.x+r.w-10,r.y+r.h/2);return;
  }
...
function drawTeamIndicator(c,p){if(!p||!currentModeSpec().teamBased)return;const team=myTeam==='red'?'red':'blue',color=TEAM_COLORS[team]||'#fff',label=currentModeSpec().cooperative?'SURVIVOR':team.toUpperCase();c.save();c.textAlign='center';c.textBaseline='middle';c.font='900 9px system-ui';c.shadowColor='rgba(0,0,0,.86)';c.shadowBlur=3;c.fillStyle=color;c.globalAlpha=.88;c.fillText(`◆ ${label}`,p.x,p.y);c.restore();}
...
function drawAmmo(c,r){
  ...
  c.font='900 16px system-ui';const currentW=c.measureText(current).width;
  c.font='700 11px system-ui';const capacityW=c.measureText(capacity).width;
  const x=r.x+r.w-currentW-capacityW,y=r.y+r.h/2;
  c.fillStyle=unlimited?HUD_ACCENT:count<=3?'#ff747d':'#fff';c.font='900 16px system-ui';c.fillText(current,x,y);
  c.fillStyle='#aab5bd';c.font='700 11px system-ui';c.fillText(capacity,x+currentW,y+2);
  c.restore();
}
function minimapDotColor(rmt){return modeFriendly(rmt.team)?(!rmt.bot?'#62ef86':TEAM_COLORS.blue):TEAM_COLORS.red;}
