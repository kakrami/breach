...
function renderLoadoutStats(surface,slot,weapon,attachments){
  ...
  for(const row of loadoutWeaponStats(weapon,attachments,comparison)){const delta=row.score-row.baseScore,changed=row.capacity?row.value!==row.baseValue:Math.abs(delta)>1.2,card=document.createElement('div');card.className='loadout-stat';if(row.capacity)card.classList.add('capacity');else if(delta>1.2)card.classList.add('better');else if(delta<-1.2)card.classList.add('worse');const head=document.createElement('div');head.className='loadout-stat-head';const label=document.createElement('span');label.textContent=row.label;const value=document.createElement('strong');value.textContent=row.value;head.append(label,value);const change=document.createElement('div');change.className='loadout-stat-delta';if(row.capacity)change.textContent=changed?`${row.baseValue} → ${row.value}`:'CURRENT BUILD';else change.textContent=changed?`${delta>0?'▲':'▼'} ${Math.abs(Math.round(delta))}`:'—';card.append(head,change);if(!row.capacity){const bar=document.createElement('div');bar.className='loadout-stat-bar',neutral=document.createElement('i'),changeFill=document.createElement('b'),low=Math.min(row.score,row.baseScore),diff=Math.abs(row.score-row.baseScore);neutral.className='stat-neutral';neutral.style.width=`${Math.max(3,low)}%`;changeFill.className='stat-change';changeFill.style.left=`${low}%`;changeFill.style.width=`${diff}%`;bar.append(neutral,changeFill);card.append(bar);}host.append(card);}
}
...
function buildLoadoutPreviewModel(weapon,attachments){
  ...
  const box=new THREE.Box3().setFromObject(root),size=box.getSize(new THREE.Vector3()),center=box.getCenter(new THREE.Vector3()),maxDim=Math.max(.01,size.x,size.y,size.z),scale=2.82/maxDim;root.position.sub(center);const wrapper=new THREE.Group();wrapper.add(root);wrapper.scale.setScalar(scale);if(sightPoint)wrapper.userData.adsPoint=sightPoint.sub(center).multiplyScalar(scale);wrapper.userData.calloutPoints={};for(const [slot,point] of Object.entries(calloutPoints))wrapper.userData.calloutPoints[slot]=point.clone().sub(center).multiplyScalar(scale);return wrapper;
}
