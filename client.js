...
const SOUND_CUES = {
  ...
  ksUfoInbound:{url:`audio/ks-ufo-inbound.wav?rev=${KILLSTREAK_AUDIO_REV}`,group:'Killstreaks',gain:.74,maxVoices:2},
  ksUfoBeam:{url:`audio/ks-ufo-beam.wav?rev=${KILLSTREAK_AUDIO_REV}`,group:'Killstreaks',gain:.68,maxVoices:3},
  ksLightning:{url:`audio/ks-lightning.wav?rev=${KILLSTREAK_AUDIO_REV}`,group:'Killstreaks',gain:.92,maxVoices:4},
  ksAsteroidIncoming:{url:`audio/ks-asteroid-incoming.wav?rev=${KILLSTREAK_AUDIO_REV}`,group:'Killstreaks',gain:.68,maxVoices:4},
  ksAsteroidImpact:{url:`audio/ks-asteroid-impact.wav?rev=${KILLSTREAK_AUDIO_REV}`,group:'Killstreaks',gain:.96,maxVoices:4},
  ksEarthquakeRumble:{url:`audio/ks-earthquake-rumble.wav?rev=${KILLSTREAK_AUDIO_REV}`,group:'Killstreaks',gain:.76,maxVoices:2},
  ksSolarCharge:{url:`audio/ks-solar-charge.wav?rev=${KILLSTREAK_AUDIO_REV}`,group:'Killstreaks',gain:.72,maxVoices:1},
  ksSolarBlast:{url:`audio/ks-solar-blast.wav?rev=${KILLSTREAK_AUDIO_REV}`,group:'Killstreaks',gain:1,maxVoices:1},
};
...
function syncLoadoutFocusedView(surface,draft){
  const safe=normalizeLoadoutChoice(draft);for(const slot of ['primary','secondary']){const weapon=loadoutSlotWeapon(safe,slot),attachments=loadoutSlotAttachments(safe,slot),key=`${surface}-${slot}`;renderLoadoutStats(surface,slot,weapon,attachments);loadoutPreviewDesired.set(key,{weapon,attachments:{...attachments},signature:`${weapon}:${JSON.stringify(attachments||{})}`});}
...
const samePlayerId=(a,b)=>playerKey(a)===playerKey(b);
const normalizeClientTeam=value=>String(value||'').toLowerCase()==='red'?'red':'blue';
const remoteById=id=>remotes.get(playerKey(id));
...
let storedKillstreakSelection=[];try{storedKillstreakSelection=JSON.parse(localStorage.getItem('breachKillstreaks')||'[]')||[];}catch{}
let preferredKillstreakSelection=normalizeKillstreakSelection(storedKillstreakSelection.length?storedKillstreakSelection:DEFAULT_KILLSTREAK_SELECTION);
...
let adsWanted=false,adsBlend=0,baseFov=70,sniperBreathReserve=1,sniperBreathSteadyBlend=0,sniperSwayScopeBlend=0,sniperBreathExhausted=false,sniperBreathActive=false,sniperSwayYaw=0,sniperSwayPitch=0,lastWastedBy='',lastWastedWeapon='',lastWastedHeadshot=false,lastWastedDistance=0,deathViewStartYaw=0,deathViewTargetYaw=NaN,deathViewStartPitch=0;
...
const DEFAULT_PLAYER_SETTINGS=Object.freeze({lookSensitivity:1,adsSensitivity:1,touchSensitivity:1,controllerVerticalSensitivity:1,controllerResponseCurve:'dynamic',controllerAimAssist:'on',controllerMoveDeadzone:.10,controllerLookDeadzone:.07,masterVolume:.85,sfxVolume:.9,musicVolume:.55,graphics:isTouch?'medium':'high',diagnostics:'off'});
...
function requestClientUpdate(meta,{immediate=false}={}){
  ...
  if(activeMatchForUpdate()&&!immediate){const label=updateTargetKey(meta);if(pendingClientUpdateNotice!==label){pendingClientUpdateNotice=label;showToast('UPDATE READY · APPLYING AFTER MATCH',{priority:2,key:'client-update-ready'});}return false;}
  return forceClientUpdateReload(pendingClientUpdate);
}
function maybeApplyPendingClientUpdate(state=null){if(!pendingClientUpdate)return false;const snapshot=state||shell.snapshot();if(pendingClientUpdate.immediate||!snapshot.inMatch||matchState?.status===MATCH_STATUS.ENDED)return forceClientUpdateReload(pendingClientUpdate);return false;}
async function fetchVersionMeta(url,source){const target=new URL(url,location.href);target.searchParams.set('__check',String(Date.now()));const response=await fetch(target,{cache:'no-store'});if(!response.ok)throw new Error(`${source} version check failed`);return normalizeVersionMeta(await response.json(),source);}
async function checkForClientUpdate(reason='timer'){
  if(versionCheckPromise)return versionCheckPromise;
  versionCheckPromise=(async()=>{
    const results=await Promise.allSettled([fetchVersionMeta('./version.json','client'),fetchVersionMeta(`${ONLINE_API}/health`,'server')]);
    const clientMeta=results[0].status==='fulfilled'?results[0].value:null,serverMeta=results[1].status==='fulfilled'?results[1].value:null;
    const clientProtocolChanged=!!clientMeta?.protocol&&clientMeta.protocol!==PROTOCOL_VERSION;
    const serverProtocolChanged=!!serverMeta?.protocol&&serverMeta.protocol!==PROTOCOL_VERSION;
    if(clientProtocolChanged||serverProtocolChanged){
      // Two repos deploy independently. A protocol-breaking client reload waits
      // until version.json and the live server report the same new protocol.
      if(clientMeta?.protocol&&clientMeta.protocol===serverMeta?.protocol&&versionMetaDiffers(clientMeta))requestClientUpdate(clientMeta,{immediate:true});
      else if(pendingClientUpdateNotice!=='deploying'){pendingClientUpdateNotice='deploying';showToast('UPDATE DEPLOYING · RETRYING',{priority:2,key:'client-update-deploying'});}
      return;
    }
    if(clientMeta&&versionMetaDiffers(clientMeta)){requestClientUpdate(clientMeta);return;}
    if(clientMeta){clearSuccessfulUpdateGuard();pendingClientUpdateNotice='';}
  })().catch(error=>console.debug('Breach version check skipped',reason,error)).finally(()=>{versionCheckPromise=null;});
  return versionCheckPromise;
}
function startVersionWatcher(){
  void checkForClientUpdate('startup');
  if(!versionIntervalId)versionIntervalId=setInterval(()=>void checkForClientUpdate('interval'),VERSION_CHECK_INTERVAL_MS);
  addEventListener('focus',()=>void checkForClientUpdate('focus'),{passive:true});
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)void checkForClientUpdate('visible');},{passive:true});
}
const loadoutEditClass={lobby:activeClassId,match:activeClassId};
const bullets = new Map();
const throwables = new Map();
const tacticalFx = [];
const killstreakFx = [];
const solarBurnFx = [];
let killstreakStatus={streak:0,available:[],earned:[]},killstreakSelected='',killstreakTargeting=null,killstreakTargetRect=null,killstreakTargetConfirmRect=null,killstreakTargetCancelRect=null,killstreakControlUntil=0,earthquakeControlUntil=0,earthquakeControlSeed=0,earthquakeControlStrength=0,ufoLocalLift=0;
let solarEnvironmentBaseline=null;
...
function selectReadyKillstreak(preferred=''){
  const ready=readyKillstreakKinds(),wanted=normalizeKillstreak(preferred);if(wanted&&ready.includes(wanted))killstreakSelected=wanted;else if(!ready.includes(killstreakSelected))killstreakSelected=ready[0]||'';hudLastDraw=0;return killstreakSelected;
}
function killstreakKeyboardKey(kind){const i=killstreakSelection.indexOf(normalizeKillstreak(kind));return i>=0&&i<KILLSTREAK_SELECTION_COUNT?String(i+3):'';}
function applyKillstreakState(value={}){
  if(Array.isArray(value.selection)){killstreakSelection=normalizeKillstreakSelection(value.selection);rememberKillstreakSelection(killstreakSelection);lobbyKillstreakSyncPending=false;if(!lobbyKillstreakDraft||lobbyKillstreakDraft.length===KILLSTREAK_SELECTION_COUNT)lobbyKillstreakDraft=[...killstreakSelection];}
  const selected=new Set(killstreakSelection);killstreakStatus={streak:Math.max(0,Math.floor(Number(value.streak)||0)),available:normalizedKillstreakList(value.available).filter(kind=>selected.has(kind)),earned:normalizedKillstreakList(value.earned,{unique:true}).filter(kind=>selected.has(kind))};selectReadyKillstreak(killstreakSelected);
  for(const kind of normalizedKillstreakList(value.justEarned,{unique:true}).filter(id=>selected.has(id))){const spec=KILLSTREAK_SPECS[kind],key=killstreakKeyboardKey(kind);if(spec)queueAnnouncer(`${spec.name} READY`,controllerInputActive()?'D-PAD ↑ SELECT · → USE':isTouch?'TAP STREAK TO USE':`${spec.kills} KILL STREAK · ${key}`,1900,7);}
...
function closeKillstreakTargeting({recapture=false}={}){if(!killstreakTargeting)return;killstreakTargeting=null;killstreakTargetRect=null;killstreakTargetConfirmRect=null;killstreakTargetCancelRect=null;hudLastDraw=0;if(recapture&&!isTouch&&!controllerInputActive())void shell.capturePointerFromGesture();}
function openKillstreakTargeting(kind){const id=normalizeKillstreak(kind),spec=KILLSTREAK_SPECS[id];if(!spec?.targeted||!killstreakCanUse(id))return false;clearFireInput();cancelEquipmentAction();cancelSprint();scoreboardOpen=false;killstreakSelected=id;killstreakTargeting={kind:id,x:position?.x||0,z:position?.z||0,chosen:false};killstreakTargetRect=null;killstreakTargetConfirmRect=null;killstreakTargetCancelRect=null;if(!isTouch&&document.pointerLockElement===canvas)document.exitPointerLock?.();hudLastDraw=0;return true;}
function confirmKillstreakTarget({recapture=false}={}){if(!killstreakTargeting)return false;if(!killstreakTargeting.chosen){showToast('SELECT A TARGET FIRST',{duration:900,key:'streak-target'});return false;}const target={...killstreakTargeting};send({t:'killstreak',kind:target.kind,x:target.x,z:target.z});closeKillstreakTargeting({recapture});return true;}
...
function setActiveInputMode(mode,{quiet=false}={}){
  ...
  for(const surface of ['lobby','match'])flashLoadoutInputHint(surface,1600);
  return true;
}
...
async function ensureThreeEngine(){
  ...
  engineLoadPromise=(async()=>{
    try{
      ...
      if(!shell.inMatch)setStatus(webglError?'WEBGL2 UNAVAILABLE':'3D ENGINE UNAVAILABLE','error');
      return false;
    }
...
function controllerAdsSensitivityScale(){
  ...
  const targetFov=effectiveWeaponSpec(currentWeapon)?.adsFov||baseFov;
...
function controllerAssistRadiusDeg(){
  const fov=magnifiedScopeFov();if(fov)return fov<=10.5?1.2:fov<=13.5?1.45:1.8;
  return THREE.MathUtils.lerp(4.2,2.8,smoothstep01(adsBlend));
}
function controllerAssistMaxDistance(){return magnifiedScopeFov()?180:THREE.MathUtils.lerp(72,115,smoothstep01(adsBlend));}
...
function applyControllerAim(dt){
  ...
  const scopeFov=magnifiedScopeFov(),minSlow=scopeFov?(scopeFov<=10.5?.49:scopeFov<=13.5?.51:.53):THREE.MathUtils.lerp(.70,.60,ads),slowdown=assist?THREE.MathUtils.lerp(1,minSlow,assistStrength):1;
...
const MAGNIFIED_SCOPE_SCREEN_RADIUS=.435;
const SNIPER_SCOPE_AXIS_Y=.14;
const SNIPER_BREATH_READY_BLEND=.965;
const SNIPER_BREATH_HOLD_MS=4200;
const SNIPER_BREATH_RECOVER_MS=3200;
const SNIPER_BREATH_RESTART_RESERVE=.28;
const thermalOpticMaterialStates=new Map();
const thermalOpticLightStates=new Map();
let thermalOpticBackgroundState=null,thermalOpticFogState=null,thermalOpticWorldBlend=0,thermalOpticLastCollectAt=0;
function weaponOpticSpec(weapon=currentWeapon){const id=String(attachmentsForWeapon(weapon)?.optic||'');return id?ATTACHMENTS[id]||null:null;}
function scopeZoomLabelForWeapon(weapon=currentWeapon){return String(weaponOpticSpec(weapon)?.zoomLabel||(weapon==='sniper'?'4X':''));}
function activeScopeZoomLabel(){return scopeZoomLabelForWeapon(currentWeapon)||'SCOPE';}
function sniperZoomLabel(){return scopeZoomLabelForWeapon('sniper');}
function sniperTargetFov(){return Number(effectiveWeaponSpec('sniper')?.adsFov)||18;}
function magnifiedScopeFov(){if(adsBlend<=.2)return 0;if(currentWeapon==='sniper'||thermalOpticMode())return Number(effectiveWeaponSpec(currentWeapon)?.adsFov)||0;return 0;}
function magnifiedScopeSensitivityBase(){const fov=Number(effectiveWeaponSpec(currentWeapon)?.adsFov)||baseFov;return fov<=10.5?.15:fov<=13.5?.20:.28;}
function thermalOpticMode(){return String(weaponOpticSpec(currentWeapon)?.thermalMode||'');}
function thermalOpticScopeAmount(){if(!thermalOpticMode())return 0;if(currentWeapon==='sniper')return sniperMaskAmount();const t=smoothstep01((THREE.MathUtils.clamp(adsBlend,0,1)-.52)/.43);return Math.pow(t,1.45);}
function activeScopeMaskAmount(){return currentWeapon==='sniper'?sniperMaskAmount():thermalOpticScopeAmount();}
function activeScopeReticleAmount(){return currentWeapon==='sniper'?sniperReticleAmount():smoothstep01((thermalOpticScopeAmount()-.48)/.42);}
function thermalOpticAmount(){if(!thermalOpticMode())return 0;if(currentWeapon==='sniper')return smoothstep01((sniperAdsMotion()-.66)/.28);return smoothstep01((thermalOpticScopeAmount()-.42)/.48);}
function thermalOpticWorldAmount(){if(!thermalOpticMode())return 0;if(currentWeapon==='sniper')return smoothstep01((sniperMaskAmount()-.88)/.10);return smoothstep01((thermalOpticScopeAmount()-.82)/.15);}
let thermalOpticColorScratch=null;
function thermalGrayTarget(color,mode,out){out=out||(thermalOpticColorScratch||(thermalOpticColorScratch=new THREE.Color()));const lum=color.r*.2126+color.g*.7152+color.b*.0722,gray=THREE.MathUtils.clamp(lum*(mode==='white'?.38:.44)+(mode==='white'?.022:.028),.018,mode==='white'?.44:.50);return out.setRGB(gray,gray,gray);}
function captureThermalOpticMaterial(mat){if(!mat||mat.userData?.thermalOpticSignature||thermalOpticMaterialStates.has(mat)||!mat.color?.isColor)return;const map=mat.map||null,thermalMap=map?.userData?.worldTextureName?getWorldThermalTexture(map.userData.worldTextureName):(map?.userData?.thermalTexture||null);thermalOpticMaterialStates.set(mat,{color:mat.color.clone(),emissive:mat.emissive?.isColor?mat.emissive.clone():null,map,thermalMap});}
function collectThermalOpticSceneState(){if(!scene)return;scene.traverse(obj=>{if(obj.isLight){if(!thermalOpticLightStates.has(obj))thermalOpticLightStates.set(obj,{color:obj.color?.clone?.()||null,groundColor:obj.groundColor?.clone?.()||null});return;}const mats=Array.isArray(obj.material)?obj.material:[obj.material];for(const mat of mats)captureThermalOpticMaterial(mat);});if(!thermalOpticBackgroundState&&scene.background?.isColor)thermalOpticBackgroundState=scene.background.clone();if(!thermalOpticFogState&&scene.fog?.color?.isColor)thermalOpticFogState=scene.fog.color.clone();}
function updateThermalOpticWorld(){
  const mode=thermalOpticMode(),amount=thermalOpticWorldAmount(),now=performance.now();
  if(amount>.001&&(thermalOpticWorldBlend<=.001||now-thermalOpticLastCollectAt>500)){collectThermalOpticSceneState();thermalOpticLastCollectAt=now;}
  if(!thermalOpticMaterialStates.size&&!thermalOpticLightStates.size&&!thermalOpticBackgroundState&&!thermalOpticFogState){thermalOpticWorldBlend=amount;return;}
  for(const [mat,state] of thermalOpticMaterialStates){if(!mat?.color?.isColor)continue;thermalGrayTarget(state.color,mode||'white',thermalOpticColorScratch);mat.color.copy(state.color).lerp(thermalOpticColorScratch,amount);if(state.emissive&&mat.emissive?.isColor){thermalGrayTarget(state.emissive,mode||'white',thermalOpticColorScratch).multiplyScalar(.28);mat.emissive.copy(state.emissive).lerp(thermalOpticColorScratch,amount);}const nextMap=amount>.32&&state.thermalMap?state.thermalMap:state.map;if(mat.map!==nextMap)mat.map=nextMap;}
  for(const [light,state] of thermalOpticLightStates){if(state.color&&light.color?.isColor){thermalGrayTarget(state.color,mode||'white',thermalOpticColorScratch);light.color.copy(state.color).lerp(thermalOpticColorScratch,amount);}if(state.groundColor&&light.groundColor?.isColor){thermalGrayTarget(state.groundColor,mode||'white',thermalOpticColorScratch);light.groundColor.copy(state.groundColor).lerp(thermalOpticColorScratch,amount);}}
  if(thermalOpticBackgroundState&&scene?.background?.isColor){thermalGrayTarget(thermalOpticBackgroundState,mode||'white',thermalOpticColorScratch);scene.background.copy(thermalOpticBackgroundState).lerp(thermalOpticColorScratch,amount);}
  if(thermalOpticFogState&&scene?.fog?.color?.isColor){thermalGrayTarget(thermalOpticFogState,mode||'white',thermalOpticColorScratch);scene.fog.color.copy(thermalOpticFogState).lerp(thermalOpticColorScratch,amount);}
  thermalOpticWorldBlend=amount;
  if(amount<=.001){thermalOpticMaterialStates.clear();thermalOpticLightStates.clear();thermalOpticBackgroundState=null;thermalOpticFogState=null;thermalOpticLastCollectAt=0;}
}
...
function sniperBreathInputHeld(){
  ...
  if(controllerInputActive()&&!!gamepadFrame?.held?.[GAMEPAD_BUTTON.LS])return true;
  return touchRoleActive('steady');
}
function resetSniperBreath({full=true}={}){sniperBreathSteadyBlend=0;sniperSwayScopeBlend=0;sniperBreathExhausted=false;sniperBreathActive=false;sniperSwayYaw=0;sniperSwayPitch=0;if(full)sniperBreathReserve=1;}
function updateSniperBreathing(dt,now=performance.now()){
  const frameDt=Math.min(.05,Math.max(0,Number(dt)||0)),inputHeld=sniperBreathInputHeld();
  if(inputHeld&&!sniperBreathExhausted&&sniperBreathReserve>.0001){
    sniperBreathActive=true;sniperBreathReserve=Math.max(0,sniperBreathReserve-frameDt/(SNIPER_BREATH_HOLD_MS/1000));
    if(sniperBreathReserve<=.0001){sniperBreathReserve=0;sniperBreathExhausted=true;sniperBreathActive=false;}
  }else{
    sniperBreathActive=false;sniperBreathReserve=Math.min(1,sniperBreathReserve+frameDt/(SNIPER_BREATH_RECOVER_MS/1000));
    if(!inputHeld&&sniperBreathReserve>=SNIPER_BREATH_RESTART_RESERVE)sniperBreathExhausted=false;
  }
  const target=sniperBreathActive?1:0,follow=1-Math.exp(-(target?18:7)*frameDt);sniperBreathSteadyBlend+=(target-sniperBreathSteadyBlend)*follow;
  const scopeTarget=sniperBreathEligible()?1:0,scopeFollow=1-Math.exp(-(scopeTarget?6:28)*frameDt);sniperSwayScopeBlend+=(scopeTarget-sniperSwayScopeBlend)*scopeFollow;
  if(sniperSwayScopeBlend<=.0001){sniperSwayYaw=0;sniperSwayPitch=0;return;}
  const seconds=now*.001,steadyScale=1-.965*sniperBreathSteadyBlend,moveScale=1+THREE.MathUtils.clamp(localMoveAmount,0,1)*.32;
  // Two slow, offset harmonics avoid a mechanical circle while keeping the
  // motion readable as breathing. The reticle stays fixed; this moves the real
  // camera/shot ray beneath it, so what the player sees is what the rifle fires.
  const yawWave=Math.sin(seconds*1.18+.35)*.00165+Math.sin(seconds*.53+1.90)*.00055;
  const pitchWave=Math.sin(seconds*1.53+.82)*.00220+Math.sin(seconds*.69+2.25)*.00065;
  sniperSwayYaw=yawWave*sniperSwayScopeBlend*steadyScale*moveScale;sniperSwayPitch=pitchWave*sniperSwayScopeBlend*steadyScale*moveScale;
}
...
function rememberKillstreakSelection(selection=killstreakSelection){const next=normalizeKillstreakSelection(selection);preferredKillstreakSelection=[...next];localStorage.setItem('breachKillstreaks',JSON.stringify(next));}
...
function modeFriendly(team){return currentModeSpec().teamBased&&normalizeClientTeam(team)===normalizeClientTeam(myTeam);}
function remoteDisplayColor(team){const normalized=normalizeClientTeam(team);return currentModeSpec().teamBased?TEAM_COLORS[normalized]:TEAM_COLORS.red;}
...
function replaceLobbyParticipants(players=[],bots=[]){lobbyParticipants.clear();for(const player of [...players,...bots]){const row=normalizeLobbyParticipant(player);if(row&&!samePlayerId(row.id,clientId))lobbyParticipants.set(row.id,row);}}
function upsertLobbyParticipant(player){const row=normalizeLobbyParticipant(player);if(row&&!samePlayerId(row.id,clientId))lobbyParticipants.set(row.id,row);}
...
function renderLobbyRoster(modeOverride=currentGameMode()){
  ...
  const playerRow=p=>{const owner=p.id===matchOwnerId,role=owner?'HOST':p.admin?'ADMIN':'',roleChip=role?` <span class="lobby-role-chip">${role}</span>`:'';return`<div class="lobby-player ${p.self?'self':''}"><span class="lobby-player-color ${teamBased?p.team:'ffa'}"></span><div class="lobby-player-copy"><strong>${escapeHtml(p.bot?'[BOT] '+p.name:p.name)}${p.self?' · YOU':''}${roleChip}</strong><small>${teamBased?String(p.team||'blue').toUpperCase()+' · ':''}${escapeHtml(WEAPON_SPECS[p.primaryWeapon]?.name||'ASSAULT RIFLE')}</small></div>${hostActions(p)}</div>`;};
...
function applyKillstreakLoadoutAck(message={}){
...
function setLobbyActionState(){
  if(!shell.inLobby)return;const status=$('lobbyStatus');if(status)status.textContent=(lobbyLoadoutDirty||lobbyKillstreakSyncPending)?'SYNCING':'';
}
...
function renderLobbySetupControls(){
  if(!shell.inLobby)return;refreshLobbyDraftOwnership();const host=!!isMatchAdmin,draft=lobbyMatchDraft||committedLobbyMatchDraft(),draftSpec=gameModeSpec(draft.mode),draftLoadout=lobbyLoadoutDraft||normalizeLoadoutChoice(selectedLoadout());renderLoadoutClassStrip('lobby');renderAttachmentEditors('lobby',draftLoadout);renderLobbyKillstreakSetup();
...
function renderLobbyShell(){
  ...
  $('lobbyTeamGroup').classList.toggle('hide',!displaySpec.teamBased);$('lobbyHostSetup').classList.toggle('hide',!host);$('lobbyGuestSetup').classList.toggle('hide',host);$('lobbyCheatsTab')?.classList.toggle('hide',!host);
...
function syncPauseContext(){
  ...
  if($('pauseRoom'))$('pauseRoom').textContent=`${mapSpec(currentMapId).short} · ${spec.short}`;
  ...
  const teamBtn=$('teamSwitchBtn');if(teamBtn)teamBtn.classList.toggle('hide',!spec.teamBased);const teamText=$('teamSwitchText');if(teamText)teamText.textContent=pendingTeam?`SWITCHING TO ${pendingTeam.toUpperCase()} ON RESPAWN`:'SWITCH TEAM';
}
...
function makeRedDotAttachment({sightY=.15,z=-.18,mountY=.12,eyeZ=-.38}={}){
  const g=new THREE.Group(),mat=attachmentVisualMaterial(0x14191d,.34,.44),dark=attachmentVisualMaterial(0x0e1215,.28,.58),lens=new THREE.MeshStandardMaterial({color:0x9ab8c1,roughness:.06,metalness:.05,transparent:true,opacity:.13,emissive:0x142d34,emissiveIntensity:.07,depthWrite:false,side:THREE.DoubleSide}),ret=opticReticleMaterials();
  const base=new THREE.Mesh(new THREE.BoxGeometry(.060,.014,.072),mat);base.position.set(0,mountY+.007,z+.003);const riserH=Math.max(.008,sightY-mountY-.028),riserL=new THREE.Mesh(new THREE.BoxGeometry(.009,riserH,.024),mat),riserR=riserL.clone();riserL.position.set(-.017,mountY+riserH*.5+.002,z-.002);riserR.position.set(.017,mountY+riserH*.5+.002,z-.002);
  const frameW=.006,windowW=.050,windowH=.039,left=new THREE.Mesh(new THREE.BoxGeometry(frameW,windowH+.006,.016),mat),right=left.clone(),top=new THREE.Mesh(new THREE.BoxGeometry(windowW+.006,frameW,.016),mat),chin=new THREE.Mesh(new THREE.BoxGeometry(windowW*.74,.006,.016),mat);left.position.set(-windowW*.5,sightY,z-.014);right.position.set(windowW*.5,sightY,z-.014);top.position.set(0,sightY+windowH*.5,z-.014);chin.position.set(0,sightY-windowH*.49,z-.014);
  ...
  const dial=new THREE.Mesh(new THREE.CylinderGeometry(.008,.008,.010,10),dark);dial.rotation.z=Math.PI/2;dial.position.set(.034,sightY+.006,z-.006);const frontLip=new THREE.Mesh(new THREE.BoxGeometry(.057,.007,.008),dark);frontLip.position.set(0,mountY+.017,z-.034);
  g.add(base,riserL,riserR,left,right,top,chin,glass,dial,frontLip,dotGlow,dotCore);g.userData.aimPoint=dotCore;g.userData.adsPose={x:0,y:-sightY,z:Number(eyeZ)-Number(z-.015),rx:0,ry:0,rz:0};return g;
}
function makeHoloAttachment({sightY=.15,z=-.17,mountY=.12,eyeZ=-.38}={}){
  const g=new THREE.Group(),mat=attachmentVisualMaterial(0x171c20,.30,.47),dark=attachmentVisualMaterial(0x0d1114,.24,.62),lens=new THREE.MeshStandardMaterial({color:0xa5bec4,roughness:.06,metalness:.05,transparent:true,opacity:.10,emissive:0x13292e,emissiveIntensity:.05,depthWrite:false,side:THREE.DoubleSide}),ret=opticReticleMaterials();
  const base=new THREE.Mesh(new THREE.BoxGeometry(.098,.020,.102),mat);base.position.set(0,mountY+.010,z+.005);const frameW=.008,windowW=.086,windowH=.066,left=new THREE.Mesh(new THREE.BoxGeometry(frameW,windowH+.010,.020),mat),right=left.clone(),top=new THREE.Mesh(new THREE.BoxGeometry(windowW+.010,frameW,.020),mat),chin=new THREE.Mesh(new THREE.BoxGeometry(windowW*.80,.008,.020),mat);left.position.set(-windowW*.5,sightY,z-.020);right.position.set(windowW*.5,sightY,z-.020);top.position.set(0,sightY+windowH*.5,z-.020);chin.position.set(0,sightY-windowH*.48,z-.020);
  const guardL=new THREE.Mesh(new THREE.BoxGeometry(.010,.044,.074),mat),guardR=guardL.clone();guardL.position.set(-.050,mountY+.038,z+.006);guardR.position.set(.050,mountY+.038,z+.006);guardL.rotation.z=-.09;guardR.rotation.z=.09;
  ...
  const buttonA=new THREE.Mesh(new THREE.CylinderGeometry(.005,.005,.005,8),dark),buttonB=buttonA.clone();buttonA.rotation.z=buttonB.rotation.z=Math.PI/2;buttonA.position.set(.054,mountY+.026,z+.024);buttonB.position.set(.054,mountY+.026,z+.008);const rearShelf=new THREE.Mesh(new THREE.BoxGeometry(.072,.010,.026),dark);rearShelf.position.set(0,mountY+.024,z+.047);
  g.add(base,guardL,guardR,left,right,top,chin,glass,buttonA,buttonB,rearShelf,ringGlow,ring,dot);g.userData.aimPoint=dot;g.userData.adsPose={x:0,y:-sightY,z:Number(eyeZ)-Number(z-.022),rx:0,ry:0,rz:0};return g;
}
function makeCombatOptic({sightY=.15,z=-.17,mountY=.12,eyeZ=-.38,length=.19,scopeRadius=.037}={}){
  const g=new THREE.Group(),mat=attachmentVisualMaterial(0x171b1e,.30,.48),dark=attachmentVisualMaterial(0x0c1012,.32,.50),innerMat=new THREE.MeshStandardMaterial({color:0x07090b,roughness:.98,metalness:.01,side:THREE.BackSide}),lensMat=new THREE.MeshStandardMaterial({color:0xb9cdd2,roughness:.05,metalness:.04,transparent:true,opacity:.09,emissive:0x102126,emissiveIntensity:.035,depthWrite:false,side:THREE.DoubleSide}),ret=opticReticleMaterials(),radius=Math.max(.026,Number(scopeRadius)||.037);
  ...
  const body=new THREE.Mesh(new THREE.CylinderGeometry(radius*.92,radius,length,20,1,true),mat);body.rotation.x=Math.PI/2;body.position.set(0,sightY,z);const inner=new THREE.Mesh(new THREE.CylinderGeometry(radius*.72,radius*.76,length*.91,20,1,true),innerMat);inner.rotation.x=Math.PI/2;inner.position.copy(body.position);
  const objectiveRing=new THREE.Mesh(new THREE.TorusGeometry(radius*.94,.0048,8,24),mat),eyeRing=new THREE.Mesh(new THREE.TorusGeometry(radius*.82,.0045,8,24),mat),focusRing=new THREE.Mesh(new THREE.TorusGeometry(radius*.93,.0036,7,22),dark);objectiveRing.position.set(0,sightY,z-length*.50);eyeRing.position.set(0,sightY,z+length*.50);focusRing.position.set(0,sightY,z+length*.26);const front=new THREE.Mesh(new THREE.CircleGeometry(radius*.78,24),lensMat),rear=new THREE.Mesh(new THREE.CircleGeometry(radius*.68,24),lensMat);front.position.set(0,sightY,z-length*.485);rear.position.set(0,sightY,z+length*.485);
  const topRail=new THREE.Mesh(new THREE.BoxGeometry(.030,.010,length*.33),mat);topRail.position.set(0,sightY+radius*.92,z-.018);addBakedRailRibs(topRail,{count:4});const turret=new THREE.Mesh(new THREE.CylinderGeometry(.010,.010,.016,10),dark);turret.position.set(0,sightY+radius+.010,z);const sideDial=new THREE.Mesh(new THREE.CylinderGeometry(.009,.009,.013,10),dark);sideDial.rotation.z=Math.PI/2;sideDial.position.set(radius+.007,sightY,z+.008);
  const reticle=new THREE.Group(),chevron=makeChevronReticle(ret.core,{size:.0080}),centerDot=new THREE.Mesh(new THREE.CircleGeometry(.00135,14),ret.core);reticle.add(chevron,centerDot);for(let i=1;i<=4;i++){const tick=new THREE.Mesh(new THREE.BoxGeometry(.0060-i*.0007,.0008,.0001),ret.core);tick.position.set(0,-.008-i*.0054,0);reticle.add(tick);}reticle.position.set(0,sightY,z+length*.492);reticle.traverse(o=>{if(o.isMesh)o.renderOrder=10;});
  g.add(railBase,footA,footB,body,inner,objectiveRing,eyeRing,focusRing,front,rear,topRail,turret,sideDial,reticle);g.userData.aimPoint=centerDot;g.userData.adsPose={x:0,y:-sightY,z:Number(eyeZ)-Number(z+length*.485),rx:0,ry:0,rz:0};return g;
}
function makeThermalOpticAttachment({sightY=.17,z=-.12,mountY=.12,eyeZ=-.40,length=.20,radius=.040,long=false}={}){
  const g=makeCombatOptic({sightY,z,mountY,eyeZ,length,scopeRadius:radius}),mat=attachmentVisualMaterial(0x252d31,.18,.60),dark=attachmentVisualMaterial(0x101518,.20,.68),sensorMat=attachmentVisualMaterial(0x647d82,.08,.28);
  const module=new THREE.Mesh(new THREE.BoxGeometry(radius*.72,radius*.62,length*.30),mat),battery=new THREE.Mesh(new THREE.BoxGeometry(radius*.36,radius*.34,length*.22),dark),sensor=new THREE.Mesh(new THREE.CircleGeometry(radius*.15,14),sensorMat),wire=new THREE.Mesh(new THREE.CylinderGeometry(radius*.035,radius*.035,length*.22,8),dark);
  module.position.set(-radius*1.22,sightY-radius*.08,z+length*.05);battery.position.set(-radius*1.22,sightY-radius*.55,z+length*.06);sensor.rotation.y=Math.PI/2;sensor.position.set(-radius*1.59,sightY-radius*.06,z+length*.01);wire.rotation.x=Math.PI/2;wire.position.set(-radius*.92,sightY-radius*.40,z+length*.10);g.add(module,battery,sensor,wire);
...
function init3D(){
  ...
  worldHemiLight = new THREE.HemisphereLight(0xdaf4ff, 0x52604c, 2.2); scene.add(worldHemiLight);
  worldSunLight = new THREE.DirectionalLight(0xffffff, 2.15); worldSunLight.position.set(26,38,18);worldSunLight.castShadow=!isTouch;if(!isTouch){worldSunLight.shadow.mapSize.set(1536,1536);worldSunLight.shadow.camera.left=-125;worldSunLight.shadow.camera.right=125;worldSunLight.shadow.camera.top=125;worldSunLight.shadow.camera.bottom=-125;worldSunLight.shadow.camera.near=5;worldSunLight.shadow.camera.far=110;worldSunLight.shadow.bias=-.00035;} scene.add(worldSunLight);
...
const worldTextureCache=new Map(),worldThermalTextureCache=new Map();
let worldTextureLoader=null;
function configureWorldTexture(tex,def){tex.colorSpace=THREE.SRGBColorSpace;tex.wrapS=tex.wrapT=THREE.RepeatWrapping;tex.repeat.set(def.repeat[0],def.repeat[1]);tex.minFilter=THREE.LinearMipmapLinearFilter;tex.magFilter=THREE.LinearFilter;tex.anisotropy=Math.min(isTouch?1:8,renderer?.capabilities?.getMaxAnisotropy?.()||1);tex.userData.preserveWorldTexture=true;return tex;}
...
function getWorldThermalTexture(name){
  if(!THREE||!name)return null;if(worldThermalTextureCache.has(name))return worldThermalTextureCache.get(name);const def=WORLD_TEXTURE_DEFS[name];if(!def)return null;worldTextureLoader ||= new THREE.TextureLoader();const tex=configureWorldTexture(worldTextureLoader.load(`textures/thermal/${def.file}?rev=${WORLD_TEXTURE_ASSET_REV}`),def);tex.userData.worldThermalTexture=true;worldThermalTextureCache.set(name,tex);return tex;
}
...
function makeTerrainTexture(){
  ...
  const tex=new THREE.CanvasTexture(c);tex.colorSpace=THREE.SRGBColorSpace;tex.wrapS=tex.wrapT=THREE.RepeatWrapping;tex.repeat.set(yard?34:(rig?30:28),yard?34:(rig?30:28));tex.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy?.()||1);tex.userData.preserveWorldTexture=true;
  const tc=document.createElement('canvas');tc.width=tc.height=size;const tx=tc.getContext('2d');tx.drawImage(c,0,0);const td=tx.getImageData(0,0,size,size);for(let i=0;i<td.data.length;i+=4){const g=Math.round(td.data[i]*.2126+td.data[i+1]*.7152+td.data[i+2]*.0722);td.data[i]=td.data[i+1]=td.data[i+2]=g;}tx.putImageData(td,0,0);const thermalTex=new THREE.CanvasTexture(tc);thermalTex.colorSpace=THREE.SRGBColorSpace;thermalTex.wrapS=thermalTex.wrapT=THREE.RepeatWrapping;thermalTex.repeat.copy(tex.repeat);thermalTex.anisotropy=tex.anisotropy;thermalTex.userData.preserveWorldTexture=true;thermalTex.userData.worldThermalTexture=true;tex.userData.thermalTexture=thermalTex;return tex;
}
...
function bindUI(){
  ...
  document.addEventListener('keydown', e => {
    ...
    if(killstreakTargeting){
      ...
      if(e.code==='Enter'&&!e.repeat){e.preventDefault();confirmKillstreakTarget();return;}
      const step=e.shiftKey?5:2;if(!e.repeat||['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','KeyW','KeyA','KeyS','KeyD'].includes(e.code)){const limit=minimapWorldLimit();if(e.code==='ArrowUp'||e.code==='KeyW')killstreakTargeting.z=Math.max(-limit,killstreakTargeting.z-step);else if(e.code==='ArrowDown'||e.code==='KeyS')killstreakTargeting.z=Math.min(limit,killstreakTargeting.z+step);else if(e.code==='ArrowLeft'||e.code==='KeyA')killstreakTargeting.x=Math.max(-limit,killstreakTargeting.x-step);else if(e.code==='ArrowRight'||e.code==='KeyD')killstreakTargeting.x=Math.min(limit,killstreakTargeting.x+step);else return;killstreakTargeting.chosen=true;e.preventDefault();hudLastDraw=0;return;}
    }
    ...
    if(['KeyW','KeyA','KeyS','KeyD','ShiftLeft','ShiftRight','Space','KeyC','KeyR','KeyQ','KeyB','KeyF','KeyG','Digit1','Digit2','Digit3','Digit4','Digit5','Tab'].includes(e.code)) e.preventDefault();
    keys.add(e.code);
    ...
    if(e.code==='Digit3' && !e.repeat) requestKillstreak(killstreakSelection[0]);
    if(e.code==='Digit4' && !e.repeat) requestKillstreak(killstreakSelection[1]);
    if(e.code==='Digit5' && !e.repeat) requestKillstreak(killstreakSelection[2]);
...
function openGameTextEditor(target){
  ...
  const mode=gameTextMode();gameTextEditorTitle.textContent=mode==='code'?'ROOM CODE':mode==='class'?'CLASS NAME':'CALLSIGN';
...
function onCanvasPointerDown(e){
  ...
  if(killstreakTargeting){e.preventDefault();const recapture=!isTouch&&e.pointerType==='mouse';if(pointInRect(p.x,p.y,killstreakTargetConfirmRect)){confirmKillstreakTarget({recapture});return;}if(pointInRect(p.x,p.y,killstreakTargetCancelRect)){closeKillstreakTargeting({recapture});return;}killstreakTargetFromCanvas(p.x,p.y,{choose:true});return;}
  if(chatOpen){
  ...
  if(isTouch||directTouch){
    ...
    if(!liveHud)return;
    if(layout.streak&&pointInRect(p.x,p.y,layout.streak)){const equipped=normalizeKillstreakSelection(killstreakSelection),slot=Math.max(0,Math.min(equipped.length-1,Math.floor((p.x-layout.streak.x)/(layout.streak.w/equipped.length))));requestKillstreak(equipped[slot]);touchRoles.set(e.pointerId,'killstreak');return;}
...
function onCanvasPointerMove(e){
  if(!shell.canPlay)return;
  if(killstreakTargeting&&e.pointerType==='mouse'){const p=canvasPoint(e);killstreakTargetFromCanvas(p.x,p.y,{choose:true});return;}
...
async function createMatch(){
  ...
  try{
    ...
    const data=await response.json();if(!initialConnectionCurrent(attempt))return;if(!response.ok){if(response.status===409)void checkForClientUpdate('create-protocol');throw new Error(data.error||'Could not create lobby.');}void connectMatch(data.code,false,attempt);
...
function applyMatchPhaseTransition(previous,next){
  ...
  if(next.status===MATCH_STATUS.ENDED){
    clearKillstreakFx();resetKillstreakClientState();
...
function resetMatchPresentationForLobby(){
  ...
  resetTouchInput();clearFireInput();cancelEquipmentAction();keys.clear();clearRemotes();clearBullets();clearRocketTrailPuffs();clearThrowables();clearTacticalFx();clearSmokeClouds();clearKillstreakFx();resetKillstreakClientState();
...
function handleMessage(m){
  ...
  if(m.t==='state'){const r=remoteById(m.id);if(r){if(m.team&&normalizeClientTeam(m.team)!==r.team)applyRemoteTeamVisual(r,m.team,{reason:'state'});updateRemoteTarget(r,m);}return;}
  ...
  if(m.t==='weapon'){const r=remoteById(m.id);if(r){r.weapon=m.weapon||'pistol';r.swapStartedAt=performance.now();syncRemoteWeapon(r);}return;}
  if(m.t==='reload'){const r=remoteById(m.id);if(r){r.reloadUntil=Number(m.reloadAt)||0;r.reloadStartedAt=serverNow();r.reloadWeapon=m.weapon||r.weapon;if(r.reloadWeapon!=='shotgun'&&r.reloadUntil)playSpatialCue(reloadSoundId(r.reloadWeapon),r.group.position.x,r.group.position.y+1,r.group.position.z,34,.72);}return;}
  if(m.t==='reloadShell'){const r=remoteById(m.id);if(r){r.reloadUntil=Number(m.reloadAt)||0;r.reloadStartedAt=r.reloadUntil?serverNow():0;r.reloadWeapon=r.reloadUntil?'shotgun':'';playSpatialCue('reloadShotgun',r.group.position.x,r.group.position.y+1,r.group.position.z,34,.72);}return;}
  if(m.t==='god'){if(typeof m.custom==='boolean')matchCustom=m.custom;if(samePlayerId(m.id,clientId)){godMode=!!m.enabled;syncLobby();syncPauseContext();if(shell.panel===SHELL_PANEL.LOADOUT)syncMatchLoadoutEditor();}else{const row=lobbyParticipants.get(String(m.id||''));if(row)row.godMode=!!m.enabled;const r=remoteById(m.id);if(r){r.godMode=!!m.enabled;if(r.godRing)r.godRing.visible=r.godMode;}}renderAdminPlayers();syncLobby();return;}
  if(m.t==='adminRole'){if(samePlayerId(m.id,clientId)){isMatchAdmin=!!m.enabled;syncPauseContext();if(!isMatchAdmin&&shell.panel===SHELL_PANEL.ADMIN)closeAdminPanel();showToast(isMatchAdmin?'ADMIN PRIVILEGES GRANTED':'ADMIN PRIVILEGES REMOVED');}else{const row=lobbyParticipants.get(String(m.id||''));if(row)row.admin=!!m.enabled;const r=remoteById(m.id);if(r)r.admin=!!m.enabled;}renderAdminPlayers();syncLobby();return;}
  if(m.t==='teamQueued'){if(samePlayerId(m.id,clientId)){pendingTeam=m.pendingTeam||'';syncPauseContext();showToast(pendingTeam?`TEAM SWITCH QUEUED · ${pendingTeam.toUpperCase()}`:'TEAM SWITCH CANCELED');syncLobby();}return;}
  if(m.t==='matchLobby'){handleMatchLobby(m);return;}
  if(m.t==='match'){applyClientMatchState(m.match);matchCustom=!!m.custom;if(shell.panel===SHELL_PANEL.ADMIN&&m.rulesUpdated&&samePlayerId(m.by,clientId))setAdminStatus('');if(matchAllowsLobbyEdits(matchState)){syncLobby();syncModeVisuals();}return;}
  if(m.t==='matchReset'){if(m.mapId)setActiveMap(m.mapId,{rebuild:true});if(m.settings)applyWorldSettings(m.settings);if(m.botConfig)botConfig=normalizeBotConfig(m.botConfig);lobbyMatchDraft=null;lobbyMatchDirty=false;lobbyMapDraft='';lobbyMapDirty=false;lobbyLoadoutDraft=null;lobbyClassDrafts=null;lobbyLoadoutDirty=false;lobbyLoadoutRevision=0;lobbyLoadoutAckRevision=0;lobbyKillstreakDraft=[...killstreakSelection];lobbyKillstreakRevision=0;lobbyKillstreakAckRevision=0;lobbyKillstreakSyncPending=false;applyClientMatchState(m.match);matchCustom=!!m.custom;myStats={kills:0,deaths:0};resetKillstreakClientState();const players=m.players||[],bots=m.bots||[],self=players.find(pl=>samePlayerId(pl?.id,clientId))||null;if(self){myTeam=self.team||myTeam;pendingTeam='';if(self.activeClassId)activeClassId=normalizeLoadoutClassId(self.activeClassId);pendingClassId=self.pendingClassId?normalizeLoadoutClassId(self.pendingClassId):'';primaryWeapon=PRIMARY_WEAPONS.includes(self.primaryWeapon)?self.primaryWeapon:primaryWeapon;secondaryWeapon=SECONDARY_WEAPONS.includes(self.secondaryWeapon)?self.secondaryWeapon:secondaryWeapon;applyAttachmentState(self);tacticalEquipment=normalizeTactical(self.tactical);lethalEquipment=normalizeLethal(self.lethal);pendingLoadout=null;rememberPrimary(primaryWeapon);rememberSecondary(secondaryWeapon);rememberAttachments(primaryAttachments,secondaryAttachments);rememberEquipment(tacticalEquipment,lethalEquipment);rememberLoadoutClasses(loadoutClasses,activeClassId);}pendingGameSnapshot=gameSnapshot(self,players,bots,m.match?.serverTime);replaceLobbyParticipants(players,bots);syncModeVisuals();if(shell.inLobby||!engineReady){void enterGame(pendingGameSnapshot,{resetRound:true});}else applyGameSnapshot(pendingGameSnapshot,{resetRound:true});return;}
  if(m.t==='kill'){handleKill(m);return;}
  if(m.t==='settings'){applyWorldSettings(m.settings||DEFAULT_WORLD_SETTINGS);if(typeof m.custom==='boolean')matchCustom=m.custom;const section=m.section==='advanced'?'advanced':'gameplay';if(shell.panel===SHELL_PANEL.ADMIN){if(samePlayerId(m.by,clientId)){if(section==='advanced')populateAdminWeapons(worldSettings);else populateAdminGameplay(worldSettings);setAdminStatus('');}else if(activeAdminTab!==section){if(section==='advanced')populateAdminWeapons(worldSettings);else populateAdminGameplay(worldSettings);}}syncLobby();return;}
  ...
  if(m.t==='health'){if(samePlayerId(m.id,clientId)){hp=Math.max(0,Math.min(100,Number(m.hp)||0));syncLocalStatus();}else{const r=remoteById(m.id);if(r)r.hp=Math.max(0,Math.min(100,Number(m.hp)||0));}return;}
...
function renderAdminPlayers(force=false){
  ...
  for(const pl of adminPlayerSnapshot()){
    ...
    row.innerHTML=`<div class="admin-player-identity"><span class="admin-team-dot ${currentModeSpec().teamBased?pl.team:'ffa'}"></span><div><strong>${escapeHtml(pl.name)}${self?' (You)':''}${roleChip}</strong><small>${currentModeSpec().teamBased?String(pl.team||'blue').toUpperCase():'FFA'}</small></div></div><div class="admin-player-actions">${godAction}${roleAction}</div>`;root.appendChild(row);
  }
...
function applyRemoteTeamVisual(r,team,{reason='authoritative'}={}){
  if(!r)return;
  const previous=normalizeClientTeam(r.team),next=normalizeClientTeam(team),display=remoteDisplayColor(next);
  r.team=next;r.color=display;
  const teamMat=r.teamMaterial||r.thermalBaseMaterials?.body||r.body?.material;
  if(teamMat?.color)teamMat.color.set(display);
  if(teamMat?.emissive){teamMat.emissive.set(0x000000);teamMat.emissiveIntensity=0;}
  if(r.hitFlashTimer){clearTimeout(r.hitFlashTimer);r.hitFlashTimer=0;}
  r.hitFlashToken=(r.hitFlashToken||0)+1;
  if(r.tag){r.group.remove(r.tag);r.tag.material?.map?.dispose?.();r.tag.material?.dispose?.();r.tag=makeNameTag(r.bot?`[BOT] ${r.name}`:r.name,display);r.tag.position.set(0,2.18,0);r.tag.visible=r.hp>0&&(modeFriendly(next)||samePlayerId(r.id,aimedRemoteId));r.group.add(r.tag);}
  if(previous!==next)diagnosticsRecordEvent('team_visual_change',{playerId:playerKey(r.id),from:previous,to:next,reason,material:teamMat?.color?.getHexString?.()||''});
}
function diagnosticsTeamVisualSnapshot(){
  const rows=[];
  for(const [key,r] of remotes){
    const expected=remoteDisplayColor(r.team),mat=r.teamMaterial||r.thermalBaseMaterials?.body||r.body?.material,actual=mat?.color?`#${mat.color.getHexString()}`:'',emissive=mat?.emissive?`#${mat.emissive.getHexString()}`:'',sceneCopies=(()=>{if(!scene)return 0;let count=0;scene.traverse(object=>{if(playerKey(object?.userData?.remoteId)===playerKey(r.id))count++;});return count;})();
    rows.push({key:playerKey(key),id:playerKey(r.id),name:r.name||'',team:normalizeClientTeam(r.team),expected,actual,emissive,thermalActive:!!r.thermalActive,hitFlash:!!r.hitFlashTimer,sceneCopies});
  }
  return{self:{id:playerKey(clientId),team:normalizeClientTeam(myTeam),color:selfColor},remoteCount:remotes.size,players:rows};
}
...
function applyGameSnapshot(snapshot,{resetRound=false}={}){
  ...
  clearRemotes();
  const self=snapshot.self||snapshot.players.find(player=>samePlayerId(player?.id,clientId))||null;
...
function makeRemote(player){
  ...
  const team=normalizeClientTeam(player.team);
...
function upsertRemote(player,instant=false){
  ...
  if(samePlayerId(key,clientId)){for(const [id] of remotes){if(samePlayerId(id,clientId))removeRemote(id);}return;}
  let r=remotes.get(key);
  if(!r){r=makeRemote({...player,id:key});remotes.set(key,r);diagnosticsRecordEvent('remote_created',{playerId:key,team:r.team,bot:!!r.bot});}
  const authoritativeTeam=Object.prototype.hasOwnProperty.call(player,'team')?normalizeClientTeam(player.team):r.team;
  r.name=player.name||r.name;r.bot=!!player.bot;r.admin=player.admin??r.admin;r.weapon=player.weapon||r.weapon;
  if(PRIMARY_WEAPONS.includes(player.primaryWeapon))r.primaryWeapon=player.primaryWeapon;if(SECONDARY_WEAPONS.includes(player.secondaryWeapon))r.secondaryWeapon=player.secondaryWeapon;
  ...
  r.hp=player.hp??r.hp;r.kills=Number(player.kills??r.kills)||0;r.deaths=Number(player.deaths??r.deaths)||0;r.godMode=player.godMode??r.godMode;
  if(player.traversal&&typeof player.traversal==='object'&&Number(player.traversal.seq)!==Number(r.traversal?.seq))r.traversal=traversalPlanFromServer({id:key,accepted:true,...player.traversal});else if(player.bot&&player.traversal===null)r.traversal=null;
  if(player.ladder&&typeof player.ladder==='object')r.ladder=ladderStateFromServer(player.ladder);else if(player.ladder===null)r.ladder=null;if(r.godRing)r.godRing.visible=!!r.godMode;
  if(authoritativeTeam!==r.team)applyRemoteTeamVisual(r,authoritativeTeam,{reason:'upsert'});else{r.team=authoritativeTeam;r.color=remoteDisplayColor(authoritativeTeam);}
  syncRemoteWeapon(r);updateRemoteTarget(r,player,instant);
}
function removeRemote(id){const key=playerKey(id),r=remotes.get(key);if(!r)return;if(r.hitFlashTimer)clearTimeout(r.hitFlashTimer);restoreRemoteThermalSignature(r,true);scene.remove(r.group);r.group.traverse(o=>{if(o.geometry)o.geometry.dispose?.();if(o.material){if(o.material.map)o.material.map.dispose?.();o.material.dispose?.();}});remotes.delete(key);}
function clearRemotes(){for(const id of [...remotes.keys()])removeRemote(id);}

function deathLookYawFromHit(m){
  const attacker=m?.attacker&&!samePlayerId(m.attacker,clientId)?remoteById(m.attacker):null;
...
function enterLocalDeath(m){
  ...
  reloadRequestPending=false;reloadUntil=0;reloadWeapon='';reloadStartedAt=0;pendingWeapon='';weaponSwapStartedAt=0;localEquipmentCooldownUntil=0;sprintActionReadyAt=0;
  moveVelocityX=moveVelocityZ=0;localMoveAmount=0;
}
function handleHit(m){
  const targetRemote=remoteById(m.target);if(targetRemote){targetRemote.hp=m.hp;if(m.wasted){targetRemote.traversal=null;targetRemote.ladder=null;}flashRemote(targetRemote);}
  if(samePlayerId(m.attacker,clientId)){showHitmarker(!!m.headshot);if(m.headshot)soundHeadshot();else soundHitmarker(m.weapon||'pistol');}
  if(samePlayerId(m.target,clientId)){
...
function addDamageFeedback(m){
  ...
  if(Math.hypot(kx,kz)>.01){
    const attacker=m.attacker&&!samePlayerId(m.attacker,clientId)?remoteById(m.attacker):null,bearing=Math.atan2(kx,kz),knockLen=Math.hypot(kx,kz),sourceDistance=Math.max(1,Number(m.distance)||8);
...
function handleRespawn(player){
  if(!player?.id)return;
  const selfRespawn=samePlayerId(player.id,clientId);
  ...
  if(samePlayerId(player.id,clientId)&&Math.max(0,Number(player.hp)||0)<=0){
...
function handleTraversalMessage(m){
  ...
  const r=remoteById(m.id);if(!r||m.accepted===false)return;r.traversal=traversalPlanFromServer(m);
}
...
function handleLadderMessage(m){
  ...
  const r=remoteById(m.id);if(!r||m.accepted===false)return;r.ladder=m.ladder?ladderStateFromServer(m.ladder):null;if(m.action==='detach'||m.action==='dismount')r.ladder=null;
}
...
function effectiveAimYaw(){return yaw+(currentWeapon==='sniper'?sniperSwayYaw:0);}
function effectiveAimPitch(){return THREE.MathUtils.clamp(pitch+(currentWeapon==='sniper'?sniperSwayPitch:0),-1.28,1.28);}
...
function toggleFireMode(){if(!combatWeaponAvailable())return;if(currentWeapon!=='assault'){showToast('FIRE MODE · ASSAULT ONLY');return;}assaultFireMode=assaultFireMode==='semi'?'auto':'semi';clearFireInput();touchVisual.modeUntil=performance.now()+180;localStorage.setItem('breachAssaultFireMode',assaultFireMode);}
...
function beginEquipmentAim(kind){
  ...
  clearQueuedSprintShot();clearFireInput();cancelSprint();endRecoilBurst();setAim(false);
...
function isSimulationLeader(now=performance.now()){
  ...
  return samePlayerId(leader,clientId);
}
...
function createTracer(m){
  ...
  if(samePlayerId(m.ownerId,clientId)){const muzzle=localMuzzleObject(m.weapon,m.hand||'right');if(muzzle){camera.updateMatrixWorld(true);muzzle.getWorldPosition(start);}}
  let visualDir=dir.clone();if(samePlayerId(m.ownerId,clientId)){const converge=serverStart.clone().addScaledVector(dir,9);visualDir=converge.sub(start).normalize();}
...
function createLauncherProjectile(m){
  ...
  if(samePlayerId(m.ownerId,clientId)){const muzzle=localMuzzleObject(weapon);if(muzzle){camera.updateMatrixWorld(true);muzzle.getWorldPosition(visualStart);}}
...
function handleShot(m){
  ...
  if(samePlayerId(m.ownerId,clientId)){
...
function spawnThrowableVisual(m){
  ...
  if(samePlayerId(m.ownerId,clientId))soundThrowableThrow(kind);else playSpatialCue(sticky||frag?'stickyThrow':'flashThrow',m.x,m.y,m.z,30,.55);
}
...
function playBulletImpactSound(kind,x,y,z){
  ...
  playSpatialCue('impactWall',Number(x)||0,Number(y)||0,Number(z)||0,42,.92,{playbackRate:.94+Math.random()*.10});
}
function spawnBulletImpactFx(m){
  if(!scene||!THREE)return;const kind=String(m?.kind||'world'),playerHit=kind==='player',x=Number(m?.x)||0,y=Number(m?.y)||0,z=Number(m?.z)||0,root=new THREE.Group();root.position.set(x,y,z);
  const count=playerHit?8:6,positions=new Float32Array(count*3),velocities=new Float32Array(count*3),geometry=new THREE.BufferGeometry();
  for(let i=0;i<count;i++){const j=i*3,a=Math.random()*Math.PI*2,speed=(playerHit?.42:.82)+Math.random()*(playerHit?.72:1.05);velocities[j]=Math.cos(a)*speed;velocities[j+1]=(playerHit?.10:.18)+Math.random()*(playerHit?.70:1.05);velocities[j+2]=Math.sin(a)*speed;}
  geometry.setAttribute('position',new THREE.BufferAttribute(positions,3));
  const color=playerHit?0x9e1828:0xe1c49a,material=new THREE.PointsMaterial({color,size:playerHit?.055:.042,sizeAttenuation:true,transparent:true,opacity:playerHit?.72:.86,depthWrite:false,blending:playerHit?THREE.NormalBlending:THREE.AdditiveBlending});
...
function makeUfoVisual(){
  const root=new THREE.Group(),disc=new THREE.Mesh(new THREE.CylinderGeometry(2.15,2.55,.42,24),new THREE.MeshStandardMaterial({color:0x8f9ca5,metalness:.72,roughness:.28,emissive:0x16262c,emissiveIntensity:.32})),dome=new THREE.Mesh(new THREE.SphereGeometry(1.05,20,12,0,Math.PI*2,0,Math.PI*.55),new THREE.MeshStandardMaterial({color:0x6edff5,emissive:0x1f91ac,emissiveIntensity:.88,transparent:true,opacity:.78,roughness:.18,metalness:.15})),ring=new THREE.Mesh(new THREE.TorusGeometry(2.35,.11,8,32),new THREE.MeshBasicMaterial({color:0xd7ff58,transparent:true,opacity:.92})),underGlow=new THREE.Mesh(new THREE.RingGeometry(.85,2.05,28),new THREE.MeshBasicMaterial({color:0x78efff,transparent:true,opacity:.34,side:THREE.DoubleSide,depthWrite:false,blending:THREE.AdditiveBlending}));
  dome.position.y=.22;ring.rotation.x=Math.PI/2;underGlow.rotation.x=-Math.PI/2;underGlow.position.y=-.24;root.add(disc,dome,ring,underGlow);root.userData.underGlow=underGlow;return root;
}
function jaggedLightningPoints(start,end,segments=18,jitter=1){
  const out=[],direction=end.clone().sub(start),side=new THREE.Vector3(1,0,0),cross=new THREE.Vector3().crossVectors(direction,side);if(cross.lengthSq()<.01)cross.set(0,0,1);cross.normalize();const side2=new THREE.Vector3().crossVectors(direction,cross).normalize();
  for(let i=0;i<=segments;i++){const t=i/segments,p=start.clone().lerp(end,t);if(i>0&&i<segments){const taper=Math.sin(Math.PI*t),j=jitter*taper;p.addScaledVector(cross,(Math.random()-.5)*j).addScaledVector(side2,(Math.random()-.5)*j);}out.push(p);}return out;
}
function lightningLine(points,color=0xeaf8ff,opacity=1){
  const material=new THREE.LineBasicMaterial({color,transparent:true,opacity,depthWrite:false,blending:THREE.AdditiveBlending});material.userData.baseOpacity=opacity;return new THREE.Line(new THREE.BufferGeometry().setFromPoints(points),material);
}
function lightningScorchTexture(){
  const q=document.createElement('canvas');q.width=128;q.height=128;const c=q.getContext('2d'),g=c.createRadialGradient(64,64,5,64,64,62);g.addColorStop(0,'rgba(8,5,4,.92)');g.addColorStop(.34,'rgba(24,15,10,.84)');g.addColorStop(.68,'rgba(46,30,19,.52)');g.addColorStop(1,'rgba(34,22,15,0)');c.fillStyle=g;c.fillRect(0,0,128,128);for(let i=0;i<70;i++){const a=Math.random()*Math.PI*2,r=10+Math.random()*49,rr=.5+Math.random()*2.8;c.fillStyle=`rgba(${18+Math.floor(Math.random()*24)},${10+Math.floor(Math.random()*15)},${7+Math.floor(Math.random()*10)},${.18+Math.random()*.35})`;c.beginPath();c.arc(64+Math.cos(a)*r,64+Math.sin(a)*r,rr,0,Math.PI*2);c.fill();}const tex=new THREE.CanvasTexture(q);tex.colorSpace=THREE.SRGBColorSpace;tex.minFilter=THREE.LinearFilter;tex.magFilter=THREE.LinearFilter;return tex;
}
function buildLightningVisual(m){
  const x=Number(m.x)||0,y=Number(m.y)||0,z=Number(m.z)||0,root=new THREE.Group(),boltGroup=new THREE.Group(),structureGroup=new THREE.Group(),boltMaterials=[],structureMaterials=[];root.add(boltGroup,structureGroup);
  const strike=new THREE.Vector3(x,y,z),sky=new THREE.Vector3(x+(Math.random()-.5)*5,y+52,z+(Math.random()-.5)*5),mainPoints=jaggedLightningPoints(sky,strike,24,2.4),main=lightningLine(mainPoints,0xf4fbff,1);boltGroup.add(main);boltMaterials.push(main.material);
  const glow=lightningLine(mainPoints,0x62bfff,.48);boltGroup.add(glow);boltMaterials.push(glow.material);
  for(const idx of [5,8,11,14,17,20]){const origin=mainPoints[Math.min(idx,mainPoints.length-2)],drop=8+Math.random()*14,end=origin.clone().add(new THREE.Vector3((Math.random()-.5)*10,-drop,(Math.random()-.5)*10)),branch=lightningLine(jaggedLightningPoints(origin,end,7,.95),0xaedfff,.62);boltGroup.add(branch);boltMaterials.push(branch.material);}
  // Do not add transient PointLights here. Three.js bakes NUM_POINT_LIGHTS into
  // lit-material shader programs, so changing that count during a strike forces
  // synchronous recompilation of world/weapon/player MeshStandardMaterials.
  const flash=new THREE.Mesh(new THREE.SphereGeometry(2.9,12,8),new THREE.MeshBasicMaterial({color:0xbfe8ff,transparent:true,opacity:.28,depthWrite:false,blending:THREE.AdditiveBlending}));flash.position.copy(strike).add(new THREE.Vector3(0,1.25,0));boltGroup.add(flash);
  const hit=new THREE.Mesh(new THREE.SphereGeometry(.72,12,8),new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:.96,depthWrite:false,blending:THREE.AdditiveBlending}));hit.position.copy(strike);boltGroup.add(hit);boltMaterials.push(hit.material);
  const sx=Number(m.structureX),sz=Number(m.structureZ),sw=Math.max(.5,Number(m.structureW)||1),sd=Math.max(.5,Number(m.structureD)||1),roof=Math.max(y,Number(m.structureRoofY)||y),base=terrainHeight(Number.isFinite(sx)?sx:x,Number.isFinite(sz)?sz:z),sh=Math.max(.45,roof-base),structureX=Number.isFinite(sx)?sx:x,structureZ=Number.isFinite(sz)?sz:z;
  if(Number.isFinite(sw)&&Number.isFinite(sd)){
    const boxGeometry=new THREE.BoxGeometry(sw+.12,sh+.12,sd+.12),edgeGeometry=new THREE.EdgesGeometry(boxGeometry);boxGeometry.dispose();const edges=new THREE.LineSegments(edgeGeometry,new THREE.LineBasicMaterial({color:0x6edfff,transparent:true,opacity:.78,depthWrite:false,blending:THREE.AdditiveBlending}));edges.position.set(structureX,base+sh/2,structureZ);structureGroup.add(edges);structureMaterials.push(edges.material);
    const halfW=sw/2,halfD=sd/2;for(let i=0;i<7;i++){const side=i%4,ex=side===0?structureX-halfW:side===1?structureX+halfW:structureX+(Math.random()-.5)*sw,ez=side===2?structureZ-halfD:side===3?structureZ+halfD:structureZ+(Math.random()-.5)*sd,ey=base+Math.random()*sh,end=new THREE.Vector3(ex,ey,ez),arc=lightningLine(jaggedLightningPoints(strike,end,8,.48),0x8de8ff,.7);structureGroup.add(arc);structureMaterials.push(arc.material);}
  }
  const scorch=new THREE.Mesh(new THREE.PlaneGeometry(Math.min(4.2,Math.max(2.1,Math.min(sw,sd)*.42)),Math.min(4.2,Math.max(2.1,Math.min(sw,sd)*.42))),new THREE.MeshBasicMaterial({map:lightningScorchTexture(),transparent:true,opacity:.9,depthWrite:false,polygonOffset:true,polygonOffsetFactor:-2}));scorch.rotation.x=-Math.PI/2;scorch.rotation.z=Math.random()*Math.PI*2;scorch.position.set(x,y+.035,z);root.add(scorch);
  const smokeSprites=[],smokeTex=getSharedSmokeTexture();for(let i=0;i<5;i++){const mat=new THREE.SpriteMaterial({map:smokeTex,color:0x5b6265,transparent:true,opacity:.36,depthWrite:false}),sprite=new THREE.Sprite(mat);sprite.scale.set(1.7+Math.random()*1.5,1.7+Math.random()*1.5,1);sprite.position.set(x+(Math.random()-.5)*.8,y+.4+Math.random()*.8,z+(Math.random()-.5)*.8);sprite.userData={phase:Math.random()*Math.PI*2,baseX:sprite.position.x,baseZ:sprite.position.z,rise:.28+Math.random()*.22};root.add(sprite);smokeSprites.push(sprite);}
  return{root,boltGroup,structureGroup,boltMaterials,structureMaterials,flash,scorch,smokeSprites};
}
function earthquakeMapPoint(seed,index,cycle,limit){
  const a=Math.sin((seed%100003)*.00017+index*12.9898+cycle*4.1414)*43758.5453,b=Math.sin((seed%99991)*.00019+index*78.233+cycle*9.173)*24634.6345,u=a-Math.floor(a),v=b-Math.floor(b),span=Math.max(8,Number(limit)||24)*.92;
  return{x:(u*2-1)*span,z:(v*2-1)*span};
}
function buildEarthquakeVisual(m){
  const root=new THREE.Group(),seed=Math.floor(Number(m.seed)||tracerHash(String(m.id||'quake'))),limit=Math.max(24,minimapWorldLimit()),centerY=worldSupportHeight(0,0,1000,false),count=isTouch?28:52,geometry=new THREE.BoxGeometry(.30,.23,.34),material=new THREE.MeshBasicMaterial({color:0x67584a,transparent:true,opacity:0}),debris=new THREE.InstancedMesh(geometry,material,count),dummy=new THREE.Object3D(),items=[];
  debris.instanceMatrix.setUsage(THREE.DynamicDrawUsage);root.add(debris);
  for(let i=0;i<count;i++){
    const pt=earthquakeMapPoint(seed,i,0,limit),ground=worldSupportHeight(pt.x,pt.z,1000,false),a=((seed+i*977)%6283)/1000,scale=.62+((seed+i*43)%115)/100,spin=1.05+((seed+i*67)%220)/100;
    items.push({x:pt.x,z:pt.z,y:ground+10+((seed+i*211)%1600)/100,vx:Math.sin(a*1.7)*.95,vz:Math.cos(a*1.3)*.95,vy:-(8.8+((seed+i*89)%520)/100),rx:a,ry:a*.7,rz:a*.3,spin,scale,cycle:0});
    dummy.position.set(pt.x,items[i].y,pt.z);dummy.scale.setScalar(scale);dummy.rotation.set(items[i].rx,items[i].ry,items[i].rz);dummy.updateMatrix();debris.setMatrixAt(i,dummy.matrix);
  }
  debris.instanceMatrix.needsUpdate=true;
  const dustCount=isTouch?84:160,dustGeo=new THREE.BufferGeometry(),dustPos=new Float32Array(dustCount*3),dustItems=[];
  for(let i=0;i<dustCount;i++){
    const pt=earthquakeMapPoint(seed+317,i,0,limit),y=worldSupportHeight(pt.x,pt.z,1000,false)+.12+((seed+i*73)%220)/100;
    dustPos[i*3]=pt.x;dustPos[i*3+1]=y;dustPos[i*3+2]=pt.z;dustItems.push({x:pt.x,z:pt.z,baseY:y,speed:.22+((seed+i*29)%60)/100,phase:(seed+i*97)%6283/1000,drift:.55+((seed+i*31)%90)/100});
  }
  dustGeo.setAttribute('position',new THREE.BufferAttribute(dustPos,3));
  const dust=new THREE.Points(dustGeo,new THREE.PointsMaterial({color:0x9a8064,size:isTouch?.62:.78,transparent:true,opacity:.34,depthWrite:false,sizeAttenuation:true}));root.add(dust);
  return{root,debris,debrisDummy:dummy,debrisItems:items,dust,dustItems,seed,limit,centerY};
}
function captureSolarEnvironment(){
  if(solarEnvironmentBaseline)return solarEnvironmentBaseline;
  solarEnvironmentBaseline={
    exposure:Number.isFinite(Number(renderer?.toneMappingExposure))?Number(renderer.toneMappingExposure):1.05,
    sunIntensity:Number.isFinite(Number(worldSunLight?.intensity))?Number(worldSunLight.intensity):2.15,
    hemiIntensity:Number.isFinite(Number(worldHemiLight?.intensity))?Number(worldHemiLight.intensity):2.2,
  };
  return solarEnvironmentBaseline;
}
function buildSolarNukeVisual(m){
  const baseline=captureSolarEnvironment(),limit=Math.max(72,minimapWorldLimit()),root=new THREE.Group(),core=new THREE.Mesh(new THREE.SphereGeometry(1,34,26),new THREE.MeshBasicMaterial({color:0xfff3b8,depthWrite:true})),halo=new THREE.Mesh(new THREE.SphereGeometry(1.18,26,18),new THREE.MeshBasicMaterial({color:0xffa43a,transparent:true,opacity:.15,depthWrite:false,blending:THREE.AdditiveBlending,side:THREE.BackSide}));
  root.add(halo,core);root.position.set(limit*.94,Math.max(160,limit*1.45),limit*.78);root.scale.setScalar(7.5);
  const endPos=new THREE.Vector3(limit*.08,48,limit*.06),retreatPos=new THREE.Vector3(limit*.08,Math.max(178,limit*1.52),limit*.06);
  return{root,core,halo,startPos:root.position.clone(),endPos,retreatPos,baseExposure:baseline.exposure,baseSunIntensity:baseline.sunIntensity,baseHemiIntensity:baseline.hemiIntensity,burnField:null};
}
function activeEarthquakeVisual(){const now=serverNow();for(let i=killstreakFx.length-1;i>=0;i--){const f=killstreakFx[i];if(f.type==='earthquake'&&f.affectsLocal!==false&&now>=f.start&&now<f.end)return f;}return null;}
function activeSolarNukeVisual(){const now=serverNow();for(let i=killstreakFx.length-1;i>=0;i--){const f=killstreakFx[i];if(f.type==='solarnuke'&&now>=f.start&&now<f.end)return f;}return null;}
function restoreSolarEnvironment(){
  const baseline=solarEnvironmentBaseline;if(!baseline)return;
  if(renderer&&Number.isFinite(baseline.exposure))renderer.toneMappingExposure=baseline.exposure;
  if(worldSunLight&&Number.isFinite(baseline.sunIntensity))worldSunLight.intensity=baseline.sunIntensity;
  if(worldHemiLight&&Number.isFinite(baseline.hemiIntensity))worldHemiLight.intensity=baseline.hemiIntensity;
  solarEnvironmentBaseline=null;
}
function solarBurnTexture(seed=1){
  const q=document.createElement('canvas');q.width=256;q.height=256;const c=q.getContext('2d');c.clearRect(0,0,256,256);c.save();c.translate(128,128);
  const points=34;c.beginPath();for(let i=0;i<points;i++){const a=i/points*Math.PI*2,j=.78+.12*Math.sin(a*5+seed*.0017)+.07*Math.sin(a*11+seed*.0031),r=111*j,x=Math.cos(a)*r,y=Math.sin(a)*r;if(i)c.lineTo(x,y);else c.moveTo(x,y);}c.closePath();c.clip();
  const g=c.createRadialGradient(-12,-10,4,0,0,118);g.addColorStop(0,'rgba(5,4,3,.98)');g.addColorStop(.32,'rgba(17,10,6,.96)');g.addColorStop(.66,'rgba(48,23,11,.88)');g.addColorStop(.86,'rgba(98,42,13,.60)');g.addColorStop(1,'rgba(122,54,16,.12)');c.fillStyle=g;c.fillRect(-128,-128,256,256);
  for(let i=0;i<46;i++){const a=((seed+i*239)%6283)/1000,r=18+((seed+i*83)%7600)/100,rr=1.2+((seed+i*29)%260)/100;c.fillStyle=`rgba(${70+(i*17)%70},${18+(i*11)%34},${4+(i*7)%12},${.18+(i%5)*.055})`;c.beginPath();c.arc(Math.cos(a)*r,Math.sin(a)*r,rr,0,Math.PI*2);c.fill();}
  c.lineCap='round';for(let i=0;i<14;i++){const a=((seed+i*421)%6283)/1000,len=28+((seed+i*71)%5200)/100;c.strokeStyle=i%3===0?'rgba(255,92,18,.34)':'rgba(9,5,3,.66)';c.lineWidth=i%3===0?1.2:2.1;c.beginPath();let x=Math.cos(a)*12,y=Math.sin(a)*12;c.moveTo(x,y);for(let k=1;k<=4;k++){const d=len*k/4,aa=a+Math.sin(seed*.0003+i*1.7+k)*.16;x=Math.cos(aa)*d;y=Math.sin(aa)*d;c.lineTo(x,y);}c.stroke();}
  c.restore();const tex=new THREE.CanvasTexture(q);tex.colorSpace=THREE.SRGBColorSpace;tex.minFilter=THREE.LinearFilter;tex.magFilter=THREE.LinearFilter;return tex;
}
function prepareSolarBurnField(solar,m={}){
  if(!scene||!THREE||!solar)return null;const seed=tracerHash(`${m.id||'solar'}:${Math.floor(Number(m.startedAt)||serverNow())}`),root=new THREE.Group(),texture=solarBurnTexture(seed),markGeo=new THREE.PlaneGeometry(1,1),markMat=new THREE.MeshBasicMaterial({map:texture,transparent:true,opacity:.90,depthWrite:false,side:THREE.DoubleSide,polygonOffset:true,polygonOffsetFactor:-3}),smokeTex=getSharedSmokeTexture(),candidates=[];
  const addCandidate=(x,z,w,d,y,kind,index)=>{if(!Number.isFinite(x)||!Number.isFinite(z)||!Number.isFinite(y))return;const width=Math.max(1.5,Math.min(7.2,Number(w)||3.2)),depth=Math.max(1.5,Math.min(7.2,Number(d)||3.2)),mark=new THREE.Mesh(markGeo,markMat);mark.rotation.x=-Math.PI/2;mark.rotation.z=((seed+index*313)%6283)/1000;mark.position.set(x,y+.035,z);mark.scale.set(width*.18,depth*.18,1);mark.visible=false;root.add(mark);const smoke=[];const smokeCount=kind==='building'&&!isTouch?2:1;for(let s=0;s<smokeCount;s++){const mat=new THREE.SpriteMaterial({map:smokeTex,color:0x4b4540,transparent:true,opacity:0,depthWrite:false}),sprite=new THREE.Sprite(mat),size=1.8+Math.min(width,depth)*.22+s*.25;sprite.scale.set(size,size,1);sprite.position.set(x+(s?width*.16:-width*.08),y+.22,z+(s?-depth*.12:depth*.07));sprite.visible=false;root.add(sprite);smoke.push(sprite);}const heatFactor=.88+((seed+index*97)%250)/1000;candidates.push({x,y,z,width,depth,kind,mark,smoke,heatFactor,burnAt:0});};
  const spreadPick=(items,count)=>{if(items.length<=count)return items;const out=[];for(let i=0;i<count;i++)out.push(items[Math.min(items.length-1,Math.floor(i*(items.length-1)/Math.max(1,count-1)))]);return out;};
...
function updateSolarBurnProgress(solar,approach,now=performance.now(),force=false){
  const field=solar?.burnField;if(!field)return;const sun=solar.root.position,radius=Math.max(1,solar.root.scale.x),heatReach=force?170:THREE.MathUtils.lerp(4,138,smoothstep01(Math.max(0,(approach-.10)/.90)));
  for(const candidate of field.candidates){if(candidate.burnAt)continue;const dx=candidate.x-sun.x,dy=candidate.y-sun.y,dz=candidate.z-sun.z,shellDistance=Math.max(0,Math.hypot(dx,dy,dz)-radius);if(shellDistance<=heatReach*candidate.heatFactor)igniteSolarCandidate(candidate,now);}
}
...
function updateKillstreakFx(dt){
  const srv=serverNow(),now=performance.now();ufoLocalLift=0;
  for(let i=killstreakFx.length-1;i>=0;i--){const f=killstreakFx[i];let expired=false;
    if(f.type==='ufoFlyby'){const duration=Math.max(1,f.end-f.start),p=Math.max(0,Math.min(1,(srv-f.start)/duration)),limit=minimapWorldLimit()+42,dir=f.index%2?-1:1,curve=Math.sin(Math.PI*p);f.root.position.set(dir*(-limit+2*limit*p),31+f.index*3-curve*5,(-.72+f.index*.72)*minimapWorldLimit());f.root.rotation.y=dir>0?-Math.PI/2:Math.PI/2;f.root.rotation.z=Math.sin(p*Math.PI*5)*.04;expired=srv>f.end;}
    else if(f.type==='ufoAbduct'){
      const actor=samePlayerId(f.targetId,clientId)?null:remoteById(f.targetId),baseX=actor?.group?.position?.x??f.targetX,baseZ=actor?.group?.position?.z??f.targetZ,baseY=actor?.group?.position?.y??position?.y??f.targetY,total=Math.max(1,f.killAt-f.start),p=Math.max(0,Math.min(1,(srv-f.start)/total)),approach=smoothstep01(p/.28),hoverY=baseY+11.2,targetPos=new THREE.Vector3(baseX,hoverY,baseZ);f.root.position.lerpVectors(f.startPos,targetPos,approach);f.root.rotation.y+=dt*(approach<.98?3.5:1.6);f.root.rotation.z=(1-approach)*Math.sin(p*Math.PI*3)*.12;
      const beamP=smoothstep01((p-.25)/.16),liftP=smoothstep01((p-.34)/.56),lift=9.6*liftP;f.beam.visible=beamP>.01;f.beamGlow.visible=beamP>.01;f.beam.material.opacity=beamP*(.18+.16*Math.sin(now*.026));f.beamGlow.material.opacity=beamP*(.18+.12*Math.sin(now*.019));if(f.root.userData.underGlow)f.root.userData.underGlow.material.opacity=.3+.18*beamP;
      if(samePlayerId(f.targetId,clientId))ufoLocalLift=Math.max(ufoLocalLift,lift);else if(actor?.group)actor.group.position.y+=lift;expired=srv>f.end;
    }
    else if(f.type==='asteroid'){const p=Math.max(0,Math.min(1,(srv-f.start)/Math.max(1,f.impactAt-f.start))),height=42*(1-p);f.root.position.set(f.x+7*(1-p),f.y+height,f.z-6*(1-p));f.root.rotation.x+=dt*9;f.root.rotation.z+=dt*6;f.root.scale.setScalar(.72+.6*p);expired=srv>f.end;}
    else if(f.type==='lightning'){
      const boltRemain=Math.max(0,(f.boltEnd-now)/Math.max(1,f.boltEnd-f.start)),boltAlive=boltRemain>0,flicker=boltAlive?(.6+.4*Math.abs(Math.sin(now*.11))):0;if(f.boltGroup)f.boltGroup.visible=boltAlive;if(boltAlive){for(const mat of f.boltMaterials)mat.opacity=Math.min(1,(mat.userData?.baseOpacity||1)*flicker);if(f.flash){f.flash.material.opacity=.30*boltRemain*(.45+.55*Math.abs(Math.sin(now*.13)));f.flash.scale.setScalar(.82+(1-boltRemain)*.42);}}const electricRemain=Math.max(0,(f.electricEnd-now)/Math.max(1,f.electricEnd-f.start)),electricAlive=electricRemain>0;if(f.structureGroup)f.structureGroup.visible=electricAlive;if(electricAlive)for(const mat of f.structureMaterials)mat.opacity=.78*electricRemain*(.55+.45*Math.abs(Math.sin(now*.072)));
      const smokeRemain=Math.max(0,(f.smokeEnd-now)/Math.max(1,f.smokeEnd-f.start));for(const sprite of f.smokeSprites){const u=sprite.userData||{};sprite.position.y+=dt*(u.rise||.35);sprite.position.x=(u.baseX||sprite.position.x)+Math.sin(now*.0012+(u.phase||0))*.22;sprite.position.z=(u.baseZ||sprite.position.z)+Math.cos(now*.0010+(u.phase||0))*.22;sprite.material.opacity=.34*smokeRemain;}if(f.scorch){const fade=now<f.smokeEnd?1:Math.max(0,(f.end-now)/Math.max(1,f.end-f.smokeEnd));f.scorch.material.opacity=.88*fade;}expired=now>f.end;
    }else if(f.type==='earthquake'){
      const p=THREE.MathUtils.clamp((srv-f.start)/Math.max(1,f.end-f.start),0,1),fade=Math.min(1,p/.06,Math.max(0,(1-p)/.12)),limit=Math.max(24,Number(f.limit)||minimapWorldLimit());
      for(let j=0;j<f.debrisItems.length;j++){
        const item=f.debrisItems[j];item.x+=item.vx*dt;item.z+=item.vz*dt;item.y+=item.vy*dt;item.rx+=item.spin*dt;item.ry+=item.spin*.73*dt;item.rz+=item.spin*.49*dt;
        const ground=worldSupportHeight(item.x,item.z,item.y,false);
        if(item.y<ground-.4||Math.abs(item.x)>limit||Math.abs(item.z)>limit){item.cycle=(item.cycle||0)+1;const pt=earthquakeMapPoint(f.seed,j,item.cycle,limit);item.x=pt.x;item.z=pt.z;item.y=worldSupportHeight(item.x,item.z,1000,false)+10+((f.seed+j*233+item.cycle*47)%1600)/100;item.vx=Math.sin((f.seed+j*37+item.cycle*19)*.013)*.95;item.vz=Math.cos((f.seed+j*53+item.cycle*23)*.011)*.95;item.vy=-(8.8+((f.seed+j*89+item.cycle*31)%520)/100);}
        f.debrisDummy.position.set(item.x,item.y,item.z);f.debrisDummy.scale.setScalar(item.scale);f.debrisDummy.rotation.set(item.rx,item.ry,item.rz);f.debrisDummy.updateMatrix();f.debris.setMatrixAt(j,f.debrisDummy.matrix);
      }
      f.debris.instanceMatrix.needsUpdate=true;f.debris.material.opacity=fade;
      const attr=f.dust.geometry.attributes.position;for(let j=0;j<f.dustItems.length;j++){const item=f.dustItems[j],drift=item.drift||1;attr.array[j*3]=item.x+Math.sin(now*.0014+item.phase)*drift;attr.array[j*3+1]=item.baseY+((now*.001*item.speed)%5.6);attr.array[j*3+2]=item.z+Math.cos(now*.0011+item.phase)*drift;}attr.needsUpdate=true;f.dust.material.opacity=.42*fade;expired=srv>f.end;
    }else if(f.type==='solarnuke'){
      const pre=THREE.MathUtils.clamp((srv-f.start)/Math.max(1,f.blastAt-f.start),0,1),approach=smoothstep01(pre),lightRamp=smoothstep01(Math.max(0,(pre-.10)/.90)),post=Math.max(0,srv-f.blastAt),postDuration=Math.max(1,f.end-f.blastAt),postP=THREE.MathUtils.clamp(post/postDuration,0,1),retreatP=f.blasted?smoothstep01(Math.max(0,(post-700)/Math.max(1,postDuration-700))):0;
...
function animate(){
  ...
  try{updateKillstreakFx(visualDt);}catch(error){console.error('Killstreak visual update failed; clearing transient killstreak FX.',error);clearKillstreakFx();}
...
function controllerFocusKey(el){
  ...
  const stableData=['deployTab','lobbySideTab','lobbyCheatTab','lobbyAdminGod','lobbyAdminRole','adminGod','adminRole','lobbyMapChoice','lobbyMode','killstreakChoice','loadoutChoice','loadoutClass','loadoutEditItem','loadoutBackClass','loadoutBackClasses','loadoutWeaponPickerToggle','loadoutAdsPreview','calloutSlot','matchPrimaryChoice','matchTacticalChoice','matchLethalChoice','settingsTab','adminTab','chatChar','chatAction','editorChar','editorAction','gameControl'];
...
function controllerFieldAdjustHeld(){
  const held=gamepadFrame.held||[];if(held[GAMEPAD_BUTTON.DPAD_LEFT]||held[GAMEPAD_BUTTON.DPAD_RIGHT])return true;
  return Math.abs(Number(gamepadFrame.moveX)||0)>=.40;
}
...
function cycleControllerTabs(direction){
  ...
  requestAnimationFrame(()=>{if(controllerUiSurface()!==surface)return;clearControllerUiFocus();ensureControllerUiFocus();});
  return true;
}
function handleControllerUiNavigation(pressed){
  ...
  if(nav){
    if(controllerEditableField(focus)&&nav.dx){stepControllerField(focus,nav.dx);return true;}
...
function updateGamepadInput(dt){
  ...
  // Scoreboard is a read-only hold overlay. While View is held, consume the
...
function computeHudLayout(){
  ...
  const modeX=crouchX-crouchR-modeR-9,modeY=(topRowY+fireY)/2,steadyX=leftFireX+leftFireR+modeR+(compact?8:10),steadyY=leftFireY,weaponLift=touchGameplayControlsVisible()?(fireR*3.75+8):0;
  const weaponX=viewW-safe.right-margin-weaponW,weaponY=viewH-bottom-weaponH-weaponLift;
  const streakW=Math.min(contentW*.50,Math.max(mapSize,compact?145:(isTouch?160:180))),streakH=compact?38:42;let streakX=contentRight-streakW,streakY=controlsY+menuH+5;
  const streakHitsWeapon=streakX<weaponX+weaponW+6&&streakX+streakW>weaponX-6&&streakY<weaponY+weaponH+6&&streakY+streakH>weaponY-6;
  if(streakHitsWeapon){streakX=Math.max(contentLeft,Math.min(weaponX-streakW-10,mapX-streakW-10));if(streakX+streakW>weaponX-6)streakX=contentLeft;}
  ...
  return{compact,safe,moveBoundary,
    ...
    chat:{x:chatX,y:controlsY,w:chatW,h:menuH},menu:{x:menuX,y:controlsY,w:menuW,h:menuH},streak:{x:streakX,y:streakY,w:streakW,h:streakH},
    ...
    fire:{x:fireX,y:fireY,r:fireR},aim:{x:aimX,y:aimY,r:aimR},jump:{x:jumpX,y:jumpY,r:jumpR},reload:{x:reloadX,y:reloadY,r:reloadR},swap:{x:swapX,y:swapY,r:swapR},mode:{x:modeX,y:modeY,r:modeR},steady:{x:steadyX,y:steadyY,r:modeR},
...
function drawKillstreakPanel(c,r){
  if(!r)return;const equipped=normalizeKillstreakSelection(killstreakSelection),slotW=r.w/equipped.length;c.save();roundRect(c,r.x,r.y,r.w,r.h,7,'rgba(7,11,14,.88)','rgba(255,255,255,.12)');c.textAlign='center';c.textBaseline='middle';
  for(let i=0;i<equipped.length;i++){const kind=equipped[i],spec=KILLSTREAK_SPECS[kind],count=killstreakStatus.available.filter(v=>v===kind).length,ready=count>0,selected=ready&&killstreakSelected===kind,x=r.x+i*slotW,label=kind==='lightning'?'LGT':kind==='asteroids'?'AST':kind==='earthquake'?'EQ':kind==='solarnuke'?'NUKE':spec.short;if(i)c.fillStyle='rgba(255,255,255,.09)',c.fillRect(x,r.y+5,1,r.h-10);if(selected){c.fillStyle='rgba(215,255,88,.14)';c.fillRect(x+1,r.y+1,slotW-2,r.h-2);}c.fillStyle=ready?HUD_ACCENT:'rgba(225,233,238,.50)';c.font=`1000 ${slotW<38?8:9}px system-ui`;c.fillText(label,x+slotW/2,r.y+12);c.font=`900 ${slotW<38?7.5:8.5}px system-ui`;c.fillStyle=ready?'#fff':'rgba(180,191,199,.60)';c.fillText(ready?(count>1?`READY×${count}`:'READY'):`${spec.kills} K`,x+slotW/2,r.y+r.h-10);}
  c.textAlign='left';c.font='1000 7px system-ui';c.fillStyle='rgba(255,255,255,.60)';c.fillText(`STREAK ${killstreakStatus.streak}`,r.x+4,r.y+r.h-2);c.restore();
}
function drawKillstreakTargetingPlayers(c,r){
...
function drawKillstreakTargeting(c,w,h){
  ...
  c.fillStyle='#b9c2c7';c.font='850 9px system-ui';const hint=controllerInputActive()?(killstreakTargeting.chosen?'TARGET LOCKED · A DEPLOY · B EDIT':'LS MOVE TARGET · A SELECT · B CANCEL'):isTouch?'TAP MAP TO CHOOSE · THEN TAP DEPLOY':'MOVE/CLICK MAP TO CHOOSE · ENTER OR DEPLOY TO CONFIRM';c.fillText(hint,w/2,buttonY-9);c.restore();
}
...
function drawHud(now){
  ...
  if(!scoped||scopeAmount<.82){drawKillFeed(c,L.kill,now);drawMiniMap(c,L.map);drawTeamBar(c,L.team);drawTeamIndicator(c,L.teamIndicator);drawKillstreakPanel(c,L.streak);}if(scoped){drawScopeMask(c,w,h,scopeAmount);drawThermalOptic(c,w,h,thermalOpticAmount());}
...
function drawMatchStatus(c,w,h){
  ...
  if(matchState.status==='ended'){
    ...
    const panelW=Math.min(470,w-30),panelH=Math.min(185,h-30),x=(w-panelW)/2,y=Math.max(15,(h-panelH)/2-6),reason=matchState.reason==='score'?'SCORE LIMIT':matchState.reason==='time'?'TIME LIMIT':'MATCH COMPLETE';
...
function weaponLabel(w){return WEAPON_SPECS[w]?.name||EQUIPMENT_SPECS[w]?.name||KILLSTREAK_SPECS[w]?.name||'PISTOL';}
function drawWeapon(c,r){
  ...
  const left=r.x+10,top=r.y+11,right=r.x+r.w-9;
  ...
  const statusParts=[];if(currentWeapon==='assault')statusParts.push(assaultFireMode.toUpperCase());if(currentWeapon==='sniper')statusParts.push(sniperZoomLabel());if(godMode)statusParts.push(isTouch?'◆ GOD':'GOD');const statusLine=statusParts.join(' · ');if(statusLine){c.fillStyle=godMode?HUD_ACCENT:HUD_MUTED;c.font=`850 ${isTouch?8.5:10}px system-ui`;c.fillText(statusLine,left,top+13);}
...
function drawMiniMap(c,r){
  const mapLimit=minimapWorldLimit(),ix=r.x,iy=r.y,iw=r.w,ih=r.h,cx=ix+iw/2,cy=iy+ih/2,visibleWorld=(mapLimit*2)/MINIMAP_ZOOM,ppm=iw/visibleWorld,now=performance.now(),staticMap=getMinimapStatic(),staticScale=(mapLimit*2)/staticMap.width,headingUp=true,rotation=yaw,sinR=Math.sin(rotation),cosR=Math.cos(rotation);
...
function handleKill(m){
  if(!m?.attacker||!m?.victim)return;const update=p=>{if(!p)return;if(samePlayerId(p.id,clientId)){myStats.kills=Number(p.kills)||0;myStats.deaths=Number(p.deaths)||0;}else{const r=remoteById(p.id);if(r){r.kills=Number(p.kills)||0;r.deaths=Number(p.deaths)||0;}}};update(m.attacker);update(m.victim);
  if(samePlayerId(m.victim.id,clientId)){lastWastedBy=m.attacker.name||'Player';lastWastedWeapon=m.weapon||'';lastWastedHeadshot=!!m.headshot;lastWastedDistance=Math.max(0,Number(m.distance)||0);const killer=remoteById(m.attacker.id);if(killer){const dx=killer.group.position.x-position.x,dz=killer.group.position.z-position.z;if(Math.hypot(dx,dz)>.05)deathViewTargetYaw=Math.atan2(-dx,-dz);}}
  const mine=samePlayerId(m.attacker.id,clientId)&&!samePlayerId(m.victim.id,clientId),multi=Math.max(0,Number(m.multiKill)||0),distance=Math.max(0,Number(m.distance)||0);
  if(mine){
...
function drawKillFeed(c,r,now){
  ...
  for(const item of killFeed){const h=23,localKill=samePlayerId(item.attacker.id,clientId)&&!samePlayerId(item.victim.id,clientId),localDeath=itesamePlayerId(m.victim.id,clientId),weapon=`${WEAPON_SPECS[item.weapon]?.short||EQUIPMENT_SPECS[item.weapon]?.short||KILLSTREAK_SPECS[item.weapon]?.short||'PST'}${item.headshot?' ◆':''}`;if(y+h>r.y+r.h)break;
...
function drawScopeMask(c,w,h,amount=1){
  const a=THREE.MathUtils.clamp(amount,0,1),r=Math.min(w,h)*MAGNIFIED_SCOPE_SCREEN_RADIUS,cx=w/2,cy=h/2;if(a<=.001)return;
...
function ensureRemoteThermalMaterials(r){
  ...
  r.thermalMaterials.warm.userData.thermalOpticSignature=true;r.thermalMaterials.hot.userData.thermalOpticSignature=true;
}
...
function restoreRemoteThermalSignature(r,dispose=false){
  if(!r)return;const base=r.thermalBaseMaterials;if(base){if(r.body)r.body.material=base.body;if(r.head)r.head.material=base.head;if(r.armL)r.armL.material=base.armL;if(r.armR)r.armR.material=base.armR;if(r.legL)r.legL.material=base.legL;if(r.legR)r.legR.material=base.legR;}if(r.teamMaterial?.color)r.teamMaterial.color.set(remoteDisplayColor(r.team));if(r.teamMaterial?.emissive&&!r.hitFlashTimer){r.teamMaterial.emissive.set(0x000000);r.teamMaterial.emissiveIntensity=0;}r.thermalActive=false;r.thermalMode='';
...
function syncThermalOpticSignatures(){
  const mode=thermalOpticMode(),active=!!mode&&thermalOpticScopeAmount()>.90&&hp>0;
...
function drawThermalOptic(c,w,h,amount=1){
  const mode=thermalOpticMode(),a=THREE.MathUtils.clamp(amount,0,1);if(!mode||a<=.001)return;const r=Math.min(w,h)*MAGNIFIED_SCOPE_SCREEN_RADIUS,cx=w/2,cy=h/2;c.save();c.beginPath();c.arc(cx,cy,r-5,0,Math.PI*2);c.clip();c.globalAlpha=a;c.fillStyle=mode==='orange'?'rgba(16,19,20,.10)':'rgba(10,12,13,.14)';c.fillRect(cx-r,cy-r,r*2,r*2);const vignette=c.createRadialGradient(cx,cy,r*.46,cx,cy,r);vignette.addColorStop(0,'rgba(255,255,255,.015)');vignette.addColorStop(.72,'rgba(0,0,0,0)');vignette.addColorStop(1,mode==='orange'?'rgba(5,7,8,.30)':'rgba(3,4,5,.36)');c.fillStyle=vignette;c.fillRect(cx-r,cy-r,r*2,r*2);c.globalAlpha=a*.10;c.fillStyle='rgba(255,255,255,.28)';for(let y=cy-r;y<cy+r;y+=4)c.fillRect(cx-r,y,r*2,1);c.restore();
}
function drawScopeReticle(c,w,h,hit,headshot=false,amount=1){
  const a=THREE.MathUtils.clamp(amount,0,1),r=Math.min(w,h)*MAGNIFIED_SCOPE_SCREEN_RADIUS,cx=w/2,cy=h/2,gap=Math.max(7,r*.032),post=Math.max(30,r*.17);if(a<=.001)return;
  ...
  c.font='900 9px system-ui';c.textAlign='center';c.fillStyle='rgba(255,255,255,.72)';c.shadowColor='rgba(0,0,0,.8)';c.shadowBlur=2;c.fillText(activeScopeZoomLabel(),cx,cy-r+20);c.restore();
}
function drawSniperBreathMeter(c,w,h){
  if(currentWeapon!=='sniper'||adsBlend<SNIPER_BREATH_READY_BLEND)return;
  const used=sniperBreathReserve<.995||sniperBreathActive||sniperBreathExhausted;if(!used)return;
...
function drawControlIcon(c,x,y,r,type,active){
  ...
  if(type==='fire'){
    ...
    c.beginPath();c.moveTo(x-q*.70,y);c.lineTo(x-q*.40,y);c.bezierCurveTo(x-q*.30,y-q*.20,x-q*.18,y+q*.20,x-q*.08,y);c.bezierCurveTo(x+q*.02,y-q*.20,x+q*.14,y+q*.20,x+q*.24,y);c.lineTo(x+q*.70,y);c.stroke();
  }else if(type==='mode'){
