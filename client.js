...
import * as HighlandsGeometry from './world-geometry.js?v=1.41.1';
import * as DepotGeometry from './world-geometry-depot.js?v=1.41.1';
import * as YardGeometry from './world-geometry-yard.js?v=1.41.1';
import * as RigGeometry from './world-geometry-rig.js?v=1.41.1';
import * as HighlandsWorldCollision from './world-collision.js?v=1.41.1';
import * as DepotWorldCollision from './world-collision-depot.js?v=1.41.1';
import * as YardWorldCollision from './world-collision-yard.js?v=1.41.1';
import * as RigWorldCollision from './world-collision-rig.js?v=1.41.1';
import {
...
} from './game-config.js?v=1.41.1';
import { createProjectileCollisionGrid } from './collision-grid.js?v=1.41.1';
import { createAudioEngine } from './audio-engine.js?v=1.41.1';
import { normalizeMatchState as normalizeSharedMatchState } from './match-model.js?v=1.41.1';
import { MATCH_STATUS, matchAllowsLobbyEdits, matchAllowsMovement, matchAllowsCombat, matchPhaseChanged } from './gameplay-phase.js?v=1.41.1';
import { MAX_PLAYER_PHYSICS_STEP_SEC, advanceVerticalMotion, advanceKnockback, sweepHorizontalMovement, createTraversalPlan, traversalPose, tacticalThrowVelocity, LADDER_CLIMB_SPEED, ladderById, ladderClimbPoint, ladderBottomExitPoint, ladderTopExitPoint, findLadderEntry, ladderClimbStep } from './movement-model.js?v=1.41.1';
import { SHELL_PANEL, createSessionShell, detectInputPlatform } from './app-lifecycle.js?v=1.41.1';
import { GAMEPAD_BUTTON, createGamepadInput } from './gamepad-input.js?v=1.41.1';
...
const IRON_SIGHT_WEAPONS = new Set(['pistol','assault','ump','machineGun','shotgun','semiShotgun']);
...
const NET_DIAG_ENABLED = new URL(location.href).searchParams.get('netdiag')==='1';
...
const SOUND_CUES = {
  ...
  shotRpg:{url:`audio/shot-rpg.wav?rev=${AUDIO_ASSET_REV}`,group:'Gunfire',gain:.96},
  ...
  hitmarker:{url:`audio/hitmarker.wav?rev=${AUDIO_ASSET_REV}`,group:'Feedback',gain:.50},
  ...
  glExplosion:{url:`audio/gl-explosion.wav?rev=${AUDIO_ASSET_REV}`,group:'Explosions',gain:.94},
...
function renderLoadoutChoiceGrids(){
  ...
  for(const grid of document.querySelectorAll('[data-loadout-grid]')){
    ...
    for(const [id,label] of items){
      const button=document.createElement('button');button.className='loadout-choice';button.type='button';button.setAttribute(`data-${surface}-${slot}-choice`,id);
...
const loadoutFocusSlot={lobby:'primary',match:'primary'},loadoutWorkspaceMode={lobby:'overview',match:'overview'},loadoutPreviewDesired=new Map(),loadoutPreviewContexts=new Map(),loadoutAttachmentSlot={'lobby-primary':'optic','lobby-secondary':'optic','match-primary':'optic','match-secondary':'optic'},loadoutAttachmentTrayOpen=new Set(),loadoutPreviewAdsKeys=new Set();
...
function loadoutWeaponStats(weapon,attachments,comparisonAttachments={}){
  ...
  return[
    ...
    {label:'HANDLING',value:`${Math.round(spec.adsInMs||0)}MS`,score:loadoutHandlingScore(spec),baseScore:loadoutHandlingScore(compare)},
...
function activeLoadoutAttachmentSlot(surface,weaponSlot,weapon){const key=`${surface}-${weaponSlot}`,available=ATTACHMENT_SLOTS.filter(slot=>attachmentOptionsForWeapon(weapon,slot).length);if(!available.length)return'';let selected=loadoutAttachmentSlot[key];if(!available.includes(selected))selected=available[0];loadoutAttachmentSlot[key]=selected;return selected;}
function loadoutComparisonAttachments(surface,weaponSlot,weapon,attachments){const key=`${surface}-${weaponSlot}`;if(!loadoutAttachmentTrayOpen.has(key))return normalizeWeaponAttachments(weapon,{});const activeSlot=activeLoadoutAttachmentSlot(surface,weaponSlot,weapon),compare={...normalizeWeaponAttachments(weapon,attachments)};if(activeSlot)compare[activeSlot]='';return compare;}
function renderLoadoutStats(surface,slot,weapon,attachments){
  ...
  for(const row of loadoutWeaponStats(weapon,attachments,comparison)){const delta=row.score-row.baseScore,card=document.createElement('div');card.className='loadout-stat';if(delta>1.2)card.classList.add('better');else if(delta<-1.2)card.classList.add('worse');const head=document.createElement('div');head.className='loadout-stat-head';const label=document.createElement('span');label.textContent=row.label;const value=document.createElement('strong');value.textContent=row.value;head.append(label,value);const change=document.createElement('div');change.className='loadout-stat-delta';change.textContent=Math.abs(delta)>1.2?`${delta>0?'▲':'▼'} ${Math.abs(Math.round(delta))}`:'SAME';const bar=document.createElement('div');bar.className='loadout-stat-bar';const fill=document.createElement('i');fill.style.width=`${Math.max(3,row.score)}%`;bar.append(fill);card.append(head,change,bar);host.append(card);}
}
...
function attachmentEffectTags(item,weapon){if(!item)return[];const tags=[],mods=item.mods||{},push=x=>{if(x&&tags.length<5)tags.push(x);};push(modifierTag('VELOCITY',mods.bulletSpeed));push(modifierTag('RANGE',mods.falloffEnd));const recoilVals=[mods.recoilPitch,mods.recoilYaw].map(Number).filter(Number.isFinite);if(recoilVals.length){const avg=recoilVals.reduce((a,b)=>a+b,0)/recoilVals.length;push(modifierTag('RECOIL',avg,{lowerBetter:true}));}push(modifierTag('ADS TIME',mods.adsInMs,{lowerBetter:true}));push(modifierTag('SPRINT-OUT',mods.sprintOutMs,{lowerBetter:true}));push(modifierTag('RELOAD',mods.reloadMs,{lowerBetter:true}));if(item.adsMoveAdd)push({text:`ADS MOVE ${item.adsMoveAdd>0?'+':''}${Math.round(item.adsMoveAdd*100)}%`,good:item.adsMoveAdd>0});if(item.magAdd?.[weapon])push({text:`MAG +${item.magAdd[weapon]}`,good:true});if(item.accuracyMods){const vals=Object.values(item.accuracyMods).map(Number).filter(Number.isFinite);if(vals.length){const avg=vals.reduce((a,b)=>a+b,0)/vals.length;push(modifierTag('SPREAD',avg,{lowerBetter:true}));}}if(item.soundScale&&item.soundScale<1)push({text:'QUIETER / OFF RADAR',good:true});if(item.conditionalRecoilScale)push({text:`CROUCHED RECOIL -${Math.round((1-item.conditionalRecoilScale)*100)}%`,good:true});if(item.adsFov)push({text:`OPTIC ${Math.round(item.adsFov)} FOV`,good:true});return tags;}
...
function syncLoadoutFocusedView(surface,draft){
  const safe=normalizeLoadoutChoice(draft);for(const slot of ['primary','secondary']){const weapon=loadoutSlotWeapon(safe,slot),attachments=loadoutSlotAttachments(safe,slot),key=`${surface}-${slot}`,spec=resolveWeaponSpec(weapon,attachments),mods=ATTACHMENT_SLOTS.reduce((n,name)=>n+(attachments?.[name]?1:0),0),nameEl=document.querySelector(`[data-loadout-weapon-name="${key}"]`),roleEl=document.querySelector(`[data-loadout-weapon-role="${key}"]`),ammoEl=document.querySelector(`[data-loadout-weapon-ammo="${key}"]`),summary=document.querySelector(`[data-loadout-tab-summary="${key}"]`);if(nameEl)nameEl.textContent=spec.name||weapon;if(roleEl)roleEl.textContent=LOADOUT_WEAPON_ROLES[weapon]||'Weapon';if(ammoEl)ammoEl.textContent=`${spec.mag} ROUNDS`;if(summary)summary.textContent=`${spec.short||spec.name}${mods?` · ${mods} MOD${mods===1?'':'S'}`:''}`;renderLoadoutStats(surface,slot,weapon,attachments);loadoutPreviewDesired.set(key,{weapon,attachments:{...attachments},signature:`${weapon}:${JSON.stringify(attachments||{})}`});}
  renderLoadoutOverview(surface,safe);switchLoadoutWeaponTab(surface,loadoutFocusSlot[surface]||'primary',{ensurePreview:false});
}
function switchLoadoutWeaponTab(surface,slot='primary',{ensurePreview=true}={}){const safe=slot==='secondary'?'secondary':'primary';loadoutFocusSlot[surface]=safe;for(const tab of document.querySelectorAll(`[data-loadout-weapon-tab^="${surface}-"]`)){const active=tab.dataset.loadoutWeaponTab===`${surface}-${safe}`;tab.classList.toggle('active',active);tab.setAttribute('aria-selected',String(active));}for(const page of document.querySelectorAll(`[data-loadout-weapon-page^="${surface}-"]`)){const active=page.dataset.loadoutWeaponPage===`${surface}-${safe}`;page.classList.toggle('active',active);page.hidden=!active;page.inert=!active;}if(ensurePreview&&loadoutWorkspaceMode[surface]==='gunsmith'){const key=`${surface}-${safe}`;requestAnimationFrame(()=>{if(loadoutPreviewDesired.has(key))void ensureLoadoutPreviewEngine();});}}
function setLoadoutWorkspaceMode(surface,mode='overview',{slot=loadoutFocusSlot[surface]||'primary',ensurePreview=true}={}){const next=mode==='gunsmith'?'gunsmith':'overview';loadoutWorkspaceMode[surface]=next;const workspace=document.querySelector(`[data-loadout-workspace="${surface}"]`),overview=document.querySelector(`[data-loadout-overview="${surface}"]`),gunsmith=document.querySelector(`[data-loadout-gunsmith="${surface}"]`);if(workspace)workspace.dataset.view=next;if(overview){overview.hidden=next!=='overview';overview.inert=next!=='overview';overview.classList.toggle('active',next==='overview');}if(gunsmith){gunsmith.hidden=next!=='gunsmith';gunsmith.inert=next!=='gunsmith';gunsmith.classList.toggle('active',next==='gunsmith');}if(next==='gunsmith'){switchLoadoutWeaponTab(surface,slot,{ensurePreview});}else{for(const key of [...loadoutAttachmentTrayOpen])if(key.startsWith(`${surface}-`))loadoutAttachmentTrayOpen.delete(key);for(const key of [...loadoutPreviewAdsKeys])if(key.startsWith(`${surface}-`))setLoadoutAdsPreview(key,false);renderAttachmentEditors(surface,loadoutDraftForSurface(surface));}}
...
function bindLoadoutWorkspaceTabs(){
  for(const tab of document.querySelectorAll('[data-loadout-weapon-tab]'))tab.addEventListener('click',()=>{const [surface,slot]=String(tab.dataset.loadoutWeaponTab||'').split('-');setLoadoutWorkspaceMode(surface,'gunsmith',{slot});});
  for(const button of document.querySelectorAll('[data-loadout-open-gunsmith]'))button.addEventListener('click',()=>{const [surface,slot]=String(button.dataset.loadoutOpenGunsmith||'').split('-');loadoutAttachmentTrayOpen.delete(`${surface}-${slot}`);setLoadoutWorkspaceMode(surface,'gunsmith',{slot});renderAttachmentEditor(surface,slot,loadoutDraftForSurface(surface));});
  for(const button of document.querySelectorAll('[data-loadout-back-overview]'))button.addEventListener('click',()=>setLoadoutWorkspaceMode(button.dataset.loadoutBackOverview,'overview'));
...
function applyLoadoutAttachmentChoice(surface,weaponSlot,slot,id){const live=loadoutDraftForSurface(surface),key=weaponSlot==='primary'?'primaryAttachments':'secondaryAttachments',next={...loadoutSlotAttachments(live,weaponSlot),[slot]:id};if(surface==='lobby')setLobbyLoadoutDraft({[key]:next});else{loadoutDraft=normalizeLoadoutChoice({...live,[key]:next});syncMatchLoadoutEditor();}}
function renderAttachmentEditor(surface,weaponSlot,draft){
  const host=document.querySelector(`[data-attachment-editor="${surface}-${weaponSlot}"]`);if(!host)return;const key=`${surface}-${weaponSlot}`,weapon=loadoutSlotWeapon(draft,weaponSlot),current=loadoutSlotAttachments(draft,weaponSlot),groups=ATTACHMENT_SLOTS.map(slot=>[slot,attachmentOptionsForWeapon(weapon,slot)]).filter(([,items])=>items.length);host.replaceChildren();host.classList.toggle('empty',groups.length===0);if(!groups.length){loadoutAttachmentTrayOpen.delete(key);return;}
  const activeSlot=activeLoadoutAttachmentSlot(surface,weaponSlot,weapon),activeItems=groups.find(([slot])=>slot===activeSlot)?.[1]||[],equipped=ATTACHMENT_SLOTS.reduce((n,slot)=>n+(current?.[slot]?1:0),0),trayOpen=loadoutAttachmentTrayOpen.has(key),head=document.createElement('div');head.className='gunsmith-attachment-head';const title=document.createElement('strong');title.textContent='ATTACHMENTS';const count=document.createElement('small');count.textContent=trayOpen?`${activeSlot.toUpperCase()} · TAP SLOT TO COLLAPSE`:`${equipped} / ${groups.length} EQUIPPED`;head.append(title,count);
  const slotGrid=document.createElement('div');slotGrid.className='gunsmith-slot-grid';for(const [slot] of groups){const id=current?.[slot]||'',item=id?ATTACHMENTS[id]:null,b=document.createElement('button');b.type='button';b.className='gunsmith-slot-button';b.classList.toggle('active',trayOpen&&slot===activeSlot);b.classList.toggle('equipped',!!item);const label=document.createElement('span');label.textContent=slot.toUpperCase();const value=document.createElement('strong');value.textContent=item?.short||item?.name||'NONE';b.append(label,value);b.addEventListener('click',()=>{if(loadoutAttachmentTrayOpen.has(key)&&loadoutAttachmentSlot[key]===slot)loadoutAttachmentTrayOpen.delete(key);else{loadoutAttachmentSlot[key]=slot;loadoutAttachmentTrayOpen.add(key);}renderAttachmentEditor(surface,weaponSlot,loadoutDraftForSurface(surface));syncLoadoutFocusedView(surface,loadoutDraftForSurface(surface));});slotGrid.append(b);}host.append(head,slotGrid);
  if(!trayOpen)return;
  const tray=document.createElement('div');tray.className='gunsmith-attachment-tray',options=document.createElement('div');options.className='gunsmith-attachment-options';for(const item of [null,...activeItems]){const id=item?.id||'',b=document.createElement('button');b.type='button';b.className='gunsmith-attachment-option';b.classList.toggle('active',(current?.[activeSlot]||'')===id);const name=document.createElement('strong');name.textContent=item?.name||'NONE';const detail=document.createElement('small');const tags=attachmentEffectTags(item,weapon);detail.textContent=item?(tags.map(x=>x.text).join(' · ')||item.description):`BASE ${activeSlot.toUpperCase()}`;b.append(name,detail);b.addEventListener('click',()=>applyLoadoutAttachmentChoice(surface,weaponSlot,activeSlot,id));options.append(b);}
  const selectedId=current?.[activeSlot]||'',selectedItem=selectedId?ATTACHMENTS[selectedId]:null,detailBox=document.createElement('div');detailBox.className='gunsmith-attachment-detail';const detailHead=document.createElement('div');detailHead.className='gunsmith-detail-head';const detailTitle=document.createElement('strong');detailTitle.textContent=selectedItem?.name||`BASE ${activeSlot.toUpperCase()}`;const close=document.createElement('button');close.type='button';close.className='gunsmith-tray-close';close.textContent='×';close.setAttribute('aria-label','Close attachment tray');close.addEventListener('click',()=>{loadoutAttachmentTrayOpen.delete(key);renderAttachmentEditor(surface,weaponSlot,loadoutDraftForSurface(surface));syncLoadoutFocusedView(surface,loadoutDraftForSurface(surface));});detailHead.append(detailTitle,close);const desc=document.createElement('span');desc.textContent=selectedItem?.description||`No ${activeSlot} attachment equipped.`;const tagWrap=document.createElement('div');tagWrap.className='gunsmith-effect-tags';for(const tag of attachmentEffectTags(selectedItem,weapon)){const chip=document.createElement('i');chip.className=`gunsmith-effect-tag ${tag.good?'good':'bad'}`;chip.textContent=tag.text;tagWrap.append(chip);}detailBox.append(detailHead,desc,tagWrap);tray.append(options,detailBox);host.append(tray);
}
function renderAttachmentEditors(surface,draft){renderAttachmentEditor(surface,'primary',draft);renderAttachmentEditor(surface,'secondary',draft);syncLoadoutFocusedView(surface,draft);}
bindLoadoutWorkspaceTabs();
function hydrateWeaponTuneSelector(){
  const control=$('adminWeaponSelect');if(!control)return;
  const options=WEAPON_ORDER.map(id=>({value:id,label:WEAPON_SPECS[id]?.name||id}));
  control.dataset.options=JSON.stringify(options);
  if(!WEAPON_ORDER.includes(control.dataset.value))control.dataset.value=WEAPON_ORDER[0];
}
...
let adsWanted=false,adsBlend=0,baseFov=70,sniperZoomLevel=0,lastWastedBy='',lastWastedWeapon='',deathViewStartYaw=0,deathViewTargetYaw=NaN,deathViewStartPitch=0;
...
function netDiagEvent(type,data={}){netDiag.events.push({at:Math.round(performance.now()-netDiag.startedAt),type,...data});if(netDiag.events.length>24)netDiag.events.shift();}
...
let lobbyLoadoutDraft=null,lobbyClassDrafts=null,lobbyLoadoutDirty=false,lobbyLoadoutRevision=0,lobbyLoadoutAckRevision=0;const loadoutEditClass={lobby:activeClassId,match:activeClassId};
...
const ENGINE_MODULE_URL = './vendor/three.module.min.js?v=1.41.1';
...
function renderLoadoutClassStrip(surface){const host=document.querySelector(`[data-loadout-class-strip="${surface}"]`);if(!host)return;const classes=surface==='lobby'?normalizeLoadoutClasses(lobbyClassDrafts||loadoutClasses,selectedLoadout()):normalizeLoadoutClasses(matchClassDrafts||loadoutClasses,selectedLoadout()),baseline=surface==='lobby'?normalizeLoadoutClasses(loadoutClasses,selectedLoadout()):normalizeLoadoutClasses(matchClassBase||loadoutClasses,selectedLoadout()),selected=normalizeLoadoutClassId(loadoutEditClass[surface]||activeClassId);host.replaceChildren();for(const item of classes){const base=loadoutClassById(baseline,item.id,selectedLoadout()),dirty=!loadoutChoiceEqual(item,base),b=document.createElement('button');b.type='button';b.className='loadout-class-btn';b.classList.toggle('active',item.id===selected);b.classList.toggle('equipped',item.id===activeClassId);b.classList.toggle('pending',item.id===pendingClassId&&!!pendingLoadout);b.classList.toggle('dirty',dirty);b.setAttribute('role','tab');b.setAttribute('aria-selected',String(item.id===selected));b.dataset.loadoutClass=item.id;const strong=document.createElement('strong');strong.textContent=item.name;const small=document.createElement('small');const classState=item.id===pendingClassId&&pendingLoadout?'NEXT':item.id===activeClassId?'ACTIVE':dirty?'EDITED':'';small.textContent=`${classState?classState+' · ':''}${WEAPON_SPECS[item.primaryWeapon]?.short||'AR'} + ${WEAPON_SPECS[item.secondaryWeapon]?.short||'GLK'}`;b.append(strong,small);b.addEventListener('click',()=>switchLoadoutClass(surface,item.id));host.append(b);}}
function switchLoadoutClass(surface,id){const nextId=normalizeLoadoutClassId(id);setLoadoutWorkspaceMode(surface,'overview',{ensurePreview:false});if(surface==='lobby'){if(lobbyLoadoutDraft)lobbyClassDrafts=writeClassLoadout(lobbyClassDrafts||loadoutClasses,loadoutEditClass.lobby,lobbyLoadoutDraft);loadoutEditClass.lobby=nextId;lobbyLoadoutDraft=classLoadout(lobbyClassDrafts||loadoutClasses,nextId);lobbyLoadoutDirty=!classSetEqual(lobbyClassDrafts||loadoutClasses,loadoutClasses)||nextId!==activeClassId;renderLobbySetupControls();setLobbyActionState();renderLoadoutClassStrip('lobby');if(!isMatchAdmin&&socket?.readyState===WebSocket.OPEN){const rev=++lobbyLoadoutRevision;send({t:'loadout',rev,classId:nextId,loadoutClasses:lobbyClassDrafts,...lobbyLoadoutDraft});}}else{if(loadoutDraft)matchClassDrafts=writeClassLoadout(matchClassDrafts||loadoutClasses,loadoutEditClass.match,loadoutDraft);loadoutEditClass.match=nextId;loadoutBaseDraft=classLoadout(matchClassBase||loadoutClasses,nextId);loadoutDraft=classLoadout(matchClassDrafts||loadoutClasses,nextId);syncMatchLoadoutEditor();renderLoadoutClassStrip('match');}}
...
function lobbySnapshot(){
  const selfLoadout=shell.inLobby&&lobbyLoadoutDraft?normalizeLoadoutChoice(lobbyLoadoutDraft):selectedLoadout(),rows=[{id:clientId,name:myName||safeName(),team:myTeam,bot:false,godMode,admin:isMatchAdmin,...selfLoadout,kills:myStats.kills||0,deaths:myStats.deaths||0,self:true}];
...
function refreshLobbyDraftOwnership(){
  ...
  if(!lobbyMatchDraft||!lobbyMatchDirty)lobbyMatchDraft={...committedMatch};
  const committedMap=normalizeMapId(currentMapId);if(!lobbyMapDraft||!lobbyMapDirty)lobbyMapDraft=committedMap;
  const committedLoadout=normalizeLoadoutChoice(selectedLoadout());if(!lobbyClassDrafts||!lobbyLoadoutDirty){lobbyClassDrafts=normalizeLoadoutClasses(loadoutClasses,committedLoadout);loadoutEditClass.lobby=normalizeLoadoutClassId(activeClassId);lobbyLoadoutDraft=classLoadout(lobbyClassDrafts,loadoutEditClass.lobby);}renderAttachmentEditors('lobby',lobbyLoadoutDraft);renderLoadoutClassStrip('lobby');
}
...
function lobbyHasDraftChanges(){return lobbyMatchDirty||lobbyMapDirty||lobbyLoadoutDirty||lobbyGameplayIsDirty()||lobbyWeaponsIsDirty();}
...
function collectLobbyStartSetup(){
  ...
  return{
    ...
    loadout:normalizeLoadoutChoice(lobbyLoadoutDraft||selectedLoadout()),loadoutClasses:normalizeLoadoutClasses(lobbyClassDrafts||loadoutClasses,selectedLoadout()),classId:normalizeLoadoutClassId(loadoutEditClass.lobby||activeClassId)
  };
}
function renderLobbySetupControls(){
  if(!shell.inLobby)return;refreshLobbyDraftOwnership();const host=!!isMatchAdmin,draft=lobbyMatchDraft||committedLobbyMatchDraft(),draftSpec=gameModeSpec(draft.mode),draftLoadout=lobbyLoadoutDraft||normalizeLoadoutChoice(selectedLoadout());renderLoadoutClassStrip('lobby');
  ...
  if(host){
    ...
    const mapChoice=lobbyMapDraft||currentMapId;for(const btn of lobbyMapButtons){btn.classList.toggle('active',btn.dataset.lobbyMapChoice===mapChoice);btn.disabled=!matchAllowsLobbyEdits(matchState);}renderLobbyMapPreview(mapChoice);
...
function syncLobby({setup=true,roster=true}={}){if(!shell.inLobby)return;renderLobbyShell();if(setup)renderLobbySetupControls();setLobbyActionState();if(roster)renderLobbyRoster(lobbyDisplayMode());}
...
function setLobbyLoadoutDraft(next={}){
  if(!lobbyClassDrafts)lobbyClassDrafts=normalizeLoadoutClasses(loadoutClasses,selectedLoadout());if(!lobbyLoadoutDraft)lobbyLoadoutDraft=classLoadout(lobbyClassDrafts,loadoutEditClass.lobby||activeClassId);lobbyLoadoutDraft=normalizeLoadoutChoice({...lobbyLoadoutDraft,...next});lobbyClassDrafts=writeClassLoadout(lobbyClassDrafts,loadoutEditClass.lobby,lobbyLoadoutDraft);lobbyLoadoutDirty=!classSetEqual(lobbyClassDrafts,loadoutClasses)||normalizeLoadoutClassId(loadoutEditClass.lobby)!==normalizeLoadoutClassId(activeClassId);renderLobbySetupControls();setLobbyActionState();
  if(!isMatchAdmin&&socket?.readyState===WebSocket.OPEN){const rev=++lobbyLoadoutRevision;send({t:'loadout',rev,classId:normalizeLoadoutClassId(loadoutEditClass.lobby),loadoutClasses:lobbyClassDrafts,...normalizeLoadoutChoice(lobbyLoadoutDraft)});}
}
...
function switchLobbySide(name='players'){
  ...
  if(next==='map')requestAnimationFrame(renderLobbyMapPreview);if(next==='loadout'){setLoadoutWorkspaceMode('lobby','overview',{ensurePreview:false});renderAttachmentEditors('lobby',lobbyLoadoutDraft||normalizeLoadoutChoice(selectedLoadout()));}
}
...
function syncMatchLoadoutEditor(){
  ...
  if(matchClassDrafts)matchClassDrafts=writeClassLoadout(matchClassDrafts,loadoutEditClass.match,draft);renderLoadoutClassStrip('match');const targetClass=normalizeLoadoutClassId(loadoutEditClass.match),savedClass=normalizeLoadoutClassId(pendingClassId||activeClassId),dirty=!classSetEqual(matchClassDrafts||loadoutClasses,matchClassBase||loadoutClasses)||targetClass!==savedClass,status=$('loadoutStatus'),description=$('loadoutDescription');if(description)description.textContent=godMode?'God Mode is active. Save equips the new loadout immediately.':'Changes are staged until Save. Your current life stays unchanged.';if(status)status.textContent=dirty?'Unsaved changes':godMode?'Saved loadout · changes equip immediately.':(pendingLoadout?'Queued loadout is saved for your next spawn.':'Saved loadout · equips on your next spawn.');if($('loadoutResetBtn'))$('loadoutResetBtn').disabled=!dirty;if($('loadoutSaveBtn'))$('loadoutSaveBtn').disabled=!dirty;
}
function openMatchLoadout(){if(!shell.inMatch)return;matchClassBase=normalizeLoadoutClasses(loadoutClasses,selectedLoadout());matchClassDrafts=normalizeLoadoutClasses(matchClassBase,selectedLoadout());loadoutEditClass.match=normalizeLoadoutClassId(pendingClassId||activeClassId);loadoutBaseDraft=classLoadout(matchClassBase,loadoutEditClass.match);loadoutDraft=classLoadout(matchClassDrafts,loadoutEditClass.match);syncMatchLoadoutEditor();shell.openPanel(SHELL_PANEL.LOADOUT);setLoadoutWorkspaceMode('match','overview',{ensurePreview:false});}
function closeMatchLoadout(){setLoadoutWorkspaceMode('match','overview',{ensurePreview:false});loadoutDraft=null;loadoutBaseDraft=null;matchClassDrafts=null;matchClassBase=null;shell.closePanel(SHELL_PANEL.LOADOUT);}
function resetMatchLoadout(){if(!matchClassBase)return;matchClassDrafts=normalizeLoadoutClasses(matchClassBase,selectedLoadout());loadoutEditClass.match=normalizeLoadoutClassId(pendingClassId||activeClassId);loadoutBaseDraft=classLoadout(matchClassBase,loadoutEditClass.match);loadoutDraft=classLoadout(matchClassDrafts,loadoutEditClass.match);syncMatchLoadoutEditor();}
function saveMatchLoadout(){
  if(socket?.readyState!==WebSocket.OPEN||!loadoutDraft)return;matchClassDrafts=writeClassLoadout(matchClassDrafts||loadoutClasses,loadoutEditClass.match,loadoutDraft);const classId=normalizeLoadoutClassId(loadoutEditClass.match),savedClass=normalizeLoadoutClassId(pendingClassId||activeClassId);if(classSetEqual(matchClassDrafts,matchClassBase||loadoutClasses)&&classId===savedClass){closeMatchLoadout();return;}
  const next=classLoadout(matchClassDrafts,classId);send({t:'loadout',classId,loadoutClasses:matchClassDrafts,...next});loadoutClasses=normalizeLoadoutClasses(matchClassDrafts,selectedLoadout());rememberLoadoutClasses(loadoutClasses,classId);if(godMode){pendingClassId='';pendingLoadout=null;showToast(`${loadoutClassById(loadoutClasses,classId).name} APPLYING`);}else{pendingClassId=classId;pendingLoadout=next;showToast(hp<=0?`${loadoutClassById(loadoutClasses,classId).name} SAVED FOR RESPAWN`:`${loadoutClassById(loadoutClasses,classId).name} · NEXT SPAWN`);}syncPauseContext();closeMatchLoadout();
}
...
function setupWeaponAttachmentVisuals(group,weapon,opts={}){
  ...
  if(opts.verticalGrip)registerAttachmentVisual(group,'verticalGrip',makeVerticalGrip(opts.verticalGrip));if(opts.angledGrip)registerAttachmentVisual(group,'angledGrip',makeAngledGrip(opts.angledGrip));if(opts.bipod&&!group.userData.attachmentBipodBaseParts.length)registerAttachmentVisual(group,'bipod',makeBipodAttachment(opts.bipod));
...
function buildLoadoutPreviewModel(weapon,attachments){
  const root=new THREE.Group(),sources=weapon==='akimbo1887'?[akimboLeftGroup,akimboRightGroup]:[activeFirstPersonWeaponGroup(weapon)].filter(Boolean);if(!sources.length)return root;let sightPoint=null;
  const changed=[];for(const source of sources){if(source?.userData?.attachmentWeapon){syncWeaponAttachmentVisuals(source,weapon,attachments);changed.push(source);}if(!sightPoint&&sources.length===1)sightPoint=loadoutPreviewSightPoint(source,weapon,attachments);const copy=new THREE.Group();for(const child of source?.children||[]){const node=cloneLoadoutPreviewNode(child);if(node)copy.add(node);}root.add(copy);}
  ...
  if(weapon==='akimbo1887'&&root.children.length>=2){root.children[0].position.x=-.19;root.children[0].rotation.y=.08;root.children[1].position.x=.19;root.children[1].rotation.y=-.08;}
  const box=new THREE.Box3().setFromObject(root),size=box.getSize(new THREE.Vector3()),center=box.getCenter(new THREE.Vector3()),maxDim=Math.max(.01,size.x,size.y,size.z),scale=2.30/maxDim;root.position.sub(center);const wrapper=new THREE.Group();wrapper.add(root);wrapper.scale.setScalar(scale);if(sightPoint)wrapper.userData.adsPoint=sightPoint.sub(center).multiplyScalar(scale);return wrapper;
}
...
function init3D(){
  ...
  const scopeMountFront=new THREE.Mesh(new THREE.BoxGeometry(.065,.055,.032),rifleMat);scopeMountFront.position.set(0,.098,-.27);
  ...
  sniperGroup.add(dragReceiver,dragHandguard,rifleBarrel,stock,stockComb,sniperMag,scopeMountRear,scopeMountFront,scope,scopeInner,scopeShade,scopeFrontRing,scopeRearRing,scopeFrontLens,scopeRearLens,sniperBolt,sniperFlash);sniperGroup.userData.adsOpticPose={x:0,y:-.14,z:-.39,rx:0,ry:0,rz:0};sniperGroup.userData.cyclePart=sniperBolt;sniperGroup.userData.cycleBaseZ=-.06;sniperGroup.userData.cycleTravel=.085;registerWeaponHandAnchors(sniperGroup,{right:{position:[.030,-.120,.035],rotation:[-.25,-.03,.05]},left:{position:[-.030,-.045,-.45],rotation:[-.14,.04,.02]},reloadLeft:{position:[-.035,-.125,-.08],rotation:[-.16,.08,.14]},reloadRight:{parent:sniperBolt,position:[.025,.015,.00],rotation:[-.44,-.12,.25]}});sniperGroup.position.set(.28,-.28,-.48);sniperGroup.rotation.set(-.055,-.05,0);sniperGroup.visible=false;
...
function bindUI(){
  ...
  for(const btn of matchSecondaryButtons)btn.addEventListener('click',()=>{loadoutDraft=normalizeLoadoutChoice({...loadoutDraft,secondaryWeapon:btn.dataset.matchSecondaryChoice});syncMatchLoadoutEditor();});
  ...
  $('lobbyStartBtn').addEventListener('click',async()=>{
    ...
    loadoutClasses=normalizeLoadoutClasses(setup.loadoutClasses,selectedLoadout());rememberLoadoutClasses(loadoutClasses,activeClassId);send({t:'startMatch',setup});$('lobbyStatus').textContent='Starting match…';
  });
  ...
  document.addEventListener('keydown', e => {
    ...
    if(e.code==='Escape'&&!e.repeat&&shell.panel){
      ...
      if(shell.panel===SHELL_PANEL.LOADOUT){if(loadoutWorkspaceMode.match==='gunsmith'){setLoadoutWorkspaceMode('match','overview');return;}closeMatchLoadout();return;}
    }
    if(e.code==='Escape'&&!e.repeat&&shell.inLobby&&loadoutWorkspaceMode.lobby==='gunsmith'&&document.querySelector('[data-lobby-side-tab="loadout"]')?.classList.contains('active')){e.preventDefault();setLoadoutWorkspaceMode('lobby','overview');return;}
...
function applyMatchPhaseTransition(previous,next){
  ...
  if(next.status===MATCH_STATUS.ENDED){
    reloadUntil=0;reloadWeapon='';reloadStartedAt=0;pendingWeapon='';weaponSwapStartedAt=0;localEquipmentCooldownUntil=0;sprintActionReadyAt=0;clearQueuedSprintShot();resetViewRecoil();
  }
...
function resetMatchPresentationForLobby(){
  ...
  killFeed.length=0;bloodSplats.length=0;damageIndicators.length=0;flashUntil=flashPeakUntil=0;hurtUntil=hitUntil=0;headshotUntil=0;killConfirmUntil=0;killConfirmHeadshot=false;killConfirmDistance=0;lastShotVisualAt=0;wastedUntil=0;lastWastedBy='';lastWastedWeapon='';deathViewStartYaw=0;deathViewTargetYaw=NaN;deathViewStartPitch=0;
...
function enterLocalDeath(m){
  ...
  wastedUntil=m.respawnAt||serverNow()+worldSettings.combat.respawnMs;deathAnimStartedAt=now;clearFireInput();cancelEquipmentAction();clearQueuedSprintShot();setAim(false);resetViewRecoil();
...
function applyServerCorrection(m){
  ...
  const correctionMagnitude=Math.hypot(deltaX,deltaY,deltaZ),recent=!seq||stateSeq-seq<=2;recordNetCorrection(m,correctionMagnitude);
...
function effectiveAimYaw(){return yaw+viewRecoilYaw;}
function effectiveAimPitch(){return THREE.MathUtils.clamp(pitch+viewRecoilPitch,-1.40,1.40);}
...
function applyAuthoritativeLoadout(m){
  ...
  if(shell.inLobby&&lobbyLoadoutDraft){if(Array.isArray(m.loadoutClasses))lobbyClassDrafts=normalizeLoadoutClasses(loadoutClasses,selectedLoadout());lobbyLoadoutDirty=!classSetEqual(lobbyClassDrafts||loadoutClasses,loadoutClasses)||normalizeLoadoutClassId(loadoutEditClass.lobby)!==normalizeLoadoutClassId(activeClassId);}
...
function updateWeaponKick(dt){
  // Recoil-control attachments affect both the authoritative aim path and the
  // weapon's visible kick. They never change the ADS sight ray itself.
...
function applyViewRecoil(weapon,preShotHeat=0,firstShot=false,hand='right'){
  ...
  const impulse=singleShotRecoilImpulse(weapon,{step,sequence:localWeaponShotSequence[weapon]||0,hand,basePitch,baseYaw,adsScale,heatScale,firstScale});
  viewRecoilTargetPitch=THREE.MathUtils.clamp(viewRecoilTargetPitch+impulse.pitch,0,hardPitch);
  viewRecoilTargetYaw=THREE.MathUtils.clamp(viewRecoilTargetYaw+impulse.yaw,-hardYaw,hardYaw);
}
...
function updateViewRecoil(dt){
  ...
  const softPitch=Math.max(0,(Number(r.recoilMaxPitch)||Math.max(0,Number(r.recoilPitch)||0))*recoilScale),softYaw=Math.max(0,(Number(r.recoilMaxYaw)||.020)*recoilScale),maxYaw=softYaw*1.12;
  ...
  if(Math.abs(viewRecoilTargetPitch)<.00001&&Math.abs(viewRecoilPitch)<.00002){viewRecoilTargetPitch=0;viewRecoilPitch=0;}
  if(Math.abs(viewRecoilTargetYaw)<.00001&&Math.abs(viewRecoilYaw)<.00002){viewRecoilTargetYaw=0;viewRecoilYaw=0;}
}
...
function requestShot(hand='right'){
  ...
  const shotYaw=round4(effectiveAimYaw()),shotPitch=round4(effectiveAimPitch()),preShotHeat=currentShotHeat(weapon,now);
  ...
  beginRecoilBurst(weapon);recoilBurstReleaseAt=0;const recoilShot=registerLocalShotHeat(weapon,now);localWeaponShotSequence[weapon]=(localWeaponShotSequence[weapon]||0)+1;lastLocalShotAt=now;applyViewRecoil(weapon,preShotHeat,recoilShot.firstShot,hand);if(!godMode&&(ammo[weapon]||0)<=0)endRecoilBurst();clearQueuedSprintShot();return true;
}
...
function doReload(){
  ...
  endRecoilBurst();setAim(false);reloadRequestPending=true;send({t:'reload',weapon:currentWeapon});if(currentWeapon!=='shotgun')soundReload(currentWeapon);
}
...
function switchWeapon(weapon){
  ...
  setAim(false);sniperZoomLevel=0;resetViewRecoil();currentWeapon=weapon;pendingWeapon=weapon;reloadRequestPending=false;reloadUntil=0;reloadWeapon='';reloadStartedAt=0;weaponSwapStartedAt=performance.now();delayFire(WEAPON_SWITCH_MS,weapon);warmWeaponAudio(weapon);syncLocalWeaponModel();send({t:'weapon',weapon});showToast(WEAPON_SPECS[weapon].name);
}
function setAim(active){
  if(currentWeapon==='akimbo1887')active=false;
  const canAim=!!active&&shell.canPlay&&matchAllowsCombat(matchState)&&hp>0&&!reloadUntil&&!traversal&&!ladderState&&combatWeaponAvailable();if(canAim&&sprinting){const delay=weaponSprintAdsMs(currentWeapon);cancelSprint({lockMs:delay+70});sendCurrentState(true);}adsWanted=canAim;if(!adsWanted){if(currentWeapon==='sniper')sniperZoomLevel=0;}else if(currentWeapon==='sniper'&&sniperZoomLevel===0)sniperZoomLevel=1;
}
...
function syncRemoteWeapon(r){if(!r)return;for(const name of WEAPON_ORDER){if(r[name]){r[name].visible=r.weapon===name;syncWeaponAttachmentVisuals(r[name],name,remoteAttachmentsForWeapon(r,name));}}}
...
function maintainNetwork(){
  ...
  if(now-lastPing<=(NET_DIAG_ENABLED?2000:15000))return;
  lastPing=now;lastPingLocalAt=Date.now();send({t:'ping',clientAt:lastPingLocalAt,diag:NET_DIAG_ENABLED?1:0});
}
...
function controllerPreferredRegion(surface){
  ...
  if(surface===$('loadoutPanel'))return surface.querySelector('[data-loadout-overview="match"]:not([hidden]),[data-loadout-gunsmith="match"]:not([hidden])')||surface;
  return surface;
}
...
const CONTROLLER_NAV_GROUP_SELECTOR='.settings-grid,.weapon-fields,.loadout-choice-grid,.attachment-editor,.gunsmith-slot-grid,.gunsmith-attachment-options,.loadout-class-strip,.loadout-weapon-tabs,.class-overview-grid,.gunsmith-nav-bar,.lobby-mode-picker,.lobby-setup-row,.lobby-team-picker,.admin-bot-controls,.pause-actions,.pause-actions-player,.pause-actions-system,.join-controls,.lobby-map-choice-grid,.lobby-cheat-tabs,.chat-key-row,.game-text-key-row';
...
function handleControllerUiNavigation(pressed){
  ...
  if(pressed[GAMEPAD_BUTTON.B]){
    ...
    if(shell.panel===SHELL_PANEL.LOADOUT){if(loadoutWorkspaceMode.match==='gunsmith'){setLoadoutWorkspaceMode('match','overview');return true;}closeMatchLoadout();return true;}
    if(shell.inLobby&&loadoutWorkspaceMode.lobby==='gunsmith'&&document.querySelector('[data-lobby-side-tab="loadout"]')?.classList.contains('active')){setLoadoutWorkspaceMode('lobby','overview');return true;}
...
function updateGamepadInput(dt){
  ...
  if(!shell.canPlay){gamepadFireDown=false;endRecoilBurst();resetControllerAimMotion();if(controllerOwnsAim){controllerOwnsAim=false;setAim(false);}handleControllerUiNavigation(pressed);return;}
...
function updateAim(dt){
  const target=adsWanted&&hp>0&&shell.canPlay?1:0,spec=effectiveWeaponSpec(currentWeapon),transitionMs=Math.max(60,Number(target?spec.adsInMs:spec.adsOutMs)||180),rate=4.605170186/(transitionMs/1000),follow=1-Math.exp(-rate*Math.max(0,Number(dt)||0));
...
function updateWeaponView(dt){
  ...
  const sniperPose=sniperGroup.userData.adsOpticPose||{x:0,y:-.14,z:-.39,rx:0,ry:0,rz:0};sniperGroup.position.set(THREE.MathUtils.lerp(.28,sniperPose.x,a)+commonX*(1-a),THREE.MathUtils.lerp(-.28,sniperPose.y,a)+commonY*(1-a),THREE.MathUtils.lerp(-.48,sniperPose.z,a)+kickZ+commonZ*(1-a));sniperGroup.rotation.set(THREE.MathUtils.lerp(-.055,sniperPose.rx,a)+reloadCurve*.10+kickPitch,THREE.MathUtils.lerp(-.05,sniperPose.ry,a)-reloadCurve*.12+kickYaw,THREE.MathUtils.lerp(0,sniperPose.rz,a)-reloadRoll*.65-swapRoll-deathRoll-mobilityRoll+kickRoll);
...
function drawHud(now){
  ...
  if(!matchAllowsMovement(matchState)){drawMatchStatus(c,w,h);drawScoreboardButton(c,L.team);drawMenuButton(c,L.menu);drawNetworkDiagnostics(c,w,h);hudTexture.needsUpdate=true;return;}
  if(hp<=0){
    ...
    c.font='800 12px system-ui';c.fillStyle='#c8b4b8';{const remain=Math.max(0,wastedUntil-serverNow());c.fillText(remain>120?`Respawning in ${Math.max(1,Math.ceil(remain/1000))}`:'Respawning…',w/2,h/2+25);}
    ...
    drawNetworkDiagnostics(c,w,h);hudTexture.needsUpdate=true;return;
  }
...
function handleKill(m){
  ...
  if(m.victim.id===clientId){lastWastedBy=m.attacker.name||'Player';lastWastedWeapon=m.weapon||'';const killer=remotes.get(m.attacker.id);if(killer){const dx=killer.group.position.x-position.x,dz=killer.group.position.z-position.z;if(Math.hypot(dx,dz)>.05)deathViewTargetYaw=Math.atan2(-dx,-dz);}}
