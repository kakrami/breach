...
import * as HighlandsGeometry from './world-geometry.js?v=1.37.2';
import * as DepotGeometry from './world-geometry-depot.js?v=1.37.2';
import * as YardGeometry from './world-geometry-yard.js?v=1.37.2';
import * as RigGeometry from './world-geometry-rig.js?v=1.37.2';
import * as HighlandsWorldCollision from './world-collision.js?v=1.37.2';
import * as DepotWorldCollision from './world-collision-depot.js?v=1.37.2';
import * as YardWorldCollision from './world-collision-yard.js?v=1.37.2';
import * as RigWorldCollision from './world-collision-rig.js?v=1.37.2';
import {
...
} from './game-config.js?v=1.37.2';
import { createProjectileCollisionGrid } from './collision-grid.js?v=1.37.2';
import { createAudioEngine } from './audio-engine.js?v=1.37.2';
import { normalizeMatchState as normalizeSharedMatchState } from './match-model.js?v=1.37.2';
import { MATCH_STATUS, matchAllowsLobbyEdits, matchAllowsMovement, matchAllowsCombat, matchPhaseChanged } from './gameplay-phase.js?v=1.37.2';
import { MAX_PLAYER_PHYSICS_STEP_SEC, advanceVerticalMotion, advanceKnockback, sweepHorizontalMovement, createTraversalPlan, traversalPose, tacticalThrowVelocity } from './movement-model.js?v=1.37.2';
import { SHELL_PANEL, createSessionShell, detectInputPlatform } from './app-lifecycle.js?v=1.37.2';
import { GAMEPAD_BUTTON, createGamepadInput } from './gamepad-input.js?v=1.37.2';
...
let onGround = true, lastShotVisualAt = 0, lastLocalShotAt = 0, fireReadyAt = freshClientFireReady(), lastStateSent = 0, lastPing = 0, lastPingLocalAt = 0, serverClockOffset = 0;
...
function setActiveInputMode(mode,{quiet=false}={}){
  ...
  if(previous===INPUT_MODE.CONTROLLER){clearControllerGameplayInput();clearControllerUiFocus();}
...
const killFeed = [];
const chatMessages=[];
let chatOpen=false,chatDraft='',chatShift=false;
...
const ENGINE_MODULE_URL = './vendor/three.module.min.js?v=1.37.2';
...
function syncLobby(){
  ...
  $('lobbyTeamGroup').classList.toggle('hide',!spec.teamBased);$('lobbyHostSetup').classList.toggle('hide',!host);$('lobbyGuestSetup').classList.toggle('hide',host);$('lobbyHostControlBtn')?.classList.toggle('hide',!host);$('lobbySetupOwnerLabel').textContent=host?'Admin controls':'Match info';
  ...
  lobbyBlueBotCount.value=String(botConfig.blueBots||0);lobbyRedBotCount.value=String(botConfig.redBots||0);lobbyFfaBotCount.value=String(total);lobbyBotDifficulty.value=botConfig.difficulty||'normal';lobbyMinimapMode.value=matchState.minimapRevealAll?'all':'standard';for(const btn of lobbyMapButtons){btn.classList.toggle('active',btn.dataset.lobbyMapChoice===currentMapId);btn.disabled=!host||!matchAllowsLobbyEdits(matchState);}const mapInfo=mapSpec(currentMapId);if($('lobbyMapName'))$('lobbyMapName').textContent=mapInfo.name;if($('lobbyMapDescription'))$('lobbyMapDescription').textContent=host?'Select a level. The full layout is shown at right.':'Selected by the lobby admin.';renderLobbyMapPreview();
  ...
  const ruleSummary=spec.scoreType==='none'?'No score / time limit':`First ${matchState.scoreLimit||spec.scoreLimit} · ${Math.max(2,Math.round((matchState.timeLimitMs||spec.timeLimitMs)/60000))} min`,minimapSummary=matchState.minimapRevealAll?'Minimap: Everyone':'Minimap: Standard',mapName=mapSpec(currentMapId).name;$('lobbyRuleSummary').textContent=`${mapName} · ${ruleSummary} · ${minimapSummary}${matchCustom?' · CUSTOM':''}`;$('lobbyGuestMode').textContent=`${spec.name} · ${mapName}`;$('lobbyGuestBots').textContent=`${total} bot${total===1?'':'s'} · ${(botConfig.difficulty||'normal').replace(/^./,c=>c.toUpperCase())}`;$('lobbyGuestRules').textContent=`${ruleSummary} · ${minimapSummary}${matchCustom?' · Custom rules':''}`;
...
function sendLobbyMatchRules(){
  if(!isMatchAdmin||socket?.readyState!==WebSocket.OPEN||!matchAllowsLobbyEdits(matchState))return;
  const spec=currentModeSpec();if(spec.scoreType==='none'){syncLobby();return;}
  const scoreLimit=Math.max(5,Math.min(100,Math.round(Number(lobbyScoreLimit.value)||spec.scoreLimit)));
  const minutes=Math.max(2,Math.min(30,Math.round(Number(lobbyTimeLimit.value)||spec.timeLimitMs/60000)));
  lobbyScoreLimit.value=String(scoreLimit);lobbyTimeLimit.value=String(minutes);send({t:'adminMatch',rules:{scoreLimit,timeLimitMs:minutes*60000}});
}
...
function switchLobbySide(name='players'){
  const next=['players','match','map','loadout'].includes(name)?name:'players';
...
function syncPlayerSettingsUI(){
  ...
  if($('playerGraphics'))$('playerGraphics').value=playerSettings.graphics;if($('playerMinimapOrientation'))$('playerMinimapOrientation').value=playerSettings.minimapOrientation;
}
...
function openPlayerSettings(){syncPlayerSettingsUI();switchSubTabs('[data-settings-tab]','[data-settings-page]','data-settings-tab','data-settings-page','controls');shell.openPanel(SHELL_PANEL.SETTINGS);}
...
function bindUI(){
  ...
  bindSubTabs('[data-lobby-match-tab]','[data-lobby-match-page]','data-lobby-match-tab','data-lobby-match-page','rules');
  bindSubTabs('[data-lobby-loadout-tab]','[data-lobby-loadout-page]','data-lobby-loadout-tab','data-lobby-loadout-page','primary');
  bindSubTabs('[data-match-loadout-tab]','[data-match-loadout-page]','data-match-loadout-tab','data-match-loadout-page','primary');
  bindSubTabs('[data-settings-tab]','[data-settings-page]','data-settings-tab','data-settings-page','controls');
  bindSubTabs('[data-admin-gameplay-tab]','[data-admin-gameplay-page]','data-admin-gameplay-tab','data-admin-gameplay-page','movement');
  bindSubTabs('[data-admin-weapon-tab]','[data-admin-weapon-page]','data-admin-weapon-tab','data-admin-weapon-page','rifles');
  ...
  $('lobbyHostControlBtn').addEventListener('click',()=>openAdminPanel('gameplay'));
  ...
  $('playerMinimapOrientation')?.addEventListener('change',()=>{playerSettings={...playerSettings,minimapOrientation:$('playerMinimapOrientation').value==='north'?'north':'heading'};savePlayerSettings();syncPlayerSettingsUI();hudLastDraw=0;});
...
function openChat(){
  if(chatOpen||!shell.inMatch||shell.paused||shell.panel||shell.connecting)return false;
  chatOpen=true;scoreboardOpen=false;chatShift=true;setChatDraft('');suspendGameplayInput();chatComposer.classList.remove('hide');
...
async function dismissChat({restorePointer=true}={}){
  ...
  chatOpen=false;hudLastDraw=0;if(recapture&&!captured)showToast('CLICK GAME TO CAPTURE MOUSE');return true;
}
function submitChat(){
...
function drawChatFeed(c,L,w,h,now){
  const visible=chatMessages.filter(item=>chatOpen||item.until>now).slice(-(isTouch?3:5));if(!visible.length)return;
  const rowH=isTouch?17:19,boxH=visible.length*rowH+8;
  let boxW,x,y;
  if(isTouch){const rightLimit=L.map.x-8;x=L.moveBoundary+8;boxW=Math.max(150,Math.min(246,rightLimit-x));y=L.team.y+L.team.h+7;}
  else{boxW=Math.min(L.kill.w,300);x=L.kill.x;y=Math.min(h-L.safe.bottom-boxH-8,L.kill.y+L.kill.h+7);}
  c.save();roundRect(c,x,y,boxW,boxH,7,chatOpen?'rgba(5,8,10,.74)':'rgba(5,8,10,.42)',chatOpen?'rgba(255,255,255,.12)':null);let cy=y+6+rowH/2;
  for(const item of visible){const mine=samePlayerId(item.id,clientId),teamColor=currentModeSpec().teamBased?(TEAM_COLORS[item.team]||'#fff'):'#dfe8ee';c.textAlign='left';c.font=`900 ${isTouch?10:11}px system-ui`;c.fillStyle=mine?HUD_ACCENT:teamColor;const label=`${item.name}: `;c.fillText(label,x+7,cy);const lw=c.measureText(label).width;c.font=`800 ${isTouch?10:11}px system-ui`;c.fillStyle='#edf3f6';c.fillText(clipHudText(c,item.text,boxW-lw-15),x+7+lw,cy);cy+=rowH;}c.restore();
}
...
function applyMatchPhaseTransition(previous,next){
  ...
  if(!matchAllowsMovement(next)){
    traversal=null;traversalIntentUntil=0;traversalIntentSeq=0;traversalConsumedIntentSeq=0;
  ...
  if(next.status===MATCH_STATUS.ACTIVE){
    ...
    traversal=null;traversalIntentUntil=0;jumpBufferedUntil=0;moveVelocityX=moveVelocityZ=0;knockX=knockZ=0;
  }
}
...
function handleMessage(m){
  ...
  if(m.t==='map'){
    ...
    pendingGameSnapshot=self?gameSnapshot(self,players,bots):pendingGameSnapshot;replaceLobbyParticipants(players,bots);syncLobby();
  ...
  if(m.t==='bots'){botConfig=normalizeBotConfig(m.config);syncLobbyBots(m.bots||[]);if(shell.inMatch&&engineReady)syncBotRoster(m.bots||[]);syncLobby();if(shell.panel===SHELL_PANEL.ADMIN&&m.by===clientId)setAdminStatus('Bot settings updated in the lobby.','ok');showToast(`BOTS · ${botConfig.difficulty.toUpperCase()}`);return;}
...
function syncAdminContext(){
  $('adminEyebrow').textContent=shell.inLobby?'LOBBY HOST':'MATCH HOST';
  $('adminTitle').textContent='Host control';
  $('adminDescription').textContent=shell.inLobby?'Advanced tuning and player administration. Normal match setup stays in the Match tab.':'Advanced tuning and player administration.';
}
...
function openAdminPanel(initialTab='gameplay'){if(!isMatchAdmin){showToast('ADMIN ACCESS REQUIRED');return;}syncAdminContext();populateAdminSettings(worldSettings);switchAdminTab(initialTab);syncAdminWeaponEditor($('adminWeaponSelect')?.value||'assault');setAdminStatus(matchCustom?'CUSTOM tuning active.':'Advanced settings are unchanged.');shell.openPanel(SHELL_PANEL.ADMIN);renderAdminPlayers(true);}
function closeAdminPanel(){shell.closePanel(SHELL_PANEL.ADMIN);}
function resetActiveAdminTab(){if(activeAdminTab==='gameplay')populateAdminGameplay({...worldSettings,movement:DEFAULT_WORLD_SETTINGS.movement,combat:DEFAULT_WORLD_SETTINGS.combat});else if(activeAdminTab==='advanced')populateAdminWeapons({...worldSettings,weapons:DEFAULT_WORLD_SETTINGS.weapons});}
...
function applyServerCorrection(m){
  // Once a traversal has been accepted, client and server are following the
  // same deterministic plan. Late free-movement correction packets are stale
  // visual data at this point and can pull the camera/body back into the wall
  // being crossed. The authoritative traversal event/post-traversal state is
  // the correction source until the forced move finishes.
...
function toggleCrouch(){if(!shell.canPlay||!matchAllowsMovement(matchState)||hp<=0||traversal)return;const next=!crouchWanted;if(!next&&!canStandHere()){crouched=true;crouchWanted=false;showToast('NEED CLEARANCE');sendCurrentState(true);return;}setCrouch(next);}
...
function tryJump(){
  if(!shell.canPlay||!matchAllowsMovement(matchState)||hp<=0||traversal)return;const now=performance.now();traversalIntentSeq+=1;traversalIntentUntil=now+560;
...
function applyViewRecoil(weapon,preShotHeat=0){
  const r=recoilSpec(weapon),adsScale=1-.20*Math.max(0,Math.min(1,adsBlend)),heatScale=1+Math.min(.35,Math.max(0,preShotHeat)*.055),pitch=Math.max(0,Number(r.recoilPitch)||0)*adsScale*heatScale,yawKick=Math.max(0,Number(r.recoilYaw)||0)*adsScale,maxPitch=Math.max(pitch,Number(r.recoilMaxPitch)||pitch),step=localRecoilStep[weapon]||0;
  viewRecoilPitch=Math.min(maxPitch,viewRecoilPitch+pitch);
  viewRecoilYaw=THREE.MathUtils.clamp(viewRecoilYaw+RECOIL_YAW_PATTERN[step%RECOIL_YAW_PATTERN.length]*yawKick,-.020,.020);
}
function updateViewRecoil(dt){
  const r=recoilSpec(currentWeapon),delay=Math.max(0,Number(r.recoilRecoveryDelayMs)||0),since=performance.now()-lastLocalShotAt;
  if(since<delay)return;
  const recovery=Math.max(5,Number(r.recoilRecovery)||12),follow=1-Math.exp(-Math.max(0,dt)*recovery);
...
function handleControllerUiNavigation(pressed){
  ...
  if(pressed[GAMEPAD_BUTTON.LB]||pressed[GAMEPAD_BUTTON.RB]){
  ...
  if(pressed[GAMEPAD_BUTTON.DPAD_LEFT]){if(adjustControllerField(focus,-1))return true;moveControllerUiFocus(-1,0);return true;}
  if(pressed[GAMEPAD_BUTTON.DPAD_RIGHT]){if(adjustControllerField(focus,1))return true;moveControllerUiFocus(1,0);return true;}
  if(pressed[GAMEPAD_BUTTON.DPAD_UP]){moveControllerUiFocus(0,-1);return true;}
  if(pressed[GAMEPAD_BUTTON.DPAD_DOWN]){moveControllerUiFocus(0,1);return true;}
  return false;
}
...
function updateGameFrame(dt){
  ...
  const cameraCorrectionX=traversal?0:correctionViewX,cameraCorrectionZ=traversal?0:correctionViewZ;
...
function computeHudLayout(){
  ...
  const baseKillW=compact?220:(isTouch?252:300),killH=compact?70:(isTouch?104:128);
  const teamX=(viewW-teamW)/2,teamY=safe.top+margin,moveBoundary=viewW*MOBILE_MOVE_ZONE_RATIO,chatW=compact?42:48;
  const killX=safe.left+margin,godX=teamX-31,killW=Math.max(118,Math.min(baseKillW,godX-killX-7));
  ...
  const mapX=viewW-safe.right-margin-mapSize,menuX=teamX+teamW+7,rightChatX=menuX+menuW+5,chatBesideMenu=rightChatX+chatW+5<=mapX;
  return{compact,safe,moveBoundary,
    map:{x:mapX,y:safe.top+margin,w:mapSize,h:mapSize},kill:{x:killX,y:safe.top+margin,w:killW,h:killH},
    team:{x:teamX,y:teamY,w:teamW,h:teamH},god:{x:godX,y:teamY,w:25,h:teamH},chat:{x:chatBesideMenu?rightChatX:mapX+mapSize-chatW,y:chatBesideMenu?teamY:safe.top+margin+mapSize+6,w:chatW,h:menuH},menu:{x:menuX,y:teamY,w:menuW,h:menuH},
...
function drawHud(now){
  ...
  drawChatFeed(c,L,w,h,now);if(isTouch)drawChatButton(c,L.chat);
  drawAnnouncer(c,w,h,now);drawMatchStatus(c,w,h);
  if(touchGameplayControlsVisible())drawTouchControls(c,L,now);
  ...
  if(hp<=0){
    c.fillStyle='rgba(24,3,8,.56)';c.fillRect(0,0,w,h);c.textAlign='center';c.fillStyle='#ff6676';c.font=`1000 ${L.compact?36:50}px system-ui`;c.fillText('ELIMINATED',w/2,h/2-34);
...
function drawMatchStatus(c,w,h){
  ...
  if(matchState.status==='ended'){
    ...
    const panelW=Math.min(470,w-30),panelH=Math.min(235,h-30),x=(w-panelW)/2,y=Math.max(15,(h-panelH)/2-6),restart=Math.max(0,Math.ceil(((Number(matchState.restartAt)||serverNow())-serverNow())/1000));
    ...
    c.fillStyle='#fff';c.font='900 14px system-ui';if(teamMode){const t=teamScores();c.fillText(`${t.blue}  BLUE   ·   RED  ${t.red}`,w/2,y+88);}else c.fillText(matchState.winnerName?`${String(matchState.winnerName).toUpperCase()} WINS`:'ROUND COMPLETE',w/2,y+88);
    ...
    c.fillStyle=HUD_MUTED;c.font='800 11px system-ui';c.fillText(`Round ${matchState.round} · ${restart>0?`next round in ${restart}s`:'preparing next round'}`,w/2,y+151);c.fillText(`${controllerInputActive()?'VIEW':isTouch?'Tap score bar':'TAB'} for scoreboard`,w/2,y+176);c.restore();return;
  }
  const warm=matchState.status==='warmup',text=warm?matchClockText():'WAITING FOR MATCH',sub=warm?`Round ${matchState.round}`:'Waiting for players';
...
function drawTeamBar(c,r){
  ...
  if(spec.scoreType==='player'){
    ...
    c.textAlign='center';c.font='1000 10px system-ui';c.fillStyle=matchState.status==='ended'?HUD_ACCENT:'#dbe4ea';c.fillText(clock,mid,r.y+r.h/2-4);c.font='800 10px system-ui';c.fillStyle='#69747d';c.fillText(`FFA · FIRST ${limit}`,mid,r.y+r.h/2+7);
  ...
  c.textAlign='center';c.font='1000 10px system-ui';c.fillStyle=matchState.status==='ended'?HUD_ACCENT:'#dbe4ea';c.fillText(clock,mid,r.y+r.h/2-4);c.font='800 10px system-ui';c.fillStyle='#69747d';c.fillText(spec.scoreType==='none'?'SANDBOX':`${matchCustom?'CUSTOM · ':''}FIRST ${Math.max(1,matchState.scoreLimit||spec.scoreLimit)}`,mid,r.y+r.h/2+7);
...
function drawWeapon(c,r){
  ...
  c.fillStyle=HUD_MUTED;c.font='850 10px system-ui';c.fillText(`${mode} · ${currentWeapon==='sniper'?sniperZoomLabel():(adsWanted?'ADS':'HIP')}`,left,top+13);
  c.textAlign='right';c.fillStyle=unlimited?HUD_ACCENT:count<=3?'#ff747d':'#fff';c.font=`1000 ${r.h<=60?22:24}px system-ui`;c.fillText(unlimited?'∞':String(count),right-23,top+6);c.fillStyle='#69747d';c.font='900 10px system-ui';c.fillText(unlimited?'∞':`/ ${spec.mag}`,right,top+8);
...
function minimapObstacles(){
  const out=[];
  for(const o of STATIC_BOXES)out.push({type:'box',x:o.x,z:o.z,w:o.w,d:o.d,kind:o.kind||'cover'});
  for(const b of BUILDINGS)out.push({type:'building',x:b.x,z:b.z,w:b.w,d:b.d});
  for(const p of PYRAMIDS)out.push({type:'pyramid',x:p.x,z:p.z,base:p.base});
  for(const o of NATURAL_OBSTACLES)out.push({type:o.type,x:o.x,z:o.z,r:o.r,h:o.h});
  return out;
}
function getMinimapStatic(w=512,h=512){
  ...
  const q=document.createElement('canvas');q.width=Math.max(1,Math.round(w));q.height=Math.max(1,Math.round(h));const c=q.getContext('2d'),iw=q.width,ih=q.height,terrainCells=32,cellW=iw/terrainCells,cellH=ih/terrainCells;
  c.fillStyle='rgba(19,27,26,.94)';c.fillRect(0,0,iw,ih);
  for(let gy=0;gy<terrainCells;gy++)for(let gx=0;gx<terrainCells;gx++){const wx=-mapLimit+(gx+.5)/terrainCells*mapLimit*2,wz=-mapLimit+(gy+.5)/terrainCells*mapLimit*2,hv=terrainHeight(wx,wz),t=Math.max(0,Math.min(1,(hv+2.4)/16.2));c.fillStyle=`rgba(${Math.round(58+72*t)},${Math.round(78+46*t)},${Math.round(57+31*t)},${.34+.34*t})`;c.fillRect(gx*cellW,gy*cellH,cellW+.7,cellH+.7);}
  const toX=x=>(x+mapLimit)/(mapLimit*2)*iw,toY=z=>(z+mapLimit)/(mapLimit*2)*ih;
  for(const b of minimapObstacles()){
    if(b.type==='box'||b.type==='building'){
      const x1=toX(b.x-b.w/2),x2=toX(b.x+b.w/2),y1=toY(b.z-b.d/2),y2=toY(b.z+b.d/2);c.fillStyle=b.type==='building'?'rgba(226,231,234,.64)':'rgba(190,199,204,.43)';c.fillRect(x1,y1,x2-x1,y2-y1);
      if(b.type==='building'){c.strokeStyle='rgba(255,255,255,.30)';c.lineWidth=Math.max(1,iw/512);c.strokeRect(x1+.5,y1+.5,Math.max(0,x2-x1-1),Math.max(0,y2-y1-1));}
    }else if(b.type==='pyramid'){
      const px=toX(b.x),py=toY(b.z),rr=b.base/(mapLimit*2)*iw*.55;c.beginPath();c.moveTo(px,py-rr);c.lineTo(px+rr,py+rr);c.lineTo(px-rr,py+rr);c.closePath();c.fillStyle='rgba(207,199,170,.62)';c.fill();
    }else{
      const px=toX(b.x),py=toY(b.z),rr=Math.max(1.6,(b.r||1)/(mapLimit*2)*iw*1.6);c.beginPath();c.arc(px,py,rr,0,Math.PI*2);c.fillStyle=b.type==='tree'?'rgba(41,101,52,.84)':b.type==='bush'?'rgba(65,116,59,.76)':'rgba(148,154,153,.72)';c.fill();
    }
  }
  minimapStaticCache={key,canvas:q};return q;
}
function renderLobbyMapPreview(){
  if(!lobbyMapPreview||!shell.inLobby)return;
  const rect=lobbyMapPreview.getBoundingClientRect(),cssW=Math.max(240,Math.round(rect.width||640)),cssH=Math.max(170,Math.round(rect.height||360)),dpr=Math.min(2,devicePixelRatio||1),pw=Math.round(cssW*dpr),ph=Math.round(cssH*dpr);
  if(lobbyMapPreview.width!==pw)lobbyMapPreview.width=pw;if(lobbyMapPreview.height!==ph)lobbyMapPreview.height=ph;
  const c=lobbyMapPreview.getContext('2d');c.setTransform(dpr,0,0,dpr,0,0);c.clearRect(0,0,cssW,cssH);c.fillStyle='#080d10';c.fillRect(0,0,cssW,cssH);
  const pad=12,size=Math.max(1,Math.min(cssW-pad*2,cssH-pad*2)),x=(cssW-size)/2,y=(cssH-size)/2,staticMap=getMinimapStatic(768,768);c.drawImage(staticMap,x,y,size,size);
  c.strokeStyle='rgba(255,255,255,.22)';c.lineWidth=1;c.strokeRect(x+.5,y+.5,size-1,size-1);
  c.fillStyle='rgba(5,8,10,.72)';c.fillRect(x,y,34,20);c.fillStyle='#fff';c.textAlign='center';c.textBaseline='middle';c.font='1000 11px system-ui';c.fillText('N',x+17,y+10);
}
