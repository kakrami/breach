...
function renderLoadoutOverview(surface,draft){
  ...
  if(surface==='lobby'){const startBtn=document.querySelector('[data-lobby-starting-class]');if(startBtn){const isStarting=selected===currentLobbyStartingClassId();startBtn.textContent=isStarting?'STARTING CLASS':'SET AS STARTING';startBtn.classList.toggle('active',isStarting);startBtn.setAttribute('aria-pressed',String(isStarting));startBtn.disabled=isStarting||!matchAllowsLobbyEdits(matchState)||socket?.readyState!==WebSocket.OPEN;}}
}
...
function bindLoadoutWorkspaceTabs(){
  const startingClassButton=document.querySelector('[data-lobby-starting-class]');if(startingClassButton){startingClassButton.dataset.controllerKey='lobby-starting-class';startingClassButton.addEventListener('click',()=>setLobbyStartingClass(loadoutEditClass.lobby));}
...
function makeLaserAttachment({x=.035,z=-.40,y=-.09,scale=1}={}){const g=new THREE.Group(),mat=attachmentVisualMaterial(0x191e21,.22,.54),lensMat=new THREE.MeshBasicMaterial({color:0xff302c,transparent:true,opacity:.96,depthWrite:false,toneMapped:false}),body=new THREE.Mesh(new THREE.BoxGeometry(.055*scale,.042*scale,.105*scale),mat),lens=new THREE.Mesh(new THREE.CircleGeometry(.010*scale,12),lensMat);body.position.set(x,y,z);lens.position.set(x,y,z-.053*scale);g.add(body,lens);return g;}
...
function weaponShotSoundId(weapon='pistol',suppressed=false){
  if(suppressed){if(weapon==='pistol')return'shotPistolSuppressed';if(weapon==='assault')return'shotAssaultSuppressed';if(weapon==='ump')return'shotUmpSuppressed';if(weapon==='machineGun')return'shotMachineGunSuppressed';if(weapon==='sniper')return'shotSniperSuppressed';}
