...
let lobbyLoadoutDraft=null,lobbyLoadoutDirty=false,lobbyLoadoutRevision=0,lobbyLoadoutAckRevision=0;
...
function lobbySnapshot(){
  const rows=[{id:clientId,name:myName||safeName(),team:myTeam,bot:false,godMode,admin:isMatchAdmin,primaryWeapon,secondaryWeapon,primaryAttachments,secondaryAttachments,tactical:tacticalEquipment,lethal:lethalEquipment,kills:myStats.kills||0,deaths:myStats.deaths||0,self:true}];
...
function refreshLobbyDraftOwnership(){
  ...
  const committedLoadout=normalizeLoadoutChoice(selectedLoadout());if(!lobbyLoadoutDraft||!lobbyLoadoutDirty)lobbyLoadoutDraft={...committedLoadout};renderAttachmentEditors('lobby',lobbyLoadoutDraft);
}
...
function collectLobbyStartSetup(){
  ...
  return{
    ...
    loadout:normalizeLoadoutChoice(lobbyLoadoutDraft||selectedLoadout())
  };
}
function renderLobbySetupControls(){
  if(!shell.inLobby)return;refreshLobbyDraftOwnership();const host=!!isMatchAdmin,draft=lobbyMatchDraft||committedLobbyMatchDraft(),draftSpec=gameModeSpec(draft.mode),draftLoadout=lobbyLoadoutDraft||normalizeLoadoutChoice(selectedLoadout());
...
function setLobbyLoadoutDraft(next={}){
  if(!lobbyLoadoutDraft)lobbyLoadoutDraft=normalizeLoadoutChoice(selectedLoadout());lobbyLoadoutDraft=normalizeLoadoutChoice({...lobbyLoadoutDraft,...next});lobbyLoadoutDirty=!sameLoadoutChoice(lobbyLoadoutDraft,selectedLoadout());renderLobbySetupControls();setLobbyActionState();
  if(!isMatchAdmin&&socket?.readyState===WebSocket.OPEN){const rev=++lobbyLoadoutRevision;send({t:'loadout',rev,...normalizeLoadoutChoice(lobbyLoadoutDraft)});}
}
...
function openMatchLoadout(){if(!shell.inMatch)return;loadoutBaseDraft=normalizeLoadoutChoice(pendingLoadout||selectedLoadout());loadoutDraft={...loadoutBaseDraft};syncMatchLoadoutEditor();shell.openPanel(SHELL_PANEL.LOADOUT);switchLoadoutWeaponTab('match',loadoutFocusSlot.match||'primary');requestAnimationFrame(()=>void ensureLoadoutPreviewEngine());}
function closeMatchLoadout(){closeAttachmentSheet();loadoutDraft=null;loadoutBaseDraft=null;shell.closePanel(SHELL_PANEL.LOADOUT);}
...
function bindUI(){
  ...
  $('lobbyStartBtn').addEventListener('click',async()=>{
    ...
    send({t:'startMatch',setup});$('lobbyStatus').textContent='Starting match…';
  });
...
async function joinMatch(code){
  ...
  shell.beginConnection(`Joining ${code}…`);myName=safeName();myTeam=preferredTeam;godMode=false;primaryWeapon=preferredPrimary;secondaryWeapon=preferredSecondary;primaryAttachments={...preferredPrimaryAttachments};secondaryAttachments={...preferredSecondaryAttachments};tacticalEquipment=preferredTactical;lethalEquipment=preferredLethal;pendingLoadout=null;pendingTeam='';
...
function applyAuthoritativeLoadout(m){
  ...
  if(m.pending===true&&pendingLoadout){rememberPrimary(pendingLoadout.primaryWeapon);rememberSecondary(pendingLoadout.secondaryWeapon);rememberAttachments(pendingLoadout.primaryAttachments,pendingLoadout.secondaryAttachments,pendingLoadout.primaryWeapon,pendingLoadout.secondaryWeapon);rememberEquipment(pendingLoadout.tactical,pendingLoadout.lethal);showToast('LOADOUT QUEUED · NEXT SPAWN');}
  if(shell.inLobby&&lobbyLoadoutDraft)lobbyLoadoutDirty=!sameLoadoutChoice(lobbyLoadoutDraft,selectedLoadout());
