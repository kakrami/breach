...
import * as HighlandsGeometry from './world-geometry.js?v=1.42.1';
import * as DepotGeometry from './world-geometry-depot.js?v=1.42.1';
import * as YardGeometry from './world-geometry-yard.js?v=1.42.1';
import * as RigGeometry from './world-geometry-rig.js?v=1.42.1';
import * as HighlandsWorldCollision from './world-collision.js?v=1.42.1';
import * as DepotWorldCollision from './world-collision-depot.js?v=1.42.1';
import * as YardWorldCollision from './world-collision-yard.js?v=1.42.1';
import * as RigWorldCollision from './world-collision-rig.js?v=1.42.1';
import {
...
} from './game-config.js?v=1.42.1';
import { createProjectileCollisionGrid } from './collision-grid.js?v=1.42.1';
import { createAudioEngine } from './audio-engine.js?v=1.42.1';
import { normalizeMatchState as normalizeSharedMatchState } from './match-model.js?v=1.42.1';
import { MATCH_STATUS, matchAllowsLobbyEdits, matchAllowsMovement, matchAllowsCombat, matchPhaseChanged } from './gameplay-phase.js?v=1.42.1';
import { MAX_PLAYER_PHYSICS_STEP_SEC, advanceVerticalMotion, advanceKnockback, sweepHorizontalMovement, createTraversalPlan, traversalPose, tacticalThrowVelocity, LADDER_CLIMB_SPEED, ladderById, ladderClimbPoint, ladderBottomExitPoint, ladderTopExitPoint, findLadderEntry, ladderClimbStep } from './movement-model.js?v=1.42.1';
import { SHELL_PANEL, createSessionShell, detectInputPlatform } from './app-lifecycle.js?v=1.42.1';
import { GAMEPAD_BUTTON, createGamepadInput } from './gamepad-input.js?v=1.42.1';
...
function bindLoadoutWorkspaceTabs(){
  for(const button of document.querySelectorAll('[data-loadout-edit-item]'))button.addEventListener('click',()=>{const [surface,item]=String(button.dataset.loadoutEditItem||'').split('-');loadoutAttachmentTrayOpen.delete(`${surface}-${item}`);loadoutWeaponPickerOpen.delete(`${surface}-${item}`);setLoadoutWorkspaceMode(surface,'item',{item});if(item==='primary'||item==='secondary')renderAttachmentEditor(surface,item,loadoutDraftForSurface(surface));});
  for(const button of document.querySelectorAll('[data-loadout-back-class]'))button.addEventListener('click',()=>setLoadoutWorkspaceMode(button.dataset.loadoutBackClass,'class',{ensurePreview:false}));
  for(const button of document.querySelectorAll('[data-loadout-back-classes]'))button.addEventListener('click',()=>setLoadoutWorkspaceMode(button.dataset.loadoutBackClasses,'classes',{ensurePreview:false}));
  for(const button of document.querySelectorAll('[data-loadout-weapon-picker-toggle]'))button.addEventListener('click',()=>{const key=button.dataset.loadoutWeaponPickerToggle,[surface,slot]=key.split('-');if(loadoutWeaponPickerOpen.has(key))loadoutWeaponPickerOpen.delete(key);else{loadoutWeaponPickerOpen.add(key);loadoutAttachmentTrayOpen.delete(key);}renderAttachmentEditor(surface,slot,loadoutDraftForSurface(surface));renderLoadoutStats(surface,slot,loadoutSlotWeapon(loadoutDraftForSurface(surface),slot),loadoutSlotAttachments(loadoutDraftForSurface(surface),slot));});
...
function renderAttachmentEditor(surface,weaponSlot,draft){
  ...
  const head=document.createElement('header');head.className='gunsmith-attachment-head';const title=document.createElement('div'),eyebrow=document.createElement('span');eyebrow.textContent='ATTACHMENT SLOT';const strong=document.createElement('strong');strong.textContent=activeSlot.toUpperCase();title.append(eyebrow,strong);const close=document.createElement('button');close.type='button';close.className='gunsmith-tray-close';close.textContent='×';close.setAttribute('aria-label','Close attachment options');close.addEventListener('click',()=>{loadoutAttachmentTrayOpen.delete(key);loadoutAttachmentSlot[key]='';renderAttachmentEditor(surface,weaponSlot,loadoutDraftForSurface(surface));renderLoadoutStats(surface,weaponSlot,weapon,loadoutSlotAttachments(loadoutDraftForSurface(surface),weaponSlot));});head.append(title,close);
...
const ENGINE_MODULE_URL = './vendor/three.module.min.js?v=1.42.1';
...
function renameLoadoutClass(surface,id,value){const classId=normalizeLoadoutClassId(id);if(surface==='lobby'){if(lobbyLoadoutDraft)lobbyClassDrafts=writeClassLoadout(lobbyClassDrafts||loadoutClasses,loadoutEditClass.lobby,lobbyLoadoutDraft);const classes=normalizeLoadoutClasses(lobbyClassDrafts||loadoutClasses,selectedLoadout()),idx=classes.findIndex(c=>c.id===classId);if(idx<0)return;classes[idx]={...classes[idx],name:normalizeLoadoutClassName(value,idx)};lobbyClassDrafts=classes;lobbyLoadoutDirty=!classSetEqual(lobbyClassDrafts,loadoutClasses)||normalizeLoadoutClassId(loadoutEditClass.lobby)!==normalizeLoadoutClassId(activeClassId);renderLoadoutClassStrip('lobby');renderLoadoutOverview('lobby',lobbyLoadoutDraft||classLoadout(classes,loadoutEditClass.lobby));setLobbyActionState();if(!isMatchAdmin&&socket?.readyState===WebSocket.OPEN){const rev=++lobbyLoadoutRevision;send({t:'loadout',rev,classId:normalizeLoadoutClassId(loadoutEditClass.lobby),loadoutClasses:lobbyClassDrafts,...normalizeLoadoutChoice(lobbyLoadoutDraft||classLoadout(classes,loadoutEditClass.lobby))});}}else{if(loadoutDraft)matchClassDrafts=writeClassLoadout(matchClassDrafts||loadoutClasses,loadoutEditClass.match,loadoutDraft);const classes=normalizeLoadoutClasses(matchClassDrafts||loadoutClasses,selectedLoadout()),idx=classes.findIndex(c=>c.id===classId);if(idx<0)return;classes[idx]={...classes[idx],name:normalizeLoadoutClassName(value,idx)};matchClassDrafts=classes;syncMatchLoadoutEditor();}}
function renderLoadoutClassStrip(surface){const host=document.querySelector(`[data-loadout-class-strip="${surface}"]`);if(!host)return;const classes=loadoutClassesForSurface(surface),baseline=surface==='lobby'?normalizeLoadoutClasses(loadoutClasses,selectedLoadout()):normalizeLoadoutClasses(matchClassBase||loadoutClasses,selectedLoadout()),selected=normalizeLoadoutClassId(loadoutEditClass[surface]||activeClassId);host.replaceChildren();for(const item of classes){const base=loadoutClassById(baseline,item.id,selectedLoadout()),dirty=item.name!==base.name||!loadoutChoiceEqual(item,base),card=document.createElement('div');card.className='loadout-class-card';card.classList.toggle('selected',item.id===selected);card.classList.toggle('equipped',item.id===activeClassId);card.classList.toggle('pending',item.id===pendingClassId&&!!pendingLoadout);card.classList.toggle('dirty',dirty);card.setAttribute('role','listitem');const main=document.createElement('button');main.type='button';main.className='loadout-class-main';main.dataset.loadoutClass=item.id;const head=document.createElement('div');head.className='loadout-class-card-head';const name=document.createElement('strong');name.textContent=item.name;const state=document.createElement('span');state.textContent=item.id===pendingClassId&&pendingLoadout?'NEXT':item.id===activeClassId?'ACTIVE':dirty?'EDITED':'';head.append(name,state);const weapons=document.createElement('div');weapons.className='loadout-class-weapons';const pmods=attachmentCount(item.primaryAttachments),smods=attachmentCount(item.secondaryAttachments);weapons.innerHTML=`<span><b>PRIMARY</b>${WEAPON_SPECS[item.primaryWeapon]?.name||item.primaryWeapon}${pmods?` · ${pmods} MOD${pmods===1?'':'S'}`:''}</span><span><b>SECONDARY</b>${WEAPON_SPECS[item.secondaryWeapon]?.name||item.secondaryWeapon}${smods?` · ${smods} MOD${smods===1?'':'S'}`:''}</span>`;const equipment=document.createElement('small');equipment.textContent=`${EQUIPMENT_SPECS[item.tactical]?.short||item.tactical} · ${EQUIPMENT_SPECS[item.lethal]?.short||item.lethal}`;main.append(head,weapons,equipment);main.addEventListener('click',()=>switchLoadoutClass(surface,item.id));const rename=document.createElement('button');rename.type='button';rename.className='loadout-class-rename';rename.textContent='✎';rename.setAttribute('aria-label',`Rename ${item.name}`);rename.dataset.textMode='class';rename.dataset.maxlength='18';rename.dataset.placeholder=`CLASS ${classes.indexOf(item)+1}`;rename.value=item.name;rename.addEventListener('click',e=>{e.stopPropagation();openGameTextEditor(rename);});rename.addEventListener('change',()=>renameLoadoutClass(surface,item.id,rename.value));card.append(main,rename);host.append(card);}}
function switchLoadoutClass(surface,id){const nextId=normalizeLoadoutClassId(id);if(surface==='lobby'){if(lobbyLoadoutDraft)lobbyClassDrafts=writeClassLoadout(lobbyClassDrafts||loadoutClasses,loadoutEditClass.lobby,lobbyLoadoutDraft);loadoutEditClass.lobby=nextId;lobbyLoadoutDraft=classLoadout(lobbyClassDrafts||loadoutClasses,nextId);lobbyLoadoutDirty=!classSetEqual(lobbyClassDrafts||loadoutClasses,loadoutClasses)||nextId!==activeClassId;renderLobbySetupControls();setLobbyActionState();if(!isMatchAdmin&&socket?.readyState===WebSocket.OPEN){const rev=++lobbyLoadoutRevision;send({t:'loadout',rev,classId:nextId,loadoutClasses:lobbyClassDrafts,...lobbyLoadoutDraft});}}else{if(loadoutDraft)matchClassDrafts=writeClassLoadout(matchClassDrafts||loadoutClasses,loadoutEditClass.match,loadoutDraft);loadoutEditClass.match=nextId;loadoutBaseDraft=classLoadout(matchClassBase||loadoutClasses,nextId);loadoutDraft=classLoadout(matchClassDrafts||loadoutClasses,nextId);syncMatchLoadoutEditor();}setLoadoutWorkspaceMode(surface,'class',{ensurePreview:false});renderLoadoutOverview(surface,loadoutDraftForSurface(surface));}
...
function buildLoadoutPreviewModel(weapon,attachments){
  const root=new THREE.Group(),sources=weapon==='akimbo1887'?[akimboLeftGroup,akimboRightGroup]:[activeFirstPersonWeaponGroup(weapon)].filter(Boolean);if(!sources.length)return root;let sightPoint=null,calloutPoints={};
...
function openGameTextEditor(target){
  if(!target||target.disabled)return false;gameTextEditorTarget=target;gameTextEditorDraft=String(target.value||'');gameTextEditorShift=gameTextMode()==='callsign'||gameTextMode()==='class';
...
function handleControllerUiNavigation(pressed){
  ...
  if(pressed[GAMEPAD_BUTTON.B]){
    ...
    if(shell.panel===SHELL_PANEL.LOADOUT){if(loadoutWorkspaceMode.match==='item'){setLoadoutWorkspaceMode('match','class',{ensurePreview:false});return true;}if(loadoutWorkspaceMode.match==='class'){setLoadoutWorkspaceMode('match','classes',{ensurePreview:false});return true;}closeMatchLoadout();return true;}
    if(shell.inLobby&&loadoutWorkspaceMode.lobby!=='classes'&&document.querySelector('[data-lobby-side-tab="loadout"]')?.classList.contains('active')){setLoadoutWorkspaceMode('lobby',loadoutWorkspaceMode.lobby==='item'?'class':'classes',{ensurePreview:false});return true;}
